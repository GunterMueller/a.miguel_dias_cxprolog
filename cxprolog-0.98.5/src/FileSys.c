...
#define IsEndOfSeg(c)		(c == '\0' || IsSep(c))
...
{						// test: absolute_file_name('aaaaaaaaaaaaaaaaaaaaaa/x/y', X).
	register CharPt s = cCharPt(fname);
 	register CharPt a = s;
	do {
         if( IsEndOfSeg(*a) ) {
            if( a == s )												/* skip / */
                ;
            elif( s[0] == '.' && IsEndOfSeg(s[1]) )						/* skip . */
                ;
            elif( s[0] == '.' && s[1] == '.' && IsEndOfSeg(s[2]) ) {	/* up .. */
                if( list == tNilAtom )
                ...
                list = MakeList(MakeTempAtomN(s, a - s), list) ;
            s = a + 1 ;
        }
    } while( *a++ != '\0' ) ;
    return list ;
...
static void FileSysBasicGCMark(void)
{
...
static void PFSProperty(void)
{
	...
	switch( fprop[0] ) {
		case 'r': if( StrEqual(fprop, "readable") )
					t = MakeBool(OSPropReadable(fname)) ;
				break ;
		case 'w': if( StrEqual(fprop, "writable") )
					t = MakeBool(OSPropWritable(fname)) ;
				break ;
