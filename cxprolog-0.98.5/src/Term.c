...
Bool TermIsCyclic(Pt term) /* Handles cyclic terms */
{
	...
	for(;;) {
		...
		if( IsStruct(t) ) {
#if 0
			Mesg("IsStruct %s", XStructNameArity(t));
#endif
...
Pt TermUnserialize(Hdl array, Size len)
{
	...
	for( i = 0 ; ScratchDistToSaved() != 1 ; i++ ) {
		if( i < len && array[i] != nil ) {
			...
			int arity = ScratchCurr() - ScratchBegin() - 1 ;
			Pt t = *ScratchBegin() ;
			if( arity > 0 ) {
				if( IsAtom(t) ) {
					FunctorPt f = LookupFunctor(XAtom(t), arity) ;
					t = MakeStruct(f, ScratchBegin() + 1) ;
				}
				else
...
static void PSerializeTest(void)
{
...
/*
t(c(a,X,X)) :- X=1+2*3.
t(Y), writeln(Y), copy_term(Y,Z), copy_term(Z,W).

t(c(X,X)) :- X=1*2+3.
:- t(Y), '$term_internals'(Y), copy_term(Y,Z), '$term_internals'(Z).
...
redo:
		...
		if( IsStruct(t1) ) {
			if( convUnitParams && IsUnitParam(t1) ) {
				...
				if( IsVar(t1) && Le(bottom,t1) && Lt(t1,to) ) { /* Already visited? */
...
// QQQ Mesg("GROW = %ld %s", size, TermAsStr(ZT));
	return ZT ;		
}
					...
					Bool convUnitParams, Bool cyclesAllowed)
{
    Pt t ;
    Size size ;
    while( (t = CompoundTermCopy(term, scribblingBegin, scribblingEnd,
                        env, permanentExtras, convUnitParams,
                        cyclesAllowed, &size)) == nil
    ) ScribblingExpand();
#if 0
    ...
    CompoundTermRelocate(mem, scribblingBegin, size) ;
    return IsStruct(t) ? TagStruct(mem) : TagList(mem) ;
}

/*
?- i:=f(X,X), i=:Z, arg(1,Z,c(1)), j:=Z.
{size = 3.}
{size = 7.}
*/
...
Str XTestTermName(register Pt t)
{
	...
	if( IsList(t) ) return "." ;
	if( IsNilSpecial(t) ) return FunctorName(nilIsSpecialFunctor) ;
...
FunctorPt XTestFunctor(register Pt t)
{
	...
	if( IsNilSpecial(t) ) return nilIsSpecialFunctor ;
	return TypeError("STRUCT, LIST or ATOM", t) ;
}
...
FunctorPt XTestStruct(register Pt t, Hdl *args)
{
	...
	if( IsNilSpecial(t) ) { *args = H ; return nilIsSpecialFunctor; }
	return TypeError("STRUCT, LIST or ATOM", t) ;
}
...
AtomPt XTermAtom(register Pt t)
{
	...
	if( IsNilSpecial(t) ) return FunctorAtom(nilIsSpecialFunctor) ;
	return nil ;
}
...
static Bool Exists(VoidPt addr)
{
	Hdl j ;
	for( j = ScribblingBegin(); j < scribblingPt ; j += 2 )
		if( Eq(XPt(*j), addr) )
			return true;
	return false;
}

static void Add(VoidPt addr, VoidPt t)
{
	if( !Exists(addr) ) {
		ScribblingPush(addr);
		ScribblingPush(t);
...
static void AddTerm(Hdl addr, Pt t)
{
	...
	if( IsVar(t) ) {	/* free var */
		...
		int i;
		if( Exists(addr) ) /* cycle */
			return;
		Add(addr, t) ;			
		Add(&XStructFunctor(t), XStructFunctor(t)) ;
		for( i = 0 ; i < FunctorArity(f) ; i++ )
			AddTerm(&XStructArg(t, i), XStructArg(t, i)) ;
	}
	elif( IsList(t) ) {
		if( Exists(addr) ) /* cycle */
			return;
		Add(addr, t) ;			
		AddTerm(&XListHead(t), XListHead(t)) ;
		AddTerm(&XListTail(t), XListTail(t)) ;
	}
	else
		Add(addr, t) ;		
}
...
{
	qsort(ScribblingBegin(), ScribblingUsed() / 2, 2 * sizeof(Pt), cmp);
}

void TermInternals(Pt t)
{
	WriteStd("%s\n", TermAsStr(t)) ;
	UseScribbling();
	...
	AddTerm(nil, t);
	QSort();
	for( Hdl3 h = cHdl3(ScribblingBegin()); Lt(h, scribblingPt) ; h += 2 ) {
		Hdl addr = h[0];
		Pt t = cPt(h[1]);
		if( addr == nil )
			Write("%20s: ", " ---> ");
	...
	Unused(FreeScribbling()) ;
}


/* CXPROLOG C'BUILTINS */

static void PVar(void)
{
	Pt t = Drf(X0) ;
	MustBe( IsVar(t) ) ;
}

static void PNonVar(void)
{
	Pt t = Drf(X0) ;
	MustBe( !IsVar(t) ) ;
}

static void PAtom(void)
{
	Pt t = Drf(X0) ;
	MustBe( IsAtom(t) ) ;
}

static void PPInt(void)
{
	Pt t = Drf(X0) ;
	MustBe( IsInt(t) ) ;
}

static void PPFloat(void)
{
	Pt t = Drf(X0) ;
	MustBe( IsFloat(t) ) ;
}

static void PNumber(void)
{
	Pt t = Drf(X0) ;
	MustBe( IsNumber(t) ) ;
}

static void PAtomic(void)
{
	Pt t = Drf(X0) ;
	MustBe( IsAtomicStrict(t) ) ;
}

static void PGround(void)
{
	MustBe( TermIsGround(X0) ) ;
}

static void PCompound(void)
{
	Pt t = Drf(X0) ;
	MustBe( IsCompound(t) ) ;
}

static void PCallable(void)
{
	Pt t = Drf(X0) ;
	while( IsUnitParam(t) )
		t = Drf(Z(XUnitParam(t))) ;
	MustBe( IsCompound(t) || IsAtom(t) || IsNilSpecial(t) ) ;
}
...
static void PAcyclicTerm(void)
{
	MustBe( !TermIsCyclic(X0) ) ;
}

static void PCyclicTerm(void)
{
	MustBe( TermIsCyclic(X0) ) ;
}

static void PCopyTerm(void)
{
	Pt t = ZPushTerm(X0) ; /* stacks may grow */
	MustBe( Unify(X1, t) ) ;
}

static void PCopyTermWithEnv(void)
{
	Pt t = ZPushTermWithEnv(X0, X1, false) ; /* stacks may grow */
	MustBe( Unify(X2, t) ) ;
}

static void PTermVariables(void)
{
	Size nVars ;
	TermVars(X0, &nVars) ;
...
static void PTermInternals(void)
{
	TermInternals(X0);
	JumpNext() ;
}
...
void TermsInit(void)
{
	...
	InstallCBuiltinPred("$term_internals", 1, PTermInternals) ;
