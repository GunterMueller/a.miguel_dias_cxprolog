...
Bool isOperator(AtomPt atom) {
	return AtomToOperator(atom) != nil;
}
...
/*	MakeInfixOperator("div",	   400, true, false) ; */
	...
	MakeInfixOperator("div",	   400, true, false) ;		/* ISO */
	MakeInfixOperator("//",		   400, true, false) ;		/* ISO */
	MakeInfixOperator("<<",		   400, true, false) ;		/* ISO */
	MakeInfixOperator(">>",		   400, true, false) ;		/* ISO */
...
static void POperator(void)
{
	DefineOperator(XTestInt(X0), XTestAtomName(X1), X2) ;
	JumpNext() ;
}

static void PResetOperators(void)
{
	ResetOperators() ;
	JumpNext() ;
}

static void PNDCurrentOperator(void)
{
...
static void POperatorFlag3(void)
{
...
static void POperatorFlag4(void)
{
	OperatorPt op = AtomToOperator(XTestAtom(X0)) ;
	Str fl = XTestAtomName(X1) ;
	Ensure( op != nil ) ;
