...
static BigInt ReadInt(int base)
{
	register BigInt i = 0 ;
	register int n ;
	for(;;) {
...
static double ReadReal(BigInt ipart)
{
	BigInt dpart = 0, epart = 0 ;
	int esig = 1 ;
	double d ;
	char s[512] ;
	if( cc == '.' ) {
	...
	sprintf(s, "%ld.%lde%ld", cLong(ipart), cLong(dpart), esig*cLong(epart)) ;
	if( sscanf(s, "%lf", &d) != 1 )
