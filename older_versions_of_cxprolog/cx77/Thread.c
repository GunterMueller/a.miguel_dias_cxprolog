...
{
	register ThreadPt th = TemporaryAllocate(WordsOf(Thread)) ;
...
static void ThreadTransfer(ThreadPt th, Pt message)
{
	...
	ThreadRestore(th) ;
	if( P == nil )
		CalledFromC(PushTerm(ThreadStartGoal(th))) ;
...
{
	ThreadSave(active) ;	/* trailBegin, stacksBegin might have changed */
	ThreadReset(active) ;
	ThreadRestore(active) ;
	CalledFromC(PushTerm(ThreadRestartGoal(active))) ;
	EventContinue() ;
}
