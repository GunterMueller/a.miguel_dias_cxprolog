...
int BufferAccessChar(BufferPt b, Bool advance)
{
	...
	int len ;
	if( BufferPosition(b) >= BufferLast(b) ) return eofMark ;
	len = mbtowc(&c, BufferPosition(b), BufferLast(b)-BufferPosition(b)) ;
	if( len <= 0 ) {
		if( len == 0 ) len = 1 ;
	...
	return c ;
#else
	return BufferAccessByte(b, advance) ;
#endif
}
...
void BuffersInit()
{
	bufferType = ExtraTypeNew("BUFFER", WordsOf(Buffer), nil) ;
