...
static int FindAvailableStream(void)
{
	register int i ;
...
void Flush(CharPt name)
{
	...
	if( *name == '\0' )
		FlushStream(dflOutputStream) ;
	elif( (i = FindStream(name)) != -1 )
		FlushStream(i) ;
}
...
CharPt GetLine()
{
	...
	for( b = buffer ; b < bufferEnd ; b++ ) {
		c = GetStream(inputStream) ;
		if( c == '\n' || c == eofCode ) break ;
		*b = c ;
	}
	*b = '\0' ;
	if( b == bufferEnd ) /* @@ buffer overflow */
