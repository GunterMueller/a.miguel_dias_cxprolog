...
typedef struct Predicate
{
	...
	Bool indexMade : 1 ;			/* Index already made */
	...
	Bool reconsulted : 1 ;			/* Predicate already reconsulted */
	Bool hasDeletedClauses : 1 ;	/* Predicate has deleted clauses */
...
#define PredReconsulted(pr)			( (pr)->reconsulted )
#define PredHasDeletedClauses(pr)	( (pr)->hasDeletedClauses )

#define PredIsBuiltinAux(pr)		(PredName(pr)[0] == '$')
#define PredIsTraceable(pr)			( (pr)->traceable )

/* There are 5 kind of predicates:
	Import		- no clauses
	Undefined	- no clauses
	C determ	- no clauses
	C nondeterm	- 2 clauses
	Regular		- 1 ou mor eclauses
*/
