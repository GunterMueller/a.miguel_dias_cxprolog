...
/* Make space for special buffer needed by c-nondeterm predicates */
	if( PredIsCNonDeterm(d->pred) ) arity++ ;
...
{
	switch( s[0] ) {
		case 'o': if( StrEqual(s, "off") ) return eCall - 1 ; else break ;
...
/* Initializes the special buffer needed by c-nondeterm predicates */
	...
	CP = &(d->exitInst) ;
	P = d->currClause == nil
			? PredCode(pr)	/* c-determ, undef, import */
...
{
	DebugEvent e ;
	if( (int)(e = StrToTopLeash(XTestAtomName(X0))) < (int)(eCall - 1) )
...
void DebugInit()
{
	InstallRelocateStacksHandler(RelocateDebugger) ;
