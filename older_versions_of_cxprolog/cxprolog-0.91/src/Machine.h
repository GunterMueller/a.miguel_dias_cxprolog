...
/* The macros JumpNext(), Jump(v) and DoFail() allows
   for the future introduction of threaded code */

#define	JumpNext()		return
#define Jump(v)			Do( (*cVFun(v))() ; JumpNext() ; )
#define DoFail()		Do( P = Bf(P) ; JumpNext() ; )

#define Ensure(c)		Do( if( !(c) ) { P = Bf(P) ; JumpNext() ; } )
#define MustBe(c)		Do( if( c ) JumpNext() ;	\
							else { P = Bf(P) ; JumpNext() ; } )
