...
static Size ExtraForEachHideHidden(ExtraTypePt e, ExtraFun fun)
{
	register ExtraPt x, next ;
	Size n = 0 ;
	doseq(x, ExtraTypeFirst(e), next) {
...
void ExtraPNDCurrent(ExtraTypePt e, BFunV bfun, int arity, int resPos)
{
	...
	if( A(arity) == tNilAtom ) {
		if( !IsVar(Drf(Xc(resPos))) ) {
			Discard() ;
			MustBe( !ExtraIsHidden(x)
					&& (bfun == nil || bfun(XTestExtra(e,Xc(resPos)))) ) ;
		}
...
void ExtraShow(ExtraTypePt e, ExtraFun fun)
{
	...
	ShowVersion() ;
	Write("%sS:\n", ExtraTypeName(e)) ;
	n = ExtraForEachHideHidden(e, fun) ;
	if( n == 0 ) Write("    %% None\n") ;
	JumpNext() ;
}
...
static ExtraTypePt XExtraFind(Pt t) 
{
	register int i ;
	Word tag ;
	t = Drf(t) ;
	if( !IsExtra(t) )
		InternalError("XExtraFind (1)") ;
	tag = XExtraTag(t) ;
	dotimes(i, nExtraTypes)
		if( allExtraTypes[i].tag == tag )
			return allExtraTypes + i ;
	return InternalError("XExtraFind (2)") ;
}
...
{
	ExtraTypePt e = XExtraFind(t) ;
	VoidPt x = XExtra(Drf(t)) ;
	CharPt n = ExtraTypeNamingFun(e) != nil ? ExtraTypeNamingFun(e)(x) : nil ;
	if( n != nil )
		return n ;
	else return GStrFormat("1'%s_%lx", ExtraTypeName(e), cWord(x)) ;
}
