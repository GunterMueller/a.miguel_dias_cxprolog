...
static PredicatePt SearchContext(FunctorPt f)
{
	...
	if( undefWarnings_flag )
		Warning("Predicate '%s' is not visible in context %s",
							FunctorNameArity(f), WriteTermToStr(c)) ;
	return nil ;
}
...
void ContinueMachine()
{
	Run() ;
	InternalError("ContinueMachine") ;
}
...
void CheckHostSpeed()
{
	...
	do {
		...
		Pt t = ReadTermFromStr(s, nil) ;
		WriteTermToStr(t) ;
		H = saveH ;
		hostSpeed++ ;
	} while( CurrTime() - tt < 0.05 ) ;
...
#define AT_EVERY_CALL(n,a)					\
	{	if( traceActive_flag && inTopGoal_flag )	\
			DebugCall(n, a) ;				\
...
{
	AT_EVERY_CALL(PredName(LookPred()), PredArity(LookPred()))
...
static void ExecuteVarInst()
{
	...
	if( IsStruct(t) ) {
		...
		t = cPt(XStructFunctor(t)) ;
	}
	elif( IsAtomOrText(t) ) {
		t = cPt(LookupFunctor(XAtomOrTextAsAtom(t), 0)) ;
	}
	elif( IsList(t) ) {
		X0 = Drf(XListHead(t)) ;
		X1 = Drf(XListTail(t)) ;
...
{
	AT_EVERY_CALL(FunctorName(cFunctorPt(t)), FunctorArity(cFunctorPt(t)))
...
{
	/* Got here from SwitchOnTerm: drf already constains Drf(X0) */
