...
#define StrOverflow()		( strPut >= bigBufferEnd )
...
static void AddQString(CharPt s, Bool dirty)
{
	s = dirty ? Quote(s) : s ;
	AddStr(s) ;
}
...
{	
	if( wMode != display && wMode != quoted )
	  ...
	  switch( allChars[name[0]] ) {
		case _SO:
		case ',': {
			AddQString(name, name[1] != '\0') ;
			break ;
		}
		case '[': {
			AddQString(name, name[1] != ']' || name[2] != '\0') ;
			break ;
		}
		case '{': {
			AddQString(name, name[1] != '}' || name[2] != '\0') ;
		...
		default: {
			...
			AddQString(name, dirty || name[0] == '\0') ;
			break ;
		}
	}
}
...
static void WriteTerm2(register Pt term, int p)
{
	...
	if( IsAtomOrText(term) ) {
		...
		{ sprintf(strBuffer, "%ld", XInt(term)) ; AddStr(strBuffer) ; }
	elif( IsReal(term) )
		{ sprintf(strBuffer, "%.5g", XReal(term)) ; AddStr(strBuffer) ; }
	elif( IsExtra(term) )
		...
		{ sprintf(strBuffer, "<UNKNOWN TAG::%lx>", GetTag(term)) ; AddStr(strBuffer) ; }
}
...
static void PQuote()
{
	if( Unify(X1, MakeAtomOrText(Quote(XTestAtomName(X0)))) )
		JumpNext()
	DoFail()
}
