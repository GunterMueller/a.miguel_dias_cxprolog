...
CharPt bigBuffer, bigBufferEnd ;
Hdl codeBuffer, codeBufferEnd ;	/* TermWrite, TermRead, Compiler, Index, 
...
static void InitBuffers()
{
	...
	bigBuffer = cCharPt(codeBuffer) ;
	bigBufferEnd = cCharPt(codeBufferEnd) ;
...
static void MoreMemory(long atLeast)
{
	...
	if( atLeast > 0 && memoryWarnings_flag )
		Warning("Static area increased from %ldKb to %ldKb",
			(TotalMemory() - Words(memoryBlockSize)) Wd/(1 Kb),
			TotalMemory() Wd/(1 Kb)) ;
}
