...
'$$_top_level' :-
	repeat,
		'@@_top_read'(G,V), 
				% the prompt depends on the term in '@@_env_context'
		'$$_try'(G,V),
	fail.

'$$_try'(X,_) :- var(X), !, X.
'$$_try'(end_of_file,_) :- !, writeln('^D'), halt.
'$$_try'(trace,_) :- !, set_trace(true).
'$$_try'(notrace,_) :- !, set_trace(false).
...
'$$_reconsult_term'(unit U,Vars) :- !, abolish('$$_retract_done', 2),
	'$$_bind_names'(Vars), writeqln(unit U), create_unit(U), U>>'$$_consult_loop'(true).
