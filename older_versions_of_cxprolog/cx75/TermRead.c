...
static Pt ReadTermStream(StreamPt srm, Hdl vars)
{
	input = srm ;
	noTexts = false ;
...
static Pt ReadTermStreamForAssert(StreamPt srm, Hdl vars)
{
	input = srm ;
	noTexts = true ;
	return DoReadTerm(vars) ;
...
{
	Pt t ;
	if( ( t = ReadTermStream(currIn, nil) ) != nil && Unify(X0, t) ) JumpNext()
	DoFail()
}
...
static void PSRead()
{
	Pt t ;
	StreamPt srm = XTestStream(X0, mRead) ;
	if( ( t = ReadTermStream(srm, nil) ) != nil && Unify(X1, t) ) JumpNext()
	DoFail()
}
...
{
	Pt t, names ;
	if( ( t = ReadTermStream(currIn, &names) ) != nil &&
...
static void PSReadV()
{
	Pt t, names ;
	StreamPt srm = XTestStream(X0, mRead) ;
	if( ( t = ReadTermStream(srm, &names) ) != nil &&
		Unify(X1, t) && Unify(X2, names) ) JumpNext()
	DoFail()
}
...
{
	Pt t ;
	if( ( t = ReadTermStreamForAssert(currIn, nil) ) != nil && Unify(X0, t) ) JumpNext()
	DoFail()
}
...
void InitTermRead()
{
	...
	InstallCBuiltinPred("readv", 2, PReadV) ;
	InstallCBuiltinPred("readv", 3, PSReadV) ;
