...
static void ThreadReset(register ThreadPt th)
{
	...
	ThreadReg(th,E) = maxAddr ;		/*  so that B wins in TopOfLocalStack() */
	ThreadReg(th,P) = &NotRunning ;	/* nil would put nils on the local stack */
	ThreadReg(th,CP) = &NotRunning ; /* it's ok because B is on top */
...
static ThreadPt ThreadNew(Pt startGoal, Pt restartGoal)
{
	...
	ThreadStacksBegin0(th) = PrimitiveAllocateAndClear(initialStacksCapacity) ;
	ThreadStacksEnd0(th) = ThreadStacksBegin0(th) + initialStacksCapacity ;
	ThreadStacksBegin(th) = ThreadStacksBegin0(th) ;
	ThreadStacksEnd(th) = ThreadStacksEnd0(th) ;
	ThreadTrailBegin(th) = PrimitiveAllocate(initialTrailCapacity) ;
	ThreadTrailEnd(th) = ThreadTrailBegin(th) + initialTrailCapacity ;
	ThreadReset(th) ;
