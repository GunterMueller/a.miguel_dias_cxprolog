...
/* The following builtin predicates should not be redefined, not even inside
...
'$$_lux0' :-									\
	'$$_stress_testing',						\
	try (os_arg('-quiet','')					\
			->	flag(undef_warnings,_,off),		\
				flag(memory_warnings,_,off),	\
				flag(info_messages,_,off)),		\
	(os_arg('-boot',X)							\
			-> silent_consult(X)				\
			; '$$_default_boot'),				\
	dynamic_iu('$$_top_call'/1), /* dynamic builtin */	\
	'$$_enter_user_mode',						\
...
'$$_stress_testing' :-							\
    ...
    flag(test_garbage_collection,_,off),		\
 	try (flag(test_relocation,on)				\
	   -> writeln('*** STRESS-TESTING RELLOCATION ***')),			\
	try (flag(test_garbage_collection,on)							\
...
zpl :-											\
	...
	fs_exists(pl)								\
		...
		; fs_cd([_|X],[pl|X]).					\
												\
zrelo :-flag(test_relocation,_,on).				\
zcons :-flag(old_consult,_,on).					\
zread :- read(X), write(X), nl, display(X), nl.	\
zjava :-										\
...
'$$_listing_atom'(A) :-							\
	...
	forall(current_predicate(A,H), '$$_listing_locals'(H)), nl.		\
												\
'$$_listing'(H) :-								\
...
static CharPt defaultBuiltinsStr = "			\
												\
'$cxprolog_initialise':-						\
	try (flag(info_messages, on) -> version),	\
	'$env_context' := [main].					\
												\
