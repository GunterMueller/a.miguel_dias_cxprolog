...
{
	int oldCapacity = indexTableCapacity * WordsOf(IndexElem) ;
	MemoryGrowWarning("index table", oldCapacity, oldCapacity * 2, nil) ;
	Release(indexTable) ;
	indexTableCapacity *= 2 ;
	indexTable = TempAllocate(indexTableCapacity * WordsOf(IndexElem)) ;
	IndexTableInit() ;
}
