...
#define PredIsLocal(pr)			( PredIsUserDefined(pr) && !PredIsImported(pr) )
...
static Bool GetPredIsExported(PredicatePt pr, AtomPt atom, Pt t)
{ 
	Bool res = PredIsUserDefined(pr) && PredIsVisible(pr) ;
	return t == nil ? res : res && Unify(t, TagAtom(atom)) ;
}
...
void WritePredicateProperties(StreamPt srm, PredicatePt pr)
{
	...
	if( GetPredIsExported(pr, nil, nil) )
	...
	if( GetNumberOfClauses(pr, nil, nil) ) {
		if( NumberOfClauses(pr)	== 1 ) StreamWrite(srm, " - 1 clause") ;
		else StreamWrite(srm, " - %d clauses", NumberOfClauses(pr)) ;
	}
	...
	if( A(1) == tNilAtom ) {			/* init */
		...
		if( IsVar(X0) )
			...
			if( IsVar(a0) && IsVar(a1) )
				...
				PredicatePt pr = FindPredicateInCurrUnit(XTestSlash(X0)) ;
				Discard() ;
...
static void PNDCurrentPredicate2()
{
	...
	if( A(2) == tNilAtom ) {			/* init */
		...
		if( IsVar(X0) && IsVar(X1) )	/* var-var init */
			...
			PredicatePt pr = FindPredicateInCurrUnit(XTestFunctor(X1)) ;
			Discard() ;
			MustBe( pr != nil && !PredHiddenInGeneration(pr)
