...
{											/* pre: valid signature */
	switch( *sig ) {
		...
		case 'L': {
			...
			EnsureInstanceOf(val.l, expectedClass) ;
			if( convMore && JIsSame(expectedClass, stringClass) )
...
static Pt ConvertPolyObj(jobject obj)	/* destroys arg */
{
	...
	if( JIsNil(obj) )
		res = tNullAtom ;
	elif( JIsInstanceOf(obj, stringClass) )
...
static Pt SerializedTermToPrologTerm(jarray arr)	/* destroys arg */
{
	if( JIsNil(arr) ) goto error ;
