...
Bool InterLineGetSingleChar(WChar *c)
{
	if( !InteractiveSession() )
		return false ;
	InterLinePrepareForNextLine() ;  /* Disable prompt in InterLineGet */
...
void InterLineInit()
{
	Bool isConsole = (OSIsATty(stdin) && OSIsATty(stdout)) || stdStreamsMode == 2 ;
	InterLinePromptInit() ;
	InterLineInitialize() ;
	interactiveSession = !supressPrompts && isConsole ;
}
