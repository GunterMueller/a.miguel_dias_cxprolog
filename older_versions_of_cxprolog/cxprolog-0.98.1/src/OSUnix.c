...
static Fun timerAction;

static void dflTimerAction(void)
{
	Mesg("TIMER TICK");
}

static void onAlarm(int signal)
{
	Unused(signal);
	if( timerPeriod > 0 ) {
		alarm(timerPeriod);
...
void OSStartTimer(int period, Fun action)
{
	timerPeriod = period;
	timerAction = action == nil ? dflTimerAction : action;
