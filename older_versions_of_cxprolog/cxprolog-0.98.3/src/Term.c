...
static Bool Exists(Pt t)
{
	Hdl j ;
	for( j = ScribblingBegin(); j < scribblingPt ; j++ )
		if( Eq(XPt(*j), XPt(t)) )
			return true;
	return false;
}

static void Add(Pt t)
{
	if( !Exists(t) )
		ScribblingPush(t);
}

static void AddTerm(Pt t)
{
	...
	if( IsVar(t) ) {
		...
		for( i = 0 ; i < FunctorArity(f) ; i++ )
			Add(cPt(&XStructArg(t, i))) ;
		for( i = 0 ; i < FunctorArity(f) ; i++ )
			AddTerm(XStructArg(t, i)) ;
	}
	elif( IsList(t) ) {
		if( Exists(t) ) /* cycle */
			return;
...
{
	return XPt(*cHdl(p1)) - XPt(*cHdl(p2));
}

static void QSort(void)
{
	qsort(ScribblingBegin(), ScribblingUsed(), sizeof(Pt), cmp);
}

static void TermInternals(Pt t)
{
	Pt start ;
	Hdl3 h ;
	AddTerm(t);
	start = *ScribblingBegin();
	QSort();
	Write("-------------------------------------------\n");	
	for( h = cHdl3(ScribblingBegin()); Lt(h, scribblingPt) ; h++ ) {
		Pt t = *cHdl(h);
redo:
		if( t == start )
			Write("-> %.16p: ", XPt(t));
		...
		if( IsVar(*t) )
			...
			Write("%s ----------\n", FunctorNameArity(XStructFunctor(t)));
		elif( IsList(t) ) {
			Write("%s ----------\n", "./2");
			t = cPt(&XListHead(t));
			goto redo;
