...
typedef struct WXObj {
	...
	VoidPt value  ;
	bool deleted ;
	struct WXObj *nextHash ;
} WXObj, *WXObjPt ;

#define cWXObjPt(x)			((WXObjPt)(x))

#define WXObjValue(w)		(cWXObjPt(w)->value)
#define WXObjDeleted(j)		(cWXObjPt(j)->deleted)
#define WXObjNextHash(j)	(cWXObjPt(j)->nextHash)

static Pt tNullWXObj ;
static WXObjPt nullWXObj ;

#define wxObjTableCapacity   256

static WXObjPt *WXObjTable ;

static void WXObjInit()
{
	register int i ;
	WXObjTable = (WXObjPt *)PermAllocate(wxObjTableCapacity) ;
	dotimes(i, wxObjTableCapacity)
		WXObjTable[i] = nil ;
}

static WXObjPt FindWXObj(VoidPt obj, register WXObjPt *w)
{
	while( *w != nil ) {
		if( WXObjDeleted(*w) ) {
			ExtraDelete(wxobjType, *w) ;
			*w = WXObjNextHash(*w) ;
		}
		elif( WXObjValue(*w) == obj )
			return *w ;
		else
			w = &WXObjNextHash(*w) ;
	}
...
Pt MakeWXObj(VoidPt obj) /* pre: obj must be an alive wx object */
{
    register WXObjPt w ;
    UChar slot = cWord(obj) >> 3 ;
 	if( obj == nil ) return tNullAtom ;
	if( (w = FindWXObj(obj, &WXObjTable[slot])) != nil )
		return TagExtra(w) ;
	w = (WXObjPt)ExtraNew(wxobjType) ;	/* Create new */
	WXObjValue(w) = obj ;
	WXObjNextHash(w) = WXObjTable[slot] ;
	WXObjDeleted(w) = false ;
	WXObjTable[slot] = w ;
	return TagExtra(w) ;
}

void WXDeleted(VoidPt obj) /* Available to the Gui thread */
{					/* pre: obj has really been deleted (will not reappear) */
    register WXObjPt w ;
    UChar slot = cWord(obj) >> 3 ;
 	if( obj == nil ) return ;
    doseq(w, WXObjTable[slot], WXObjNextHash(w))
        if( WXObjValue(w) == obj ) {	/* Already in the table */
 		  /* Do only this because this may be running in the Gui thread */
			WXObjDeleted(w) = true ;
			return ;
		}
}
...
VoidPt XTestWXObj(register Pt t) /* t may represent an alive or dead object */
{
	...
	w = (WXObjPt)XTestExtra(wxobjType, t) ;
	obj = WXObjValue(w) ;
	if( WXObjDeleted(w) ) {
		UChar slot = cWord(obj) >> 3 ;
		if( FindWXObj(obj, &WXObjTable[slot]) != nil )
			InternalError("XTestWXObj (1)") ;
		w = (WXObjPt)XTestExtra(wxobjType, t) ; /* Must generate error */
		InternalError("XTestWXObj (2)") ;
	}
	return obj ;
}
...
{
	qBegin = cCharPt(PermAllocate(eventQueueCapacity)) ;
...
public:
	...
	void OnExit() {
		/* When the CxProlog thread finishes, the wxWidgets thread also exits */
...
public:
	...
	/* In Windows, at least one window must exist to activate the event handler */
...
/* The implementation of all wxWidgets based builtin predicates uses the
...
void WXWidgetsInit()
{
	wxobjType = ExtraTypeNew("WXOBJ", WordsOf(WXObj), nil, WXObjNamingFun) ;
	WXObjInit() ;
	CreateNullStuff() ;
	EventQueueNew() ;
}
...
void WXSetEventNotifier(Fun f) { /* nothing */ }
Pt WXGetEvent() { Error("Gui events are inactive") ; return tNilAtom ; }
int WXHowManyEvents() { return 0 ; }
void WXDiscardAllEvents() { /* nothing */ }
