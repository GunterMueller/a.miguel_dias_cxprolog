...
{
	VoidPt mem = PrimitiveAllocate(nWords) ;
	EmptyRangeN(mem, nWords) ;
	*cHdl(last) = cPt(mem) + nWords ;
	return mem ;
}
...
Pt XTestIntOrVar(register Pt t)
{
	...
	return TypeError2("INT or VAR", t) ;
}
...
PInt XTestCharOrCode(Pt t)
{
	...
	if( IsAtom(t) ) {
		...
		return XCode(t) ;
	return ITypeError2("CHAR or CODE", t) ;
}
...
FunctorPt XTestSlash(register Pt t)
{
	...
	return TypeError2("PREDICATE INDICATOR", t) ;
}
...
void XTestSlashArgs(register Pt t, Hdl a0, Hdl a1)
{
	VarValue(t) ;
    if( !IsThisStruct(t, slashFunctor) )
		TypeError2("PREDICATE INDICATOR", t) ;
