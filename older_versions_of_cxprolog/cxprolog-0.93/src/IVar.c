...
void IVarCondSet(AtomPt atom, Pt value, Bool cons)
{
	register IVarPt iv = LookupIVar(atom) ;
...
Pt IVarGet(AtomPt atom, Bool err)
{
	register IVarPt iv = AtomToIVar(atom) ;
	if( IVarIsDefined(iv) )
		return iv->value ;
...
{
	AtomPt atom = XTestAtom(X0) ;
	Pt t = IVarGet(atom, false) ;
	Ensure( t != tUndefinedIVarAtom ) ;
...
void IVarsInit()
{
	...
	InstallBasicGCHandler("IVAR", IVarsBasicGCMark) ;
