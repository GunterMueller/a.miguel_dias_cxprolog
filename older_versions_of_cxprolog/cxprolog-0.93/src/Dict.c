...
static Size DictsBasicGCMarkAux(VoidPt x)
{
	DictPt d = cDictPt(x) ;
	register DEntryPt h ;
	for( h = DictFirst(d) ; h < DictLast(d) ; h++ ) {
...
static void DictsBasicGCMark()
{
	ExtraForEach(dictType, DictsBasicGCMarkAux) ;
}
...
void DictsInit()
{
	dictType = ExtraTypeNew("DICT", WordsOf(Dict), nil, nil) ;
	InstallBasicGCHandler("DICT", DictsBasicGCMark) ;
