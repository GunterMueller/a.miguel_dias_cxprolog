...
{
	BindPredicate(pr, PredIsVisible(pr) ? VisibleEmptyPred : EmptyPred,
											cPt(PredFunctor(pr))) ;
}
...
static void RenamePredicate(PredicatePt pr, CharPt newName)
{
	...
	if( StrEqual(PredName(pr), newName) )
		DatabaseError("Idempotent renaming of built-in predicate '%s'",
														PredNameArity(pr)) ;
	if( PredIsMeta(pr) )
		DatabaseError("The core builtin predicate '%' cannot be changed",
														PredNameArity(pr)) ;		
	f = PredFunctor(pr) ;
	builtin = PredIsBuiltin(pr) ;
...
void SetDynamic(PredicatePt pr, Bool logical)
{
	...
	if( PredIsPermanent(pr) ) /* Because dynamic-builtin allowed at boot time */
...
void SetMultifile(PredicatePt pr)
{
	...
	if( PredIsPermanent(pr) ) /* Because multifile-builtin allowed at boot time */
...
static void PAbolishBuiltin()
{
	PredicatePt pr ;
	if( !Booting() )
		DatabaseError("Abolishing a builtin only works at booting time") ;
	if( (pr = FindPredicate(XTestFunctor2(X0,X1))) == nil )
		DatabaseError("Predicate '%s' not defined", PredNameArity(pr)) ;
	if( !PredIsBuiltin(pr) )
		DatabaseError("Predicate '%s' is not a built-in", PredNameArity(pr)) ;		
	RenamePredicate(pr, GStrFormat("$$_original_%s", PredName(pr))) ;
	JumpNext() ;
}
...
static void PRenameBuiltin()
{
	PredicatePt pr ;
	if( !Booting() )
		DatabaseError("Renaming a builtin only works at booting time") ;
	if( (pr = FindPredicate(XTestFunctor2(X0,X1))) == nil )
		DatabaseError("Predicate '%s' not defined", PredNameArity(pr)) ;
	if( !PredIsBuiltin(pr) )
		DatabaseError("Predicate '%s' is not a built-in", PredNameArity(pr)) ;		
	RenamePredicate(pr, XAtomName(X2)) ;
	JumpNext() ;
}
...
void PredicatesInit()
{
	...
	undefPred = InstallCBuiltinPred("undef", 0, PFail) ;
