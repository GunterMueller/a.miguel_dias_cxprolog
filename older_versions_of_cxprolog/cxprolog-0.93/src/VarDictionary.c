...
void VarDictInit()
{
	...
	varDict = TempAllocate(varDictCapacity * WordsOf(VarDescriptor)) ;
}

static void VarDictExpand()
{
	int varDictWords = varDictCapacity * WordsOf(VarDescriptor) ;
	MemoryGrowWarning("var table", varDictWords, varDictWords * 2, nil) ;
	varDictCapacity *= 2 ;
	varDict = Reallocate(varDict, varDictWords * 2) ;
}
...
{
	if( varDictSize >= varDictCapacity )
		VarDictExpand() ;
