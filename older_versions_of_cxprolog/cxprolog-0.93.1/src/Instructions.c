...
static PredicatePt HandleUndefPredCall(FunctorPt f, Pt ctx)
{
	switch( unknown_flag ) {
		case 0: /* error */
			Error("Predicate '%s' is not visible in context %s",
...
{
	Hdl c ;
	c = ctxCodeSegm = PermAllocate(LastPos + 1) ; /* must have alloc addr */

	*c++ = cPt(WordsOf(Environment) + 1) ; /* 1 cell local-var env for CONTEXTS1 */
	*c++ = cPt(UndefPredEnd) ;
	...
	*c++ = cPt(WordsOf(Environment) + 2) ; /* 2 cell local-var env */
	*c++ = cPt(CtxEmptyEnd) ;
	
	*c++ = cPt(WordsOf(Environment) + 1) ; /* 1 cell local-var env */
	*c++ = cPt(CtxDownEnd) ;
	
	*c++ = cPt(WordsOf(Environment)) ; /* 0 cell local-var env */
	*c++ = cPt(HCtxPushEnd) ;
