...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
A deterministic built-in predicate written in C succeeds by activating the macro
call JumpNext() and fails activating the macro DoFail().

A nondeterministic built-in predicate written in C succeeds activating the
macro call JumpNext() and stops generating more alternatives and fails,
activating the macro call Jump(DiscardAndFail). A nondeterministic built-in
