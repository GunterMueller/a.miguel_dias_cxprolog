...
static jmethodID cxPrologClass_DiscardEvents ;
...
#define JObjNextHash(j)		(cJObjPt(j)->nextHash)
...
#define jObjTableCapacity   256

static JObjPt *JObjTable ;

static void JObjInit()
{
	register int i ;
	JObjTable = PermAllocate(jObjTableCapacity) ;
	dotimes(i, jObjTableCapacity)
		JObjTable[i] = nil ;
}
...
static JObjPt JObjNew(jobject obj)
{
	...
	register int hash ;
    UChar slot ;
 	if( obj == nil ) InternalError("JObjNew") ;
	hash = GetHashCode(obj) ;
	slot = hash ;
    doseq(j, JObjTable[slot], j->nextHash)
        if( JObjHash(j) == hash ) {	/* Already in the table */
			if( JObjValue(j) != obj )
				DeleteLocalRef(obj) ;
            return j ;
		}
	j = ExtraNew(jobjType) ;	/* Create new */
	JObjValue(j) = obj ;
	JObjHash(j) = hash ;
	JObjNextHash(j) = JObjTable[slot] ;
	JObjTable[slot] = j ;
	return j ;
}

static void JObjDelete(VoidPt x) /* Only called by the garbage collector */
{
	JObjPt jo = cJObjPt(x) ;
    UChar slot = JObjHash(jo) ;
	register JObjPt *j ;
	for( j = &JObjTable[slot] ; *j != nil ; j = &JObjNextHash(*j) ) 
		if( *j == jo ) {
			DeleteLocalRef(JObjValue(jo)) ;
			*j = JObjNextHash(jo) ;
			ExtraDelete(jobjType, jo) ;
			return ;
		}
	InternalError("JObjDelete") ;
}
...
static jclass FindNormalClassChecked(CharPt name)
{
	jclass cls ;
	if( (cls = JFindClass(name)) != nil ) {
		MakeJObj(cls) ;	/* Put reference in the hash table */
		return cls ;
	}
	return JError("Cannot access java class '%s'", name) ;
}
...
static Pt ConvertPolyObj(jobject obj)
{
	...
	else return MakeJObj(obj) ;

	DeleteLocalRef(obj) ;
	return res ;
}

static Pt ConvertTreeObj(jobject obj)
{
...
error:
	DeleteLocalRef(obj) ;
	return Error("Invalid Java linearized tree") ;
}


/* Java start & stop */

static void JNICALL NotifyProlog(JNIEnv *env, jobject self)
{
...
static void JavaStart(void)
{
	...
	MakeJObj(systemClass) ;	/* Only now can be put in the hash table */
...
/* Do the call and handle result */
	...
	switch( sigLast ) {
		...
		case '@': { /* tree */
			jobject obj = JCallMethodA(Object, targetIsClass, target, id, args) ;
			CheckException() ;
			MustBe( Unify(X3, ConvertTreeObj(obj)) ) ;
		}
		default:
...
void JavaInit()
{
	if( !OSExists(PREFIX "/lib/cxprolog/CxProlog.class")
	  && OSExists(PREFIX "/lib/cxprolog/CxProlog.java") )
	  	OSRun("javac " PREFIX "/lib/cxprolog/CxProlog.java") ;
	if( !OSExists(PREFIX "/lib/cxprolog/CxProlog.class") )
		FatalError("Cannot access '%s'", PREFIX "/lib/cxprolog/CxProlog.class") ;

	jobjType = ExtraTypeNew("JOBJ", WordsOf(JObj), JObjDelete, JObjNamingFun) ;
	JObjInit() ;
	PrepareJErrors("", "") ;
	CreateNullStuff() ;
...
void JavaSetEventNotifier(Fun f) { /* nothing */ }
Pt JavaGetEvent() { Error("Gui events are inactive") ; return tNilAtom ; }
int JavaHowManyEvents() { return 0 ; }
void JavaDiscardAllEvents() { /* nothing */ }
