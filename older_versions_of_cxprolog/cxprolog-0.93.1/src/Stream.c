...
static StreamPt FindEdinburghStream(CharPt name, StreamMode mode)
{
	...
	if( (res = ExtraFindFirst(streamType, FindEdinburghStreamAux, name)) != nil )
		...
		case binaryFileStream: {
			if( co->force == tFalseAtom
				...
				&& fflush(StreamChannel(srm)) != 0 )
					FileError("Could not close stream '%s'", XExtraAsStr(TagExtra(srm))) ;
	...
	switch( StreamKind(srm) ) {
		case textFileStream: {
			...
			if( MyPutChar(c, StreamChannel(srm)) == MyEOF )
				FileError("Could not write on stream '%s'", XExtraAsStr(TagExtra(srm))) ;
			break ;
		}
		case binaryFileStream: {
	...
	switch( StreamKind(srm) ) {
		...
		case binaryFileStream: {
			if( fflush(StreamChannel(srm)) != 0 )
				FileError("Could not flush stream '%s'", XExtraAsStr(TagExtra(srm))) ;
			break ;
		}
		case textBufferStream:
		case binaryBufferStream:
...
static Size PStreamsAux(VoidPt x)
{
	...
	Write("%16.16s %s> %s, %s, %s,",
			...
			StreamKindStr(srm),
			StreamModeStr(srm)) ;
	Write("\n%19s type(%s), reposition(%s), ", "",
					StreamIsBinary(srm) ? "binary" : "text",
					StreamAllowReposition(srm) ? "true" : "false") ;
	if( StreamAlias(srm) != nil )
		Write("alias(%s), ", XAtomName(StreamAlias(srm))) ;
	elif( at != nil )
		Write("alias(%s), ", AtomName(at)) ;
	Write("\n%19s eof_action(%s), encoding(%s), ", "",
					StreamEofActionStr(srm), StreamLocale(srm)) ;

	if( StreamAtEnd(srm) )
		Write("[at EOF], ") ;
	Write("\n%19s path = %s", "", AtomName(StreamPath(srm))) ;
	Write("\n") ;	
	return 1 ;
}
...
void StreamsInit2()
{
	...
	InstallBasicGCHandler("STREAM",StreamsBasicGCMark) ;
