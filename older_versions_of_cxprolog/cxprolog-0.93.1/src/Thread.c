...
#define initialStacksCapacity	16 K
#define initialTrailCapacity	256
...
static void ThreadReset(register ThreadPt th)
{
	...
	ThreadReg(th,B) = cChoicePointPt(ThreadStacksEnd(th)-1) ; /* Near top of stacks */
	ThreadReg(th,R) = cChoicePointPt(ThreadStacksEnd(th)-1) ; /* Exception chain */
	ThreadReg(th,E) = cEnvironmentPt(ThreadStacksEnd(th)) ; /* Higher for TopOfLocalStack() */
...
static ThreadPt ThreadNew(Pt startGoal, Pt restartGoal)
{
	...
	ThreadStacksBegin0(th) =
		PrimitiveAllocateEmpty(initialStacksCapacity, &ThreadStacksEnd0(th)) ;
	ThreadStacksEnd0(th)[-1] = endOfChainMark ;	/* end-of-local-chains area */
	ThreadStacksBegin(th) = ThreadStacksBegin0(th) ;
	ThreadStacksEnd(th) = ThreadStacksEnd0(th) ;
	ThreadTrailBegin(th) =
		PrimitiveAllocateEmpty(initialTrailCapacity, &ThreadTrailEnd(th)) ;
	ThreadReset(th) ;
...
static void PMaxMem()
{
	ZEnsureFreeSpaceOnStacks(128 K, nil) ; /* stacks grow now */
	JumpNext() ;
}
...
void ThreadsInit()
{
	threadType = ExtraTypeNew("THREAD", WordsOf(Thread), nil, nil) ;

	NotRunningAddr = PermAllocate(1) ;
