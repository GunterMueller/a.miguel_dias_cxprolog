	...
	if( PredHasClauses(pr) ) {
		if( PredIsIndexable(pr) )
			...
			BindPredicateFull(pr,  /* (int) cast below avoids warning */
				PredIsLogical(pr)
					? (PredIsVisible(pr) ? VisibleDynamicEnter : DynamicEnter)
					: DynamicIUEnter,
				cPt(PredClauses(pr)), cPt((int)PredArity(pr)), nil, nil) ;
	}
	else
	...
	if( PredHasDynamicChain(pr) ) {
		...
		if( PredIsLogical(pr) ) {
			ClauseInst(cl) =
					PredIsIndexable(pr)
						? DynamicElseIndexed
						: (PredIsVisible(pr) ? VisibleDynamicElse : DynamicElse) ;
...
static void DeleteClauses(PredicatePt pr)
{
	...
	doseq(cl, PredClauses(pr), next) {
		next = ClauseNext(cl) ;
		DeleteClause(cl) ; /* updates PredClauses(pr) */
	}
