...
static int nForeinInterfaces = 0 ;
static int defaultForeignInterface = fNone ;

static CharPt ForeignInterfaceAsString(ForeignInterface f)
{
	switch( f ) {
		case fJava: return "Java" ;
		case fWxWidgets: return "wxWidgets" ;
		default: return "fNone" ;
	}
}
...
void RegisterForeignInterface(ForeignInterface f)
{
	...
	if( nForeinInterfaces < maxForeignInterfaces ) {
		foreinInterfaces[nForeinInterfaces++] = f ;
		if( f > defaultForeignInterface )
			defaultForeignInterface = f ;
	}
	else
...
void SetDefaultForeignInterface(ForeignInterface f)
{
	...
	dotimes(i, nForeinInterfaces)
		if( foreinInterfaces[i] == f ) {
			defaultForeignInterface = f ;
			return ;
		}
	Error("Foreign interface '%s' is not available", ForeignInterfaceAsString(f)) ;
}

static void ZForeignLoad(CharPt a, CharPt prefix)
{
	Str2K buff ;
...
static void ZForeignLoadDefaultForeignInterface()
{
	...
	if( defaultForeignInterface == fJava )
		prefix = " j" ;
	elif( defaultForeignInterface == fWxWidgets )
		prefix = "wx" ;
	else return ;
...
static void PForeigns()
{
	...
	Write("FOREIGNS:\n") ;
	dotimes(i, nForeinInterfaces) {
		ForeignInterface f = foreinInterfaces[i] ;
		if( f == fNone ) continue ;
		Write("    %s%s\n",
			ForeignInterfaceAsString(f),
			f == defaultForeignInterface ? " (default)" : "") ;
	}
	JumpNext() ;
}
