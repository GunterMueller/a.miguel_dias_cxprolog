...
{
	wxMessageBox(wxT("This is a wxWindows CxProlog Test"),
...
static void PWCreateGUI() {		/* Perform the GUI service */
	wxString str(s0, wxConvUTF8) ;
	MyFrame *w = new MyFrame(str, wxPoint(50,50), wxSize(450,340)) ;
	w->Show(TRUE) ;
	o0 = w ;
}
void PWCreateEND() {		/* Build the results. */
	BindVarWithExtra(X1, LookupWxObj(o0)) ; /* Also handles ivars. */
}
...
static void PWAlertGUI() {
	wxString str(s0, wxConvUTF8) ;
	wxMessageDialog *d = new wxMessageDialog((wxFrame*)NULL, str, wxT("Message Box"),
...
{
	InstallWxBuiltinPred("wxw", 2, PWCreate) ;
	InstallWxBuiltinPred("wxc", 1, PWClose) ;
	InstallWxBuiltinPred("wxt", 2, PWWrite) ;
	InstallWxBuiltinPred("wxa", 1, PWAlert) ;
