...
void AliasSet(AtomPt a, ExtraPt extra)
{
	AliasPt as = AtomToAlias(a) ;
	register ExtraHdl h, stop ;
	if( ExtraIsSpecial(a) )
		StreamsReAliasing(a, cStreamPt(extra), true) ;
/* First time */
	if( as == nil ) {
		*AliasCreate(a) = extra ;
		return ;
...
void AliasUnset(AtomPt a, ExtraPt extra)
{
	AliasPt as = AtomToAlias(a) ;
	register ExtraHdl h, stop ;
	if( ExtraIsSpecial(a) )
		StreamsReAliasing(a, cStreamPt(extra), false) ;
	if( as != nil )
		for( h = AliasFirst(as), stop = AliasLast(as) ; h <= stop ; h++ )
			if( *h == extra ) {
				*h = nil ;
...
static void PAliases()
{
	...
	for( a = aliasList ; a != nil ; a = AliasNextAlias(AtomToAlias(a)) ) {
		...
		if( as != nil ) {
			for( h = AliasFirst(as), stop = AliasLast(as) ; h <= stop ; h++ )
				if( !CellIsFree(*h) )	
					Write(" %22.16s %c> %1.50s\n",
							AtomName(a),
							false && ExtraIsPermanent(*h) ? '=' : '-',
...
static void PAlias()
{
	AliasSet(XTestAtom(X0), XTestExtraStrict(X1)) ;
	JumpNext() ;
}

static void PUnalias()
{
	ExtraPt x1 = XTestExtraStrict(X1) ;
	if( 0 && ExtraIsPermanent(x1) )
		ImperativeError("You cannot unalias the special value '%s'", TermAsStr(X1)) ;
	AliasUnset(XTestAtom(X0), x1) ;
	JumpNext() ;
}
