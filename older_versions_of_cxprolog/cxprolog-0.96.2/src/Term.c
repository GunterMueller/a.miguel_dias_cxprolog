...
void BindVar(register Pt t, Pt value)
{
	VarValue(t) ;
	if( IsVar(t) ) {
		if( !UnifyWithAtomic(t, value) )
			InternalError("BindVar") ;
	}
	elif( IsAtom(t) ) {
		if( !IsExtraStrict(value) )
			Error("Extra expected") ;
		AliasSet(XAtom(t), XExtra(value)) ;
	}
	elif( IsThisStruct(t, plusFunctor) && IsAtom(Drf(XStructArg(t,0))) ) {
		AliasSet(XTestAtom(XStructArg(t,0)), XExtra(value)) ;
	}
	elif( IsThisStruct(t, minusFunctor) && IsAtom(Drf(XStructArg(t,0))) ) {
		IVarSet(XTestAtom(XStructArg(t,0)), value, false) ;
	}
	else
					...
					Bool convUnitParams, Bool cyclesAllowed)
{
	...
	for(;;) {
		Size size ;
		Pt res = CompoundTermCopy(ZT, H, H + FreeHSpace(), env, permanentExtras, convUnitParams, cyclesAllowed, &size) ;
		if( res == nil )
			ZStacksExpansion(0, "ZPushCompoundTerm") ;
		else {
			GrowH(size) ;
			return res ;
		}
...
/* AllocateCompoundTerm MAY NOT cause the stacks to grow */
