...
AtomPt PathNameListToAtom(Pt list)
{
	...
	while( len-- ) {
		GStrAddChar(SEP) ;
		GStrAddStrConv(XTestAtomName(h[len]), noSEP, SEP) ;
	}
...
CharPt ProcessFileName(CharPt s)
{
	...
	if( (cc = CharDecode(s)) == '~' ) {
		...
		BigStrBackTo(save) ;
		if( strlen(save) == 0 )
			BigStrAddStrConv(OSGetEnv("HOME", true), noSEP, SEP) ;
		else
			BigStrAddStrConv(OSGetUserHome(save, true), noSEP, SEP) ;
	}
