...
public class JavaGeneral {
	...
	public static void setLookAndFeel() {
		...
		try {
			...
			System.out.println("Exception setting native look and feel.");
		}
	}
    ...
    public static String fileChooserSimpleMsg(String msg) {
	    ...
	    // showDialog bloqueia antes de mostrar caixa de diálogo quando esta 
	    // função é chamada no windows a partir do prolog com java_call
	...
	public static int doChoice(String msg, boolean yesno, 
		...
		if (yesno && !ok && !cancel)
			...
			// Em caso de combinação inválida de opções, mostrar
			// diálogo com ok - não deverá acontecer se forem usados os 
		...
		switch (choice) {
			...
			case JOptionPane.CLOSED_OPTION:
				// que opção se o utilizador fechar a janela?
				if (cancel)
					return CHOICE_CANCEL;
				else
					return CHOICE_NO;
