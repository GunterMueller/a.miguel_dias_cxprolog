...
static void StreamsBasicGCMarkContents(VoidPt x)
{
	...
	if( StreamKind(srm) == textBufferStream
	 || StreamKind(srm) == binaryBufferStream )
...
/* open file */
	...
	if( so->type == tBinaryAtom ) strcat(m, "b") ;
...
CharPt StreamClose(StreamPt srm, CloseStreamOptionsPt co)
{
	...
	switch( StreamKind(srm) ) {
		...
		case binaryFileStream: {
			if( ExtraIsAlive(srm) ) {
				if( co->force == tFalseAtom
					&& (StreamMode(srm) == mWrite || StreamMode(srm) == mAppend)
					&& fflush(StreamChannel(srm)) != 0 )
						FileError("Could not close stream '%s'", ExtraAsStr(srm)) ;
...
static int ReturnEof(StreamPt srm, int eofCode)
{
	...
	switch( StreamKind(srm) ) {
		...
		case binaryFileStream: {
			if( (c = MyGetByte(StreamChannel(srm))) == EOF )
				return ReturnEof(srm, -1) ;  /* eof is hardwired on binary streams */
			return advance ? c : MyUngetByte(c, StreamChannel(srm)) ;
		}
		case textBufferStream: {
			MyUseLocale(StreamLocale(srm)) ;	
			c = BufferAccessChar(StreamChannel(srm), advance) ;
			if( c == eofMark ) return ReturnEof(srm, eofMark) ;
			if( !CharValidCode(c) ) c = '?' ;
			return c ;
		}
		case binaryBufferStream: {
			c = BufferAccessByte(StreamChannel(srm), advance) ;
			if( c == -1 ) return ReturnEof(srm, -1) ;
			return c ;
		}
		case nullStream: {
	...
	switch( StreamKind(srm) ) {
		case textFileStream: {
			...
			if( MyPutChar(c, StreamChannel(srm)) == MyEOF )
				FileError("Could not write on stream '%s'", ExtraAsStr(srm)) ;
			break ;
		}
		case binaryFileStream: {
...
void StreamFlush(StreamPt srm)
{
	switch( StreamKind(srm) ) {
		case textFileStream:
		case binaryFileStream: {
			if( fflush(StreamChannel(srm)) != 0 )
				FileError("Could not flush stream '%s'", ExtraAsStr(srm)) ;
			break ;
		}
		case textBufferStream:
		case binaryBufferStream:
