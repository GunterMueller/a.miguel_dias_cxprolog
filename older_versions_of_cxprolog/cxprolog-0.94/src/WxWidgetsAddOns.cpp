 ...
 *   Copyright (C) 1990-2006 Sergio Lopes, Henrique Oliveira, A.Miguel Dias, CITI, DI/FCT/UNL 
 ...
 * LOCAL CHANGELOG:
 *
 * 02/Set/2006 Took all the wxWidgets code contributed by Sergio Lopes
 *             and Henrique Oliveira and created this big source file.
 * 03/Set/2006 Fixed some problems in the low level event handling of
 *             text windows. These problems occurred on Linux only,
 *             not on Windows.
 * 04/Set/2006 Discovered that there are bugs remaining in this code.
 *             The code in this file has still "alpha quality".
 */
...
public:
	WxPixel();
	WxPixel(void *frame, int x, int y, int r, int g, int b);
	void draw();
};
...
public:
	WxLine();
	WxLine(void *frame, int x0, int y0, int x1, int y1, int r, int g, int b);
	void draw();
};
...
public:
	WxCircle();
	WxCircle(void *frame, int x, int y, int radius, int r, int g, int b, bool filled);
	void draw();
};
...
public:
	WxRectangle();
	WxRectangle(void *frame, int x, int y, int width, int height, int r, int g, int b, bool filled);
	void draw();
};
...
public:
	WxText();
	WxText(void *frame, const char *text, int x, int y);
	void draw();
};
...
public:
	WxButton();
	WxButton(void *frame, int x, int y, const char *text);
	const char *label();
};
...
public:
	WxList();
	WxList(void *frame, void *items, int x, int y);
...
WxPixel::WxPixel(void *frame, int x, int y, int r, int g, int b) {
	m_frame = frame;
	m_x = x;
	m_y = y;
	m_r = r;
...
WxLine::WxLine(void *frame, int x0, int y0, int x1, int y1, int r, int g, int b) {
	m_frame = frame;
	m_x0 = x0;
	m_y0 = y0;
	m_x1 = x1;
...
WxCircle::WxCircle(void *frame, int x, int y, int radius, int r, int g, int b, bool filled) {
	m_frame = frame;
	m_x = x;
	m_y = y;
	m_radius = radius;
...
WxRectangle::WxRectangle(void *frame, int x, int y, int width, int height, int r, int g, int b, bool filled) {
	m_frame = frame;
	m_x = x;
	m_y = y;
	m_width = width;
...
WxText::WxText(void *frame, const char *text, int x, int y) {
	m_frame = frame;
	m_text = text;
	m_x = x;
	m_y = y;
...
WxButton::WxButton(void *frame, int x, int y, const char *text) {
	m_frame = frame;
	m_x = x;
	m_y = y;
	m_text = text;
...
WxList::WxList(void *frame, void *items, int x, int y) {
	m_frame = frame;
	m_x = x;
	m_y = y;
	
	wxArrayString *temp = (wxArrayString *)items;
	...
	m_list = new wxListBox((WxGraphicalFrame *)frame, -1, wxPoint(x,y), 
		wxDefaultSize, count, choices, 0, wxDefaultValidator, "listBox");
}

void *WxList::getSelectedItem() {
	wxListBox *l = (wxListBox *)m_list;
	int i = l->GetSelection();
...
static void PGuiGfxCreateEND() {
	BindVarWithExtra(X5, MakeWxObj(gframe));
}
...
static void PGuiGfxPutPixelEND() {
	BindVarWithExtra(X6, MakeWxObj(obj));	
}
...
static void PGuiGfxLineEND() {
	BindVarWithExtra(Xc(8), MakeWxObj(obj));
}
...
static void PGuiGfxCircleEND() {
	BindVarWithExtra(X7, MakeWxObj(obj));
}
...
static void PGuiGfxDrawTextEND() {
	BindVarWithExtra(X4, MakeWxObj(obj));
}
...
static void PGuiGfxListGetItemGUI() {
	wxString *string = (wxString *)(((WxList *)obj)->getSelectedItem());
	t = MakeTempAtom(cCharPt(string->c_str()));

}
static void PGuiGfxListGetItemEND() {
	BindVarWithExtra(X1, t);
}
...
static void PGuiTextCreateEND() {
	BindVarWithExtra(X6, MakeWxObj(tframe));
}
...
void WxAddOnsInit() {
	...
	/* GENERAL PURPOSE PREDICATES */
	InstallWxCBuiltinPred("wxgui_file_chooser_simple_msg", 2, PGuiFileChooserSimpleMsg) ;
	InstallWxCBuiltinPred("wxgui_file_chooser_multiple_simple", 1, PGuiFileChooserMultipleSimple) ;
	InstallWxCBuiltinPred("wxgui_file_chooser_multiple_msg", 2, PGuiFileChooserMultipleMsg) ;
	InstallWxCBuiltinPred("wxgui_directory_chooser_msg", 2, PGuiDirectoryChooser) ;
	InstallWxCBuiltinPred("wxgui_alert", 1, PGuiAlert) ;
	InstallWxCBuiltinPred("wxgui_choice", 2, PGuiChoice) ;
	InstallWxCBuiltinPred("wxgui_choice_list", 3, PGuiChoiceList) ;
	InstallWxCBuiltinPred("wxgui_choice_yes_no_cancel", 2, PGuiChoiceYesNoCancel) ;
	InstallWxCBuiltinPred("wxgui_choice_ok_cancel", 2, PGuiChoiceOkCancel) ;
	InstallWxCBuiltinPred("wxgui_get_text", 2, PGuiGetText) ;
	
	/* GRAPHICAL WINDOW PREDICATES */
	InstallWxCBuiltinPred("wxgui_gfx_create", 6, PGuiGfxCreate) ;
	InstallWxCBuiltinPred("wxgui_gfx_putpixel", 7, PGuiGfxPutPixel) ;
	InstallWxCBuiltinPred("wxgui_gfx_line", 9, PGuiGfxLine) ;
	InstallWxCBuiltinPred("wxgui_gfx_circle", 8, PGuiGfxCircle) ;
	InstallWxCBuiltinPred("wxgui_gfx_rectangle", 9, PGuiGfxRectangle) ;
	InstallWxCBuiltinPred("wxgui_gfx_circle_filled", 8, PGuiGfxCircleFilled) ;
	InstallWxCBuiltinPred("wxgui_gfx_rectangle_filled", 9, PGuiGfxRectangleFilled) ;
	InstallWxCBuiltinPred("wxgui_gfx_draw_text", 5, PGuiGfxDrawText) ;
	InstallWxCBuiltinPred("wxgui_gfx_button", 5, PGuiGfxButton) ;
	InstallWxCBuiltinPred("wxgui_gfx_list", 5, PGuiGfxList) ;
	InstallWxCBuiltinPred("wxgui_gfx_list_getitem", 2, PGuiGfxListGetItem) ;
	InstallWxCBuiltinPred("wxgui_gfx_delete_object", 2, PGuiGfxDeleteObject) ;
	InstallWxCBuiltinPred("wxgui_gfx_clear", 1, PGuiGfxClear) ;
	InstallWxCBuiltinPred("wxgui_gfx_close", 1, PGuiGfxClose) ;
	
	/* TEXT WINDOW PREDICATES */
	InstallWxCBuiltinPred("wxgui_text_create", 7, PGuiTextCreate) ;
	InstallWxCBuiltinPred("wxgui_text_get_file_path", 2, PGuiTextGetFilePath) ;
	InstallWxCBuiltinPred("wxgui_text_open_file", 1, PGuiTextOpenFile) ;
	InstallWxCBuiltinPred("wxgui_text_save_file", 1, PGuiTextSaveFile) ;
	InstallWxCBuiltinPred("wxgui_text_save_file_as", 1, PGuiTextSaveFileAs) ;
	InstallWxCBuiltinPred("wxgui_text_get_text", 4, PGuiTextGetText) ;
	InstallWxCBuiltinPred("wxgui_text_append", 2, PGuiTextAppend) ;
	InstallWxCBuiltinPred("wxgui_text_get_selected_text", 2, PGuiTextGetSelectedText) ;
	InstallWxCBuiltinPred("wxgui_text_replace", 4, PGuiTextReplace) ;
	InstallWxCBuiltinPred("wxgui_text_set_selection", 3, PGuiTextSetSelection) ;
	InstallWxCBuiltinPred("wxgui_text_close", 1, PGuiTextClose) ;
}
