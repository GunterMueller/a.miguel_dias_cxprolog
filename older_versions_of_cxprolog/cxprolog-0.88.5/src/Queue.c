...
static void QueueDisplay(StreamPt stm, QueuePt q)
{
	Hdl h ;	
...
static Size QueuesAtomGCMarkAux(VoidPt x)
{
...
static void QueuesAtomGCMark()
{
	ForEachExtra(queueType, QueuesAtomGCMarkAux) ;
}
...
{
	QueueDisplay(currOut, XTestExtra(queueType,X0)) ;
	JumpNext()
}

static void PSQueueWrite()
{
	QueueDisplay(XTestStream(X0, mWrite), XTestExtra(queueType,X1)) ;
	JumpNext()
}

static void PNDCurrentQueue()
{
	PNDCurrentExtra(queueType, nil, 1) ;
	JumpNext()
}
...
static void PQueues()
{
	VersionShow() ;
	Write("Queues:\n") ;
	ForEachExtra(queueType, QueuesAux) ;
	JumpNext()
}
...
void QueuesInit()
{
	...
	InstallAtomGCHandler(QueuesAtomGCMark) ;
