...
{
	register VarDescriptorPt vd = FindVar(var) ;
	if( WasReferenced(vd) ) {
		if( vd->isTemp ) {
...
static void CompileGoalVar(Pt var, int i)
{
	...
	if( WasReferenced(vd) ) {
		...
		if( vd->isTemp ) {
			...
			Gen2(PutYVariable, OutPerm(vd->permIndex), OutTemp(i)) ;
			if( vd->tempIndex == -1 ) vd->tempIndex = i ;
		}
	}
}
...
{
	if( argN < vd->tempIndex && vd->tempIndex < TermArity(currGoal) )
		vd->tempIndex = FindR() ;
}
...
static void CompileHeadRecordVar(Pt var, int argN)
{
	...
	if( WasReferenced(vd) ) {
		if( vd->isTemp ) {
			...
			if( vd->hasGlobalValue ) Gen1(UnifyYValue, OutPerm(vd->permIndex)) ;
...
static void CompileHeadVar(Pt var, int i)
{
	...
	if( WasReferenced(vd) ) {
		...
		if( vd->isTemp ) ;
			...
			Gen2(GetYVariable, OutPerm(vd->permIndex), OutTemp(i)) ;
			if( vd->tempIndex == -1 ) vd->tempIndex = i ;
		}
	}
}
