...
#define GetChar0()		(strGet ? *strGet++ : StreamGetChar(input))
#define PeekChar0()		(strGet ? *strGet : StreamPeekChar(input))
...
afterSpaceL:	if( cc == '(' ) {
			...
			case _EF: {
				...
				if( !strGet ) NextCharEOF() ; /* Generate an eof error */
				GenToken(dotTk, nil) ;
				goto exitL ;
			}
			case __I: {
...
static Pt ZReadTermStream(StreamPt srm)
{
	...
	switch( setjmp(readJmp) ) {
		case 0: {
			...
			ZEnsureFreeSpaceOnStacks(expectedSize, "ZReadTermStream") ;
...
static Pt ZReadTokensStream(StreamPt srm)
{
	...
	switch( setjmp(readJmp) ) {
		case 0: {
			...
			ZEnsureFreeSpaceOnStacks(expectedSize, "ZReadTokensStream") ;
...
{
	Pt t = ZReadTermStream(currIn) ; /* stacks may grow */
...
static void PSReadTerm()
{
	...
	Pt t = ZReadTermStream(srm) ; /* stacks may grow */
	if( t == eofPt ) t = tEofAtom ;
...
{
	Pt t = ZReadTokensStream(currIn) ; /* stacks may grow */
	if( t == eofPt ) t = tEofAtom ;
	if( t != nil && Unify(X0, t) ) JumpNext()
	DoFail()
}
...
{
	StreamPt srm = XTestStream(X0, mRead) ;
	Pt t = ZReadTokensStream(srm) ; /* stacks may grow */
	if( t == eofPt ) t = tEofAtom ;
	if( t != nil && Unify(X1, t) ) JumpNext()
	DoFail()
}
