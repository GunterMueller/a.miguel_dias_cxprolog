...
static int CompareReal(Real r1, Real r2)
{
...
int Compare(register Pt t1, register Pt t2)
{
	...
	if( GetTag(t1) == GetTag(t2) ) {
		...
		if( IsAtomOrText(t1) )
			...
			return CompareInt(XInt(t1), XInt(t2)) ;
				
		elif( IsReal(t1) )
			 return CompareReal(XReal(t1), XReal(t2)) ;
		...
		if( IsAtomOrText(t1) && IsAtomOrText(t2) )
			return strcmp(XAtomOrTextName(t1), XAtomOrTextName(t2)) ;
		elif( IsInt(t1) && IsReal(t2) )
			return CompareReal(XInt(t1), XReal(t2)) ;
		elif( IsReal(t1) && IsInt(t2) )
			return CompareReal(XReal(t1), XInt(t2)) ;
...
static void RemoveDuplicates(Hdl array, int *len)	/* pre: *len > 1 */
{
	Hdl get, put, stop ;
...
static void CheckAllKey(register Hdl array, int len)
{
	register Hdl stop ;
