...
{
	static double lastTopInterr = 0.0 ;

	signal(SIGINT, SIG_IGN) ;
	if( CurrTime() - lastTopInterr < 1.0 )
		PrologEvent(4) ;
	if( inTopGoal_flag )
		traceActive_flag = trace_flag = true ;
...
void DebugCall(CharPt name, int arity)
{
	int i ;
