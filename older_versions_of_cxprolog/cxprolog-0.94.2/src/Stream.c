...
static CharPt StreamKindStr(StreamPt srm)
{
	switch( StreamKind(srm) ) {
		...
		case textFileStreamInteractive: return "file" ;
...
StreamPt FILEToStream(FILE *file, CharPt n, StreamMode m, StreamKind k, Bool i)
{
...
static void CheckDoubleEOF(StreamPt srm)
{
...
Bool StreamAtEnd(StreamPt srm)
{
	...
	switch( StreamKind(srm) ) {
		...
		case textFileStreamInteractive:
		case binaryFileStream: {
			return FileAtEnd(StreamChannel(srm)) ;
		}
		case textBufferStream: {
		case binaryBufferStream:
...
{
	register StreamPt srm = cStreamPt(x) ;
	if( IsAbsoluteFileName(StreamPath(srm)) )
	...
	Write("\n%10s", "") ;	
	Write("current(%s), at_eof(%s)",
		(srm == currIn || srm == currOut) ? "true" : "false",
		StreamAtEnd(srm) ? "true" : "false") ;
	return 1 ;
}
...
void StreamsInit2()
{
	...
	InstallGNDeterCBuiltinPred("stream_property", 2, 2, PNDStreamProperty) ;
...
void StreamsInit()
{
	...
	currIn = userIn = origIn =
		FILEToStream(stdin, "user_input", mRead, textFileStream, true) ;
	currOut = userOut = origOut =
		FILEToStream(stdout, "user_output", mWrite, textFileStream, false) ;
	userErr = origErr =
		FILEToStream(stderr, "user_error", mWrite, textFileStream, false) ;
