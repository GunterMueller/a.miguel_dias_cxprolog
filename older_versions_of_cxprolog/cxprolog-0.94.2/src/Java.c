...
static jarray NewJArray(CharPt elemSig, int size)
{
	...
	switch( *elemSig ) {
		...
		case 'B': arr = JNewArray(UChar, size) ; break ;
...
static jarray NewJArrayC(jclass elemClass, int size)
{
	...
	if( JCallMethod(Boolean, elemClass, classClass_isPrimitive) ) {
		...
		if( JIsSameObject(elemClass, byteClass) ) return JNewArray(UChar, size) ;		
...
static jvalue GetJArray(CharPt elemSig, jarray arr, int idx)
{
  ...
  switch( *elemSig ) {
	...
	case 'B': JArrayGet(UChar, arr, idx, &(val.b)) ; break ;
...
static void SetJArray(CharPt elemSig, jarray arr, int idx, jvalue *old, jvalue val)
{
  ...
  switch( *elemSig ) {
	...
	case 'B': JArraySet(UChar, arr, idx, &(old->b), &(val.b)) ; break ;
...
static Pt ConvertPolyObj(jobject obj)
{
	...
	if( JIsInstanceOf(obj, stringClass) )
		...
		res = MakeInt(JCallMethod(UChar, obj, byteObjClass_byteValue)) ;
...
void FixLibClass(CharPt name)
{
	CharPt cl = GStrFormat(PREFIX "lib/cxprolog/%s.class", name) ;
	CharPt j = GStrFormat(PREFIX "lib/cxprolog/%s.java", name) ;
...
/* Do the call and handle result */
	...
	switch( sigLast ) {
		...
		case 'B': {
			jbyte b = JCallMethodA(UChar, targetIsClass, target, id, args) ;
			CheckException() ;
...
/* Get/Set the field */
	...
	switch( *sig ) {
		...
		case 'B': {
			Pt old = MakeByte(JGetField(UChar, targetIsClass, target, id)) ;
			Ensure( UnifyWithAtomic(X2, old) ) ;
			if( Drf(X3) == old ) JumpNext() ;
			val = PrologToJava(sig, &dummySig, X3, true) ;
			JSetField(UChar, targetIsClass, target, id, val.b) ;
			JumpNext() ;
		}
		case 'C': {
