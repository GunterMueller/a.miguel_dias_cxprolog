...
static void RenamePredicate(PredicatePt pr, CharPt newName)
{
	...
	if( StrEqual(PredName(pr), newName) )
		DatabaseError("Idempotent renaming of built-in predicate '%s'",
														PredNameArity(pr)) ;
	if( PredIsMeta(pr) )
		DatabaseError("The core built-in predicate '%' cannot be changed",
														PredNameArity(pr)) ;		
	f = PredFunctor(pr) ;
	built_in = PredIsBuiltin(pr) ;
...
static void PAbolishBuiltin()
{
	PredicatePt pr ;
	if( !Booting() )
		DatabaseError("Abolishing a built-in only works at booting time") ;
	if( (pr = FindPredicate(XTestFunctor2(X0,X1))) == nil )
...
static void PRenameBuiltin()
{
	PredicatePt pr ;
	if( !Booting() )
		DatabaseError("Renaming a built-in only works at booting time") ;
	if( (pr = FindPredicate(XTestFunctor2(X0,X1))) == nil )
		DatabaseError("Predicate '%s' not defined", PredNameArity(pr)) ;
	if( !PredIsBuiltin(pr) )
		DatabaseError("Predicate '%s' is not a built-in", PredNameArity(pr)) ;		
