...
StreamPt BufferStreamOpen(BufferPt buff, StreamMode mode, OpenStreamOptionsPt s)
{
	...
	OpenStreamOptionsPt so =
				s != nil ? s : StreamPropertyGetDefaultOpenOptions(&soDef) ;
	Bool isBinary = so->type == tBinaryAtom ;
	CharPt encoding = EncodingMake(XAtomName(so->encoding)) ; /* may fail */
	StreamPt srm ;
...
{
	FilePt f = FileNew(file, n, m, DefaultEncoding(), absent3) ;
...
static void StreamFormat(StreamPt srm, CharPt fmt, Pt list)
{
	CharPt s = fmt ;
	WChar c ;
	list = Drf(list) ;
...
static void PGetCharWithPrompt()
{
	WChar c ;
	int arg ;
	c = GetCharCommand(XTestAtomName(X0), &arg) ;
...
void StreamsInit()
{
	...
	currIn = userIn = origIn =
		FILEToStream(stdin, AtomName(UserInputAtom), mRead, inKind) ;
