...
static void SetLocaleCType(CharPt l)
...
CharPt systemEncoding ; /* Used for interpreting filenames; OS environment
                      variable names and contents; command-line arguments. */
CharPt defaultEncoding ; /* Default encoding for files and streams. */
...
CharPt StrInternalizeConv(CharPt s, WChar oldChar, WChar newChar)
{
	...
	switch( EncodingGet(SystemEncoding()) ) {
		...
		default: {
			while( (c = StrGetCharInternalize(&s, SystemLocale())) != '\0' ) {
				if( c == oldChar ) c = newChar ;
				GStrAddChar(c) ;
			}
			break ;
...
CharPt StrExternalize(CharPt s)
{
	...
	switch( EncodingGet(SystemEncoding()) ) {
		...
		default: {
			while( (c = CharDecode(s)) != '\0' ) {
				GStrCheck() ;
				StrPutCharExternalize(&GStrCurr(), c, SystemLocale()) ;
			}
			break ;
		}
	}
