 ...
 *   You should have received a copy of the GNU General Public LicenseCompoundTermCopy
...
'$$_lux0' :-									\
	...
	create_unit(main),							\
	'$$_enter_user_mode',						\
	try '$$_ctx_ext'(main,'$cxprolog_initialise'),		\
	try '$$_ctx_ext'(main,'$$_handle_arg'(check)),		\
	try '$$_ctx_ext'(main,'$$_handle_arg'(script)),		\
	try '$$_ctx_ext'(main,'$$_handle_arg'(goal)).		\
														\
'$$_lux1' :-											\
	try '$$_ctx_ext'(main,'$cxprolog_top_level_goal').	\
														\
'$$_handle_arg'(quiet) :-						\
	os_arg(['-q','--quiet'],'')					\
...
'$$_handle_arg'(check) :-						\
	os_arg('--check',_)							\
			-> '$$_ctx_ext'(main,'$$_check_instalation').	\
'$$_handle_arg'(script) :-						\
	os_arg(['-s','--script'],S)					\
			-> silent_consult(S).				\
'$$_handle_arg'(goal) :-						\
...
/* Create root threads and initialize abstract machine */
	ThreadRootCreate(tNilAtom, tNilAtom) ;
...
static void PFeatures()
{
	...
	Write("   - Memory model: %d bits\n",
			ULONG_MAX == 0xFFFFFFFFL ? 32 : 64) ;
...
#if USE_JAVA
	Write("   - Java support: yes\n") ;
#else
	Write("   - Java support: no\n") ;
#endif

#if USE_WXWIDGETS
	Write("   - wxWindows support: yes\n") ;
#else
	Write("   - wxWindows support: no\n") ;
#endif

#if USE_READLINE
	Write("   - GNU Readline support: yes\n") ;
#else
...
{
	CharPt libName = WithLibDirPath("lib", "pl") ;
	if( !OSExists(libName) )
