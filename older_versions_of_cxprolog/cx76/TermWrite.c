...
static int termMaxDepth, listMaxLength ;
...
{
	while( *s ) {
		HandleBufferOverflowBytes(strPut, "write term (1)")
		AddCh(*s++) ;
	}
}
...
static void AddQStr(CharPt s, Bool dirty)
{
	if( dirty ) {
		AddCh('\'') ;
		while( *s ) {
			HandleBufferOverflowBytes(strPut, "write term (2)")
			if( *s == '\'' )
				AddCh('\'') ;
			AddCh(*s++) ;
		}
...
static void WriteTerm2(register Pt term, int p)
{
	HandleBufferOverflowBytes(strPut, "write term (3)")
	term = Drf(term) ;
