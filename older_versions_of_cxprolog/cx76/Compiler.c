...
/*
	PRECONDITIONS:
		...
		The term to be compiled has no 'texts'
		The term to be compiled has no 'extras'
			(but the compiler still considers them due to old code)
*/
...
static void CompileCall(FunctorPt f)
{
	...
	if( 0 && PredIsC(pr) ) CompileCCall(pr) ;
...
static void CompileGoalArgs(Hdl args, int nArgs)
{
	...
	dotimes(currArgN, nArgs) {
		...
		if( t == tNilAtom ) {
			...
			else CompileGoalRecord(t, currArgN) ;
		}
...
static void CompileGoal(Pt goal)
{
	...
	elif( IsText(goal) ) Error("Clause goal is a TEXT") ;

	elif( IsExtra(goal) ) Error("Clause goal is an EXTRA") ;
...
static void HandleUnifyXVariableConflicts(VarDescriptorPt vd)
{
...
static void CompileHeadArgs(Hdl args, int nArgs)
{
	...
	dotimes(currArgN, nArgs) {
		...
		if( t == tNilAtom ) Gen1(GetNil, OutTemp(currArgN)) ;
...
static void CompileHead(Pt head)
{
	...
	elif( IsText(head) ) Error("Clause head is a TEXT") ;

	elif( IsExtra(head) ) Error("Clause head is an EXTRA") ;
...
#if DEBUG == 1
	WriteTerm(head) ; PutStream("\n") ;
	WriteTerm(body) ; PutStream("\n") ;
	ListVarDic() ;
#endif
	...
	if( nGoals == 1 )
		...
		if( nGoals > 2 ) {
			...
			while( IsThisStruct(remainder, commaFunctor) ) {
				HandleBufferOverflowWords(codePt, "compile clause")
