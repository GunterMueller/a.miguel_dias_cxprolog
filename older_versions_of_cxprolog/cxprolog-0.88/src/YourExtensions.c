...
single type for the X parameter. We are referring to the "Test & Extract" family
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
A deterministic builtin predicate written in C succeeds by activating the macro
call JumpNext() and fails activating the macro DoFail().

A nondeterministic builtin predicate written in C succeeds activating the
macro call JumpNext() and stops generating more alternatives and fails,
activating the macro call Jump(DiscardAndFail). A nondeterministic builtin
predicate written in C sould not use the macro DoFail().
...
stacks at the start of each C-builtin: if this in enough for a your specific
predicate, nothing needs to be done. The vast majority of the CxProlog builtins
...
along with the stacks. The CxProlog builtins in this category are:
...
void YourPrologue()
{
	/* What you may do here is very limited because the system has not been
	   initialized yet and not even the memory manager is available.
	   Here you may:
