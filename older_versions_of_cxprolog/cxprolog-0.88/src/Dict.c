...
static void DictDisplay(StreamPt stm, DictPt d)
{
	register EntryPt h ;
...
{
	ForEachExtra(dictType, DictsAtomGCMarkAux) ;
}
...
{
	DictDisplay(currOut, XTestExtra(dictType,X0)) ;
	JumpNext()
}

static void PSDictWrite()
{
	DictDisplay(XTestStream(X0, mWrite), XTestExtra(dictType,X1)) ;
	JumpNext()
}

static void PNDCurrentDict()
{
	PNDCurrentExtra(dictType, nil, 1) ;
	JumpNext()
}
...
static void PDicts()
{
	VersionShow() ;
	Write("Dicts:\n") ;
	ForEachExtra(dictType, DictsAux) ;
	JumpNext()
}
