...
static Pt ZPushTermWithEnv(Pt env, register Pt t, Bool convUnitParams)
{
	...
	if( IsRecord(t) ) {
		...
		PushH(t) ; /* save on GS because of possible rellocation */
		ZEnsureFreeSpaceOnStacks(size, "ZPushTermWithEnv/Record") ;
		t = PopH() ; /* restore from GS */
		to = H ;
		GrowH(size) ;
		return CopyTerm(t, to, env, false, convUnitParams) ;
	}
	elif( IsVar(t) ) {
...
static void PName()
{
	Pt t0 = Drf(X0) ;
	if( IsAtom(t0) ) {
		ZEnsureFreeSpaceOnStacks(2 * strlen(XAtomName(t0)), "name/2") ; /* stacks may grow */
		if( Unify(X1, StringToPString(XAtomName(t0))) ) JumpNext()
		DoFail()
	}
	elif( IsVar(t0) ) {
		Pt t1 = Drf(X1) ;
