...
{
	switch( ((TokenType)(uintptr_t)tks[i * 3]) ) {
		case commaTk:	return true ;
		case closeRTk:	return true ;
		case closeSTk:	return true ;
		case closeCTk:	return true ;
...
static void *ParserErrorX(Str mesg) /* always called after NextToken() */
{
	if( readingFromStringOrList )
		ReaderForceFail() ;
	else
...
#define ParserError(x)	return ParserErrorX(x)
...
static Bool CheckFullStop(void)
{
	WChar c = PeekChar() ;
...
afterSpaceL:
			...
			case '[': {
				NextChar() ;
				if( cc == ']' ) {
					GenToken(nilIsSpecial_flag ? nxTk : atomTk, tNilAtom) ;
					NextChar() ;
				}
...
static void PRead(void)
{
	Pt t = ZReadTerm(currIn) ; /* stacks may grow */
	MustBe( Unify(X0, t) ) ;
}

static void PSRead(void)
{
	StreamPt srm = XTestStream(X0, mRead) ;
...
static void PSReadWithSource(void)
{
	StreamPt srm = XTestStream(X0, mRead) ;
	Pt t = ZReadTerm(srm) ; /* stacks may grow */
...
static void PReadTopLevel(void)
{
	Pt t ;
/* AMD
	InterLineRestart() ;
	InterLineSetPrompts(XTestAtomName(X0), XTestAtomName(X1)) ;
