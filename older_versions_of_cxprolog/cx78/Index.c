...
static void IndexTableExpand()
{
	...
	BlockRelease(indexTable) ;
	indexTableSize *= 2 ;
	indexTable = TempBlockAllocate(indexTableSize * WordsOf(IndexElem)) ;
	IndexTableInit() ;
}
								...
								int nVarClauses, Hdl onlyVarChain)
{
	...
	dotable(el, indexTable, indexTableSize)
		if( not IsFreeIndexElem(el) && IsAtomic(el->idxArg) ) {
			PrologHashInsert(hTable, hTSize, el->idxArg,
									...
									int nVarClauses, Hdl onlyVarChain)
{
	...
	dotable(el, indexTable, indexTableSize)
		if( not IsFreeIndexElem(el) && IsStruct(el->idxArg) ) {
			PrologHashInsert(hTable, hTSize, XPt(el->idxArg),
									GenIndexChain(clauses, el, nVarClauses)) ;
		}
	return res ;
}
...
void DoIndex(PredicatePt pr)
{
	...
	Bool hasIndex ;
	Hdl originalBuffer ;	/* used to check is buffer has moved */

	if( indexParams_flag == 0 || not PredIsIndexed(pr) )
...
redo:
	...
	if( indexParams_flag >= 3 && PredArity(pr) >= 3 ) {
		...
		if( BufferBegin() != originalBuffer ) goto redo ;
	}
