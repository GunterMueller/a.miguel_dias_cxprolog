...
static void BasicLoadFile(CharPt fileName)
{
	register Pt t ;
	See(fileName) ;
	HSave() ;
...
static void BasicLoadStr(CharPt str)
{
	if( strlen(coreBuiltinsStr) < 2 K ) {
		if( str == coreBuiltinsStr ) BasicLoadFile("CxCore.pl") ;
		elif( str == predefinedBuiltinsStr ) BasicLoadFile("CxDefs.pl") ;
		elif( str == defaultBuiltinsStr ) BasicLoadFile("CxBoot.pl") ;
		else Default("BasicLoadStr") ;
	}
	else {
		register Pt t ;
		SeeStr(str) ;
...
{
	if( UnixGetEnv("CXPROLOG_SIZE") != nil )
		Mesg("The enviromnent variable 'CXPROLOG_SIZE' has been discontinued. Undefine it now!") ;
	if( UnixGetArg("-size") != nil )
		Mesg("The command line argument '-size' has been discontinued.  Undefine it now!") ;
}
...
static void PCutTo()
{
	ChoicePointPt newB = cChoicePointPt(Drf(X0)) ;
	if( newB > B ) {
		B = newB ;
		HB = Bf(H) ;
		/*TrailTidy() ;*/
