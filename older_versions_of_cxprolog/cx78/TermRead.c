...
static Pt ReadAndMakeNumber(CharPt s)
{
	...
	PInt i ;
	if( strlen(s) > 256 )
		ReaderError("Float too long") ;
	strcpy(sr, s) ;
	if( cc == '.' && not IsFullStop(cc) ) {
		NextChar() ;	/* Skip '.' */
		strcat(sr, ".") ;
		if( cx_isdigit(cc) ) {
			s = ReadInt10() ;
		...
		else ReaderError("Malformed decimal part of float") ;
	}
	if( cc == 'e' || cc == 'E' ) {
		NextChar() ;
		strcat(sr, "e") ;
		...
		else ReaderError("Malformed exponent of float") ;
	}
	if( sscanf(sr, floatFormat, &d) != 1 )
		ReaderError("Malformed float") ;
	return MakeFloat(d) ;
}
...
atomL:		tkTerm = MakeTempAtom(s) ;
		...
		case _DG: {
			...
			if( cc == '\'' || cc == '\"' ) {
				...
				switch( base ) {
					...
					default: {
						if( base > 99 )
							ReaderError("Too big a base in base'xxx. Max is 99.") ;
						tkTerm = ReadAndMakeInt(base) ;
						break ;
					}
				}
			}
			else
				tkTerm = ReadAndMakeNumber(s) ;
			break ;
		}
		...
		default: {
			Mesg("Ascii code: %d - Categorie: %d", cc, CharType(cc)) ;
			InternalError("NextToken") ;
		}
	}
	nTokens++ ;
}
...
{
	switch( tk ) {
		case atomTk:	WriteStd("atom(%s) ", XAtomName(tkTerm)) ; break ;
...
rightL:
	...
	switch( tk ) {
		case dotTk: case closeRTk: case closeSTk: case closeCTk:
			goto doneL ;
		case vnsTk: case openRTk: case openSTk: case openCTk:
			goto errorL ;
		case commaTk: {
...
static void PSeeing()
{
	if( UnifyWithAtomic(X0, MakeTempAtom(NameOfStream(currIn))) ) JumpNext()
	DoFail()
}
