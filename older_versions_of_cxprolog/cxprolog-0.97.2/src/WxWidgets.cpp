...
public:
	ExitCode Entry() {
		if( CreateConsole() ) {
			RunInteractiveProlog(wxTheApp->argc, wxTheApp->argv) ;
			DeleteConsole() ;
		}
		return 0 ;
	}
...
public:
	...
	void GuiCall(Fun f) {	/* Called in the CxProlog thread */
		...
		if( pevent != peNone )
			SendPrologEvent(pevent) ;	/* Delayed event */
	}
	void OnGuiCall(wxCommandEvent& WXUNUSED(event)) { /* Runs in the GUI thread */
...
void WxWidgetsCallInst() /* Abstract machine instruction */
{
	WxGuiCall(Run3) ; /* No Prolog errors allowed here */
	JumpNext() ;
}
#endif
