...
static CharPt WithSomethingDirPath(CharPt prefix, CharPt path, CharPt name,
														CharPt extension)
{
	if( extension == nil ) {
		if( name == nil )
...
{
	CharPt applDirPath ;
	if( (applDirPath = OSApplDir()) == nil )
		return Error("Cannot discover the application directory path") ;
#if 0
	if( !OSExists(applDirPath) )
		applDirPath = "" ;
#endif
	return WithSomethingDirPath(applDirPath, path, name, extension) ;
}

CharPt WithPrefixDirPath(CharPt path, CharPt name, CharPt extension)
{
	CharPt prefixDirPath ;
	if( (prefixDirPath = OSPrefixDir()) == nil )
		return Error("Cannot discover the application directory path") ;
#if 0
	if( !OSExists(prefixDirPath) )
		prefixDirPath = "" ;
#endif
	return WithSomethingDirPath(prefixDirPath, path, name, extension) ;
}
...
CharPt WithPreferencesDirPath(CharPt name, CharPt extension)
{
...
{
	CharPt path ;
	path = WithAppDirPath("pl", nil, nil) ;
	if( OSExists(path) ) {
		SetCurrDir(LookupTempAtom(path), path) ;
		JumpNext() ;
	}
	else
		Error("Cannot access directory '%s'", path) ;
	JumpNext() ;
}
...
{
	CharPt path ;
	path = WithAppDirPath("examples", nil, nil) ;
	if( OSExists(path) ) {
		SetCurrDir(LookupTempAtom(path), path) ;
		JumpNext() ;
	}
