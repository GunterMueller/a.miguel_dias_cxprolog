...
static Pt MakeVarNamesList(void)
{ /* pre: space is available on stacks */
	register Hdl h ;
	register Pt list ;
	list = tNilAtom ;
...
static Bool PostReadOptions(Pt t)
{
	...
	Pt variableNamesList = variable_names ? MakeVarNamesList() : nil ;
...
static Bool readingFromStringOrList ;
...
static void PVarNames()
{
	...
	MustBe( Unify(X0, MakeVarNamesList()) ) ;
}
