...
static FILE *FileOpen(CharPt name, int mode)
{
	Str32 m ;
	strcpy(m, mode==mWrite ? "w" : mode==mRead ? "r" : "a") ;
	return fopen(StrExternalize(name), m) ;
}
...
FilePt FileNew(FILE *file, CharPt name, int mode, CharPt encoding, Bool3 bom)
{
	FilePt f ;
	Bool diskFile = file == nil ;
	name = AtomName(LookupTempAtom(name)) ; /* name string will be protected by stream */
...
void FileSetEncoding(FilePt f, CharPt encoding)
{
...
CharPt FileGetEncoding(FilePt f)
{
...
static WChar FileGet(FilePt f)
{
	...
	switch( EncodingGet(FileEncoding(f)) ) {
		...
		case textEncoding: {
			return FileGetInternalize(file, SystemLocale(), fName) ;
		}
		case asciiEncoding: {
			int c ;
			if( (c = fgetc(file)) > 127 )
		...
		default: {
			return FileGetInternalize(file, FileEncoding(f), fName) ;
		}
	}
}
...
void FilePutChar(FilePt f, WChar c)
{
	...
	switch( EncodingGet(FileEncoding(f)) ) {
		...
		case textEncoding: {
			FilePutExternalize(file, c, SystemLocale(), fName) ;
			break ;
		}
		case asciiEncoding: {
			if( !InRange(c, 0, 127) ) {
		...
		default: {
			FilePutExternalize(file, c, FileEncoding(f), fName) ;
			break ;
		}
	}
}
