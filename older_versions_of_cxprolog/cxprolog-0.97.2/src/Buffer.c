...
BufferPt BufferNew()
{
	...
	BufferInit(b, 4, 0, 0) ;
	BufferEncoding(b) = SystemEncoding() ;
	return b ;
}
...
WChar BufferGetChar(BufferPt b)
{
	...
	switch( EncodingGet(BufferEncoding(b)) ) {
		...
		case textEncoding: {
			c = StrGetCharInternalize(&s, SystemLocale()) ;
			break ;
		}
		case asciiEncoding: {
			if( (c = *cUCharPt(s++)) > 127 )
		...
		default: {
			c = StrGetCharInternalize(&s, BufferEncoding(b)) ;
			break ;
		}
	}
...
void BufferPutChar(BufferPt b, WChar c)
{
	...
	switch( EncodingGet(BufferEncoding(b)) ) {
		...
		case textEncoding: {
			StrPutCharExternalize(&s, c, SystemLocale()) ;
			break ;
		}
		case asciiEncoding: {
			if( !InRange(c, 0, 127) )
		...
		default: {
			StrPutCharExternalize(&s, c, BufferEncoding(b)) ;
			break ;
		}
	}
	BufferPosition(b) = cUCharPt(s) ;
