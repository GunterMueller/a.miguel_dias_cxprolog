...
static ProcessPt ProcessNew(Pt startGoal, Pt restartGoal)
{
	...
	if( (child = OSFork()) > 0 ) {	/* PARENT */
		...
		ProcessDeleteAll() ;
		processType = nil ;	/* mark "this is a child" */
		startGoal = MakeBinStruct(LookupFunctorByName(">>", 2),
								MakeAtom("main"),
								startGoal) ;
		restartGoal = MakeBinStruct(LookupFunctorByName(">>", 2),
								MakeAtom("main"),
								restartGoal) ;
...
static void PProcessNew()
{
	...
	if( ProcessIsChild() )
		Error("A child process cannot do that") ;
	if( IsVar(t) ) {
		ProcessPt q = ProcessNew(Drf(X1), Drf(X2)) ;
		if( UnifyWithAtomic(t, TagExtra(q)) ) JumpNext()
		InternalError("PProcessNew") ;
	}
	if( IsAtom(t) ) {
		ProcessPt q = ProcessNew(Drf(X1), Drf(X2)) ;
...
void ProcessesInit()
{
	...
	InstallNDeterCBuiltinPred("processes", 0, PProcesses) ;
