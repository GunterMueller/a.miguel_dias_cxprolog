...
void LocalStackShow()
{
	...
	for( a = stacksEnd-1 ; a >= aLim ; a-- ) {
		...
		if( IsVar(*a) ) {
			if( *a == nil )
				Write("NULL AT LOCAL STACK >>> %ld (dist = %ld)\n",
												*a, a-stacksBegin) ;
			elif( IsLocalVarStrong(*a) ) Write("LOCAL %lx\n", *a) ;
...
static void LocalStackRelocate(Hdl newStacksBegin, Hdl newStacksEnd)
{
	...
	for( a = stacksEnd-1, z = newStacksEnd-1 ; a >= aLim ; a--, z-- ) {
		if( IsVar(*a) ) {
			if( *a == nil )
				Mesg("NULL AT LOCAL STACK >>> %ld (dist = %ld)",
												*a, a-stacksBegin) ;
...
void GlobalStackShow()
{
	...
	for( a = stacksBegin ; a < H ; a++ ) {
		...
		if( IsVar(*a) ) {
			if( *a == nil )
				Write("NULL AT GLOBAL STACK >>> %ld (dist = %ld)\n",
												*a, a-stacksBegin) ;
			elif( IsGlobalVarStrong(*a) )
				Write("GLOBAL %lx\n", *a) ;
			elif( FunctorCheck(*a) )
...
static void GlobalStackRelocate(Hdl newStacksBegin) {
	register Hdl a, z ;
	for( a = stacksBegin, z = newStacksBegin ; a < H ; a++, z++ ) {
		if( IsVar(*a) ) {
			if( *a == nil )
				Mesg("NULL AT GLOBAL STACK >>> %ld (dist = %ld)",
												*a, a-stacksBegin) ;
			elif( IsGlobalVarStrong(*a) )
				*z = *a + (newStacksBegin - stacksBegin) ;
			elif( IsLocalVarStrong(*a) )
