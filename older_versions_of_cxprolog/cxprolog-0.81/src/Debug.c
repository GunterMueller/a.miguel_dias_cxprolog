...
static Bool InterDebug(DebugEvent e, DebugFramePt d)
{
	...
	switch( mode ) {
		...
		case mQuasiSkip: {
			if( d->frameN != skippingTo && not PredIsSpy(d->pred) )
				return false ;
			mode = mNeutral ;
			leash = true ;
			break ;
		...
		case mLeap: {
			if( not PredIsSpy(d->pred) ) return false ;
			mode = mNeutral ;
			leash = true ;
			break ;
		}
	...
	for(;;) {
		if( not DebugLine(e, d, leash) ) return false ;
		...
		switch( comm ) {
			...
			case 's': {					/* skip */
				if( arg >= 0 && not FindDebugFrame(arg, &d) ) break ;
				mode = mSkip ;
				skippingTo = d->frameN ;
				return false ;
			}
			...
			case '+': {					/* spy this */
				SpyFunctor(PredFunctor(d->pred), tTrueAtom) ;
				break ;
			}
			case '-': {					/* nospy this */
				SpyFunctor(PredFunctor(d->pred), tFalseAtom) ;
				break ;
			}
			default: {
				WriteStd("\
...
Bool DebugCall(PredicatePt pr)
{
	...
	switch( mode ) {
		...
		case mQuasiSkip:
		case mLeap:			if( not PredIsSpy(pr) ) return false ;
							else break ;
		case mInterrupt:	break ;
	}
	if( not PredIsTraceable(pr) ) return false ;

	if( DD == nil ) {
		if( PredIsBuiltin(pr) ) return false ;
		if( debug_flag == 1 && not PredIsSpy(pr) && mode != mInterrupt )
			return false ;
	}
...
void DebugInterrupt(int newValue) /* Called from InterruptPostHandler() */
{
	if( DebugUpdateFlags(newValue) )
