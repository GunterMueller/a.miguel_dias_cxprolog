...
int StrSeqGetIdx(Str a, CharPt *vs)
{
	register int i ;
	for( i = 0 ; *vs != nil ; i++, vs++ )
		if( StrEqual(a, *vs) )
...
CharPt StrSeqFormat(Str pre, Str sep, Str pos, CharPt *vs)
{
	GStrOpen() ;
	GStrAddStr(pre) ;
	for( ; *vs != nil ; vs++ ) {
...
void StrSplitElems(Str str, CharHdl v, int b)
{
...
#define initGStrCapacities	(scratch_debugging ? 16000 : 64) /* bytes */
...
#define bigStrInitialCapacity	(scratch_debugging ? 100 K : 4 K)	/* bytes */
