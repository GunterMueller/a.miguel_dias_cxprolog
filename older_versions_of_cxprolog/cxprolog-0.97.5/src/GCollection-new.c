 ...
 *   Copyright (C) 1990-2010 A.Miguel Dias, CITI, DI/FCT/UNLExtra
...
static void RelocateLocalStack2(Size localOffset)
{
...
static void RelocateStacksGC(Size globalOffset)
{	
#if 0
	RelocateStacks(globalOffset, 0) ;
#else
...
static void growNewGlobal(Hdl *next, Hdl *scan, Str mesg)
{
...
/* Create new area */
	newGlobalBegin2 = AllocateSegment(2 * newGlobalSize, &newGlobalEnd2) ;
...
/* Copy and relocate to new area everything except the original global stack */
	...
	*next += globalOffset2 ;
	if( scan != nil )
		*scan += globalOffset2 ;
//	Mesg("ZZZZZZZZZZZZZZZZZZZZZZZZ growNewGlobal %ld %x", globalOffset2, *next) ;
}

static void NewStackShift(Hdl next)
{
	if( stacksEnd - H  < next - newGlobalBegin )
		growNewGlobal(nil, nil, "") ;
	RelocateLocalStack2(newGlobalEnd - stacksEnd) ;
}
			...
			if( IsList(*scan) ) {
				...
				if( IsAlreadyForwarded(*XHdl(*scan)) ){
					*scan = TagStruct(*XHdl(*scan)) ;  //forward the address on stack extracting the tag to follow the correct address
					...
					int i, n = FunctorArity(f) ;
					if( next + 1 + n >= newGlobalEnd ) /* Grow if necessary */
						growNewGlobal(&next, &scan, "struct") ;
...
{
	Unused(newGlobalEnd) ;
	RelocateStacksGC(newGlobalBegin - stacksBegin) ;
	return H ;
}
#endif
...
{
//Dot("@") ;
//Mesg("E %ld", stacksEnd - cHdl(E)) ;
	//WriteNothing("INICIAL &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&  %lx \n", C);
	Hdl saveStacksBegin ;
	Hdl originalGlobalSize = H - stacksBegin ;
	Hdl newGlobalSize ;
	Size threshold = 1000 ;
Size originG = H - stacksBegin ;
Size originF = H - TopOfLocalStack() ;
	
	newGlobalBegin = AllocateSegment(H - stacksBegin, &newGlobalEnd) ;
	//WriteNothing("newGlobalBegin %x, newGlobalEnd %lx, stacksBegin = %lx stacksEnd %lx", newGlobalBegin, newGlobalEnd,stacksBegin,stacksEnd) ;
	//WriteNothing("OFFSET############# %lx \n", (stacksBegin-newGlobalBegin));
//Dot("$") ;
	...
	if( newGlobalSize <= originalGlobalSize ) {
		saveStacksBegin = stacksBegin ;
		stacksBegin = newGlobalBegin ;
		//WriteNothing("Antes da Relocação &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&  %lx \n", XPt(C));
		RelocateStacks(saveStacksBegin - newGlobalBegin, 0) ;
		//WriteNothing("Depois da Relocação &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&  %lx \n", XPt(C));
		stacksBegin = saveStacksBegin ;	
...
//if( H - stacksBegin >= originG )
//	Mesg("EXCEDEU %ld %ld %ld", H - stacksBegin - originG, originF, stacksEnd - cHdl(E)) ;
//Mesg("S %ld", stacksEnd - cHdl(E)) ;
//Dot(".") ;
}
