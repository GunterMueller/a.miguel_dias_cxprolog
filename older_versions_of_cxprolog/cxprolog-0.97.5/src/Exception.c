...
Pt BuildExceptionTermV(Str errKind, Str objectType, Str culprit,
												Str fmt, va_list v)
{
...
{
	Throw(BuildExceptionTermV(kind, nil, nil, fmt, v)) ;
}
...
void ThrowPrologExceptionMesg(Str kind, Str fmt, ...)
{
	...
	va_list v ;
	va_start(v, fmt) ;
	exc = BuildExceptionTermV(kind, nil, nil, fmt, v) ;
    va_end(v) ;
	Throw(exc) ;
}
#endif
