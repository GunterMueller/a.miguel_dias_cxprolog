...
void ShowGoal(int n ,CharPt goal)
{
	if( n > 1 ) Write("    ") ;
#if DEBUG
...
void ShowRes(int n, PrologEvent pe, CharPt vars, CharPt out)
{
	Pt t ;
	if( n > 1 ) Write("    ") ;
	if( pe == peException ) WriteException() ;
	if( n > 1 ) Write("    ") ;
#if DEBUG
...
void CallPM2(CharPt goal1, CharPt goal2) {
	PrologEvent pe1, pe2 ;
	CharPt vars1, vars2, out1, out2 ;
	Write("--------\n") ;
	ShowGoal(1, goal1) ;
