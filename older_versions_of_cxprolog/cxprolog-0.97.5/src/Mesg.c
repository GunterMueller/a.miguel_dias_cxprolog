												...
												Str fmt, va_list v)
{
	...
	switch( onError_flag ) {
		...
		case 3: {
			return BuildExceptionTermV(errKind, objectType, culprit, fmt, v) ;
		}
		default:
...
VoidPt ExistenceError(Str objectType, Str culprit, Str fmt, ...)
{
	...
	va_start(v, fmt) ;
#if ISO_ERRORS
	exc = ErrorEventPrepareV("existence_error", objectType, culprit, fmt, v) ;
#else
	Unused(objectType) ;
	Unused(culprit) ;
	exc = ErrorEventPrepareV("ERROR", nil, nil, fmt, v) ;
#endif
	va_end(v) ;
...
static VoidPt TypeErrorAux(Str fmt, ...)
{
	...
	va_list v ;
	va_start(v, fmt) ;
	exc = ErrorEventPrepareV("TYPE ERROR", nil, nil, fmt, v) ;
	va_end(v) ;
	return ErrorEventTrigger(exc) ;
}
...
VoidPt SyntaxError(Str fmt, ...)
{
	...
	va_list v ;
	va_start(v, fmt) ;
	exc = ErrorEventPrepareV("SYNTAX ERROR", nil, nil, fmt, v) ;
	va_end(v) ;
	return ErrorEventTrigger(exc) ;
}
...
VoidPt GenericError(Str kind, Str fmt, ...)
{
	...
	va_list v ;
	va_start(v, fmt) ;
	exc = ErrorEventPrepareV(kind, nil, nil, fmt, v) ;
	va_end(v) ;
	return ErrorEventTrigger(exc) ;
}
...
VoidPt DatabaseError(Str fmt, ...)
{
	...
	va_list v ;
	va_start(v, fmt) ;
	exc = ErrorEventPrepareV("DATABASE ERROR", nil, nil, fmt, v) ;
	va_end(v) ;
	return ErrorEventTrigger(exc) ;
}
...
{
	if( infoMessages_debugging > 0 )
		newValue = Max(infoMessages_debugging, newValue) ;
