...
#define Cyclic1_PrepareArgs(args, arity, replacement)					\
	Do(	ScratchPush(t) ;			/* Save the term being visited */	\
		*h = replacement ;			/* Mark it as a visited term */		\
...
#define Cyclic1P_PrepareArgs(args, arity, replacement)					\
	Do(	ScratchPush(end) ;			/* Save sequence end */				\
		...
		*h = replacement ;			/* Mark subterm as being visited */	\
	)
