...
static void RelocateXRegs(Size globalOffset, Size localOffset)
{
	...
	dotimes(i, maxX) {
		if( IsVar(Xc(i)) ) {
			if( localOffset != 0 && IsLocalRef(Xc(i)) ) Xc(i) += localOffset ;
			elif( globalOffset != 0 && IsGlobalRef(Xc(i)) ) Xc(i) += globalOffset ;
			else { /* nothing */ } ;
		}
		elif( globalOffset != 0 && IsGlobalCompound(Xc(i)) ) Xc(i) += globalOffset ;
		elif( IsAtomic(Xc(i)) || IsCompound(Xc(i)) ) ;
		...
		for( a = stacksEnd - 1, z = a + localOffset ; a >= lTop ; a--, z-- )
		{
			if( IsEmpty(*a) )
				*z = *a ;
				...
				if( localOffset != 0 && IsLocalRef(*a) ) *z = *a + localOffset ;
				elif( globalOffset != 0 && IsGlobalRef(*a) ) *z = *a + globalOffset ;
				else *z = *a ;
			}
			elif( globalOffset != 0 && IsGlobalCompound(*a) ) *z = *a + globalOffset ;
			elif( IsAtomic(*a) || IsCompound(*a) || IsInstruction(*a) ) *z = *a ;
			else FatalError("RelocateLocalStack (>=0) %lx", *a) ;
		}
	else
...
static void RelocateGlobalStack(Size globalOffset, Size localOffset) {
	...
	if( globalOffset >= 0 ) /* This allows the stacks to overlap */
		for( a = H - 1, z = a + globalOffset ; a >= stacksBegin ; a--, z-- )
		{
			...
			elif( IsAtomic(*a) || IsCompound(*a) ) *z = *a ;
...
{
	if( testRelocation_debugging > 0 )
		newValue = Max(testRelocation_debugging, newValue) ;
