		...
		for( a = &atomTable[i] ; *a != nil ; ) {
			if( not AtomPermanent(*a) && not AtomMarked(*a) ) {
				...
				if( testGCollection_flag )
					Warning("Removed atom '%s'", AtomName(*a)) ;
				*a = (*a)->nextHash ;
				BlockRelease(r) ;
				n++ ;
			}
...
static Size atomPerm, atomTemp, atomFunc ;

static Size AtomCount(AtomPt a, Pt x)
{
	if( AtomFunctors(a) != nil ) atomFunc++ ;
