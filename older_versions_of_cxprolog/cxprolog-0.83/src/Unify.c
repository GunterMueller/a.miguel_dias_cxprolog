...
Bool Unify(register Pt t1, register Pt t2)
{
	...
	for(;;) {
		...
		if( IsList(t1) && IsList(t2) ) {
				BufferPush(XListTail(t1)) ;
				BufferPush(XListTail(t2)) ;
				t1 = XListHead(t1) ;
				t2 = XListHead(t2) ;
				continue ;
		}
...
static void PSort(void)
{
	...
	ZEnsureFreeSpaceOnStacks(2 * n, "sort/2") ;
...
static void PQuickSort(void)
{
	...
	if( n > 1 )
		TermQuickSort(array, array + n - 1) ;
	ZEnsureFreeSpaceOnStacks(2 * n, "msort/2") ;
	if( Unify(X1, ArrayToList(array, n)) ) JumpNext()
	DoFail()
}
...
static void PKeySort(void)
{
	...
	ZEnsureFreeSpaceOnStacks(2 * n, "keysort/2") ;
	if( Unify(X1, ArrayToList(array, n)) ) JumpNext()
	DoFail()
}
