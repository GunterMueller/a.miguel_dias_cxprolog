 ...
 *   Copyright (C) 19ð0-2002 A.Miguel Dias, CITI, DI/FCT/UNL
...
static UnitPt NewAnonymousUnit(void)
{
	UnitPt u = TempBlockAllocate(WordsOf(Unit)) ;
	UnitFunctor(u) = emptyFunctor ;
...
static UnitPt LookupUnit(FunctorPt f)
{
	UnitPt u ;
	CharPt s = FunctorName(f) ;
	if( *s == '\0' || EqualStr(s, "[]") )
		Error("Invalid unit name '%s'.", s) ;
...
static Bool Duplicates(Hdl params, int nPars)
{
	register int i, j ;
	dotimes(i, nPars)
		for( j = i + 1 ; j < nPars ; j++ )
...
UnitPt DefineUnit(AtomPt atom, Hdl params, int nPars)
{
	...
	if( not UnitIsDefined(u) ) {
		...
		/*	if( AtomName(a)[0] != 'U' )
				DatabaseError("Unit parameter names must start with an 'U'") ;
			if( AtomName(a)[1] == '\0' )
				DatabaseError("Invalid unit parameter name 'U'") ;*/
...
UnitPt TermToUnit(register Pt term)
{
	...
	elif( IsVar(term) ) DatabaseError("Unit name is not bound") ;
...
{
	UnitPt u = CurrUnit() ;
	if( C != tNilAtom && Unify(X0, MakeStruct(UnitFunctor(u), UnitParams(u))) ) JumpNext()
	DoFail()
}

static void PUnitInUse()
{
	UnitPt u = FindUnit(XTestAtom(X0)) ;
	if( u == nil ) Mesg("There is no unit '%s'", XTestAtomName(X0)) ;
	elif( IsUnitInUse(u) ) Mesg("Unit '%s' is in use", XTestAtomName(X0)) ;
	else Mesg("There is unit '%s'", XTestAtomName(X0)) ;
	JumpNext()
}
...
static void PNDCurrentUnit()
{
	UnitPt u = A(1) == tNilAtom
				? FirstUnit()
				: NextUnit(cUnitPt(A(1))) ;
	if( u == builtinUnit ) u = NextUnit(u) ;
	A(1) = cPt(u) ;
	if( u == nil ) Jump(DiscardAndFail) ;
	if( Unify(X0, MakeStruct(UnitFunctor(u), UnitParams(u))) ) JumpNext()
	DoFail()
}
