...
static void CompilePrimitiveClause(int n) {
	switch( n ) {
		case 1:
			Gen0(PushUnitAndCallVar) ; 
			break ;
		case 2:
			Gen0(SwitchContextAndCallVar) ; 
			break ;
		case 3:
			Gen0(EmptyContextCallVar) ; 
			break ;
		default:
			Error("Not a primitive special predicate") ;
	}
}
...
void Compiler(Pt head, Pt body, Hdl *cd, long *size)
{
	...
	if( IsThisStruct(body, primitiveFunctor) ) {
		CodeReset() ;
		CompilePrimitiveClause(XTestInt(XStructArg(body,0))) ;
		*cd = codeBuffer ;
		*size = CodeSize() ;
		return ;
	}
...
a/0
  ...
  46318c TrustMe 0
  463198 Allocate
  46319c PutCutLevel X0
  4631a4 PutStructure write/1 X1
  4631b0 BuildAtomic '!'
  4631b8 Deallocate
  4631bc Execute ;/2
