...
{
	ClausePt cl = SpaceAlloc(WordsOf(Clause) + codeLen, false) ;
...
static ClausePt InstallClause(PredicatePt pr, Hdl code, int codeLen, Pt head, Pt term, Bool end)
{
...
void CompileClause(register Pt clauseTerm, Bool end)
{
	...
	clauseTerm = CopyTermToSpace(clauseTerm) ;
...
void DeleteClause(ClausePt cl)
{
	...
	if( ClauseTerm(cl) != nil ) FreeSpaceTerm(ClauseTerm(cl)) ;
	SpaceFree(cl) ;
	InvalidIndex(pr) ;
}
