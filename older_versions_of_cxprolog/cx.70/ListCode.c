	...
	for(;;) {
		...
		while( *types != '\0' )
			switch( *types++ ) {
				...
				case 'a': {
					Pt term = GetCode() ;
					if( IsAtom(term) ) Write(" '%s'", XAtomName(term)) ;
					elif( IsInt(term) ) Write(" %ld", XInt(term)) ;
					elif( IsExtra(term) ) Write(" EXTRA(%s)", XExtraName(term)) ;
					else Write(" %f", XReal(term)) ;
					break ;
				}
				case 'u': {
...
void ListAtomCode(AtomPt at)
{
	...
	dolist(f, AtomFunctors(at), f->nextArity)
		dolist(pr, FunctorPreds(f), PredNextF(pr))
			if( PredUnit(pr) == builtinUnit || PredUnit(pr) == CurrUnit() )
