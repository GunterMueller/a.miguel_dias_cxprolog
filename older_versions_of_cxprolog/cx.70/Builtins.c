...
static void PClause()
{
	...
	SplitClauseTerm(CopyTermToGlobal(ClauseTerm(cl)), &head, &body) ;
...
static void PImportedPredicate()
{
	PredicatePt pr =
...
static void PCurrentImperativeVar()
{
	ImperativeVarPt iv =
...
{
	SetLocalOperatorsFlag(true) ;
	JumpNext()
}

static void PNoLocalOperators()
{
	SetLocalOperatorsFlag(false) ;
	JumpNext()
}
...
{
	SetTraceOnFlag(false) ;
	JumpNext()
}

static void PAttention()
{
	SetAttentionOnFlag(true) ;
	JumpNext()
}

static void PNoAttention()
{
	SetAttentionOnFlag(false) ;
	JumpNext()
}

static void PForceVisibility()
{
	SetForceVisibilityFlag(true) ;
	JumpNext()
}

static void PNoForceVisibility()
{
	SetForceVisibilityFlag(false) ;
	JumpNext()
}
...
/* PREDICATES */
	InstallCBuiltinPred("$$_clause", 2, PClause) ;
	...
	InstallCBuiltinPred("$$_retract", 1, PRetract) ;
	InstallCBuiltinPred("$$_current_predicate", 1, PCurrentPredicate) ;
	InstallCBuiltinPred("$$_visible_predicate", 1, PVisiblePredicate) ;
	InstallCBuiltinPred("$$_imported_predicate", 2, PImportedPredicate) ;
	InstallCBuiltinPred("$$_builtin_predicate", 1, PBuiltinPredicate) ;
...
/* IMPERATIVE VARIABLES */
	...
	InstallCBuiltinPred("set_ivar", 2, PImperativeVarSet) ;
	InstallCBuiltinPred("get_ivar", 2, PImperativeVarGet) ;
	InstallCBuiltinPred("$$_current_ivar", 1, PCurrentImperativeVar) ;
...
/* OPERATORS */
	...
	InstallCBuiltinPred("$$_current_op", 1, PCurrentOp) ;
...
/* UNITS / CONTEXTS */
	...
	InstallCBuiltinPred("$$_current_unit", 1, PCurrentUnit) ;

/* FLAGS */
	InstallCBuiltinPred("trace", 0, PTrace) ;
	InstallCBuiltinPred("notrace", 0, PNoTrace) ;
	InstallCBuiltinPred("$$_attention", 0, PAttention) ;
	InstallCBuiltinPred("$$_noattention", 0, PNoAttention) ;
	InstallCBuiltinPred("force_visibility", 0, PForceVisibility) ;
	InstallCBuiltinPred("noforce_visibility", 0, PNoForceVisibility) ;

/* MISC */
	InstallCBuiltinPred("$$_event", 1, PEvent) ;
	InstallCBuiltinPred("status", 0, PStatus) ;
