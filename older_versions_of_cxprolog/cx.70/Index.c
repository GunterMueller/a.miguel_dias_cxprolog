...
static Hdl GenSingleEntryCode(PredicatePt pr, int nVarClauses, Bool atomic)
{
	register IndexElemPt el ;
	Hdl res ;
	...
	res = GenIndexChain(pr, el, nVarClauses) ;
	InitIndexElem(el) ;
	return res  ;
}
...
{
	indexTableSize = 256 ;
	indexTable = SpaceAlloc(indexTableSize * WordsOf(IndexElem), false) ;
	InitIndexTable() ;
}

static void ReinitIndex()
{
	SpaceFree(indexTable) ;
	indexTableSize *= 2 ;
	indexTable = SpaceAlloc(indexTableSize * WordsOf(IndexElem), false) ;
	InitIndexTable() ;
}
...
redo:
	...
	dolist(cl, PredFirstClause(pr), ClauseNext(cl)) {
		...
		elif( IsExtra(t) ) InsertIndexElem(&vars, cl, cPt(-1)) ;
	...
	PredStartInstArgs(pr)[0] = cPt(
		nConsts == 0
			? onlyVarChain :
		nConsts == 1
			? GenSingleEntryCode(pr, nVarClauses, true)
