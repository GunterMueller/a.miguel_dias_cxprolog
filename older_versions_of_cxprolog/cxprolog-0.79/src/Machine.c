...
/* The local stack contains local vars, references to global vars,
   ...
   to terms in the global stack, "state cells" of non-deterministic
   builtins. */
...
{
	PushH(TermToUnit(X0)) ;	/* Unit ref placed in the global stack */
	PushH(X0) ;
	PushH(C) ;
	C = TagList(H-2) ;
	X0 = X1 ;
...
{
	insts[nInsts].inst = *inst = InstEncode(proc) ;
...
void MachineInit()
{
	PFailAddr = InstEncode(FailInst) ;
	C = tNilAtom ;	/* This makes CurrUnit() == bottomUnit */
	InstructionsInit() ;
	Call = InstEncode(SysCallInst) ;
	Execute = InstEncode(SysExecuteInst) ;
/*	CallVar = InstEncode(SysCallVarInst) ;
	ExecuteVar = InstEncode(SysExecuteVarInst) ;*/
}

void UserModeInstructions()
{
	Call = InstEncode(CallInst) ;
	Execute = InstEncode(ExecuteInst) ;
	CallVar = InstEncode(CallVarInst) ;
	ExecuteVar = InstEncode(ExecuteVarInst) ;
}
