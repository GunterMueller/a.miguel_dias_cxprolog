...
/* hash function */
	while( *u ) slot += *u++ ;
/* find atom */
	dolist(at, atomTable[slot % atomTableSize], at->nextHash)
...
/* create new atom */
	...
	at->nextHash = atomTable[slot % atomTableSize] ;
	atomTable[slot % atomTableSize] = at ;
	AtomFunctors(at) = nil ;
...
/* find atom */
	dolist(at, atomTable[slot % atomTableSize], at->nextHash)
		if( EqualStr(AtomName(at), name) )
			return at ;	
/* create new atom */
	at = TempBlockAllocate(size = WordsOf(Atom) + Words(strlen(name) + 1)) ;
