...
static void StartTokens(Hdl start)
{
	tks = start ;
}
...
afterSpaceL:	if( cc == '(' ) {
			...
			case '[': GenToken(openSTk, nil) ; NextChar() ; break ;
			case ']': GenToken(closeSTk, nil) ; NextChar() ; break ;
			case '{': GenToken(openCTk, nil) ; NextChar() ; break ;
...
static Pt ZReadTokensStream(StreamPt srm)
{
	...
	switch( setjmp(readJmp) ) {
		case 0: {
			Pt res = tNilAtom ;
			Size expectedSize = 6 * PrepareTokens() ;
			Hdl h = &res + 1 ;
			ZEnsureFreeSpaceOnStacks(expectedSize) ;

			StartTokens(BufferBegin()) ;
			for(;;) {
				NextToken() ;
...
static void PStreamCheck()
{
	...
	if( IsExtra(t) && XExtraSubTag(t) == streamSubTag && StreamCheck(XPt(t)) )
		JumpNext()
	DoFail()
}
...
static void POpen()
{
	...
	if( IsVar(t) ) {
		StreamPt srm = StreamFileOpen(name, m) ;
