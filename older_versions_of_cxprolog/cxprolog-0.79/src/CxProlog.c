...
static void ZBasicLoadStr(CharPt str)
{
	if( strlen(coreBuiltinsStr) < 2 K ) {
		if( str == coreBuiltinsStr ) ZBasicLoadFile(":pl:CxCore.pl") ;
		elif( str == predefinedBuiltinsStr ) ZBasicLoadFile(":pl:CxDefs.pl") ;
		elif( str == defaultBuiltinsStr ) ZBasicLoadFile(":pl:CxBoot.pl") ;
		else Default("ZBasicLoadStr") ;
	}
	else {
		register Pt t ;
		SeeStr(str) ;
