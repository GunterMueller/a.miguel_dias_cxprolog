...
'$$_lux1' :-									\
	try('$cxprolog_top_level_goal').			\
												\
'$$_tests0' :-									\
	rename_builtin('$$_test_rename_builtin', 0, '$$_test_rename_builtin2').	\
												\
'$$_handle_arg'(quiet) :-						\
	os_arg(['-q','--quiet'],'')					\
...
#else
	Write("   - Java suppor enabledt: no\n") ;
#endif

#if USE_WXWIDGETS
	Write("   - wxWidgets suppor enabledt: yes\n") ;
#else
	Write("   - wxWidgets suppor enabledt: no\n") ;
#endif
