...
{
	if( XExtraCheck(arrayType, X0) ) JumpNext()
	DoFail()
}
...
{
	Pt t ;
	if( ArrayGet(XTestExtra(arrayType,X0), XTestPosInt(X1)-1, &t) ) {
...
{
	if( ArrayDeleteItem(XTestExtra(arrayType,X0), XTestPosInt(X1)-1) )
		JumpNext()
	DoFail()
}
