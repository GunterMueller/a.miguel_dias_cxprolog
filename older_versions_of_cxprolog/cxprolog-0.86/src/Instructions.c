...
static void CutInst()
{
	CutTo(Ef(B0)) ; /* Cut to B at clause entry */
	JumpNext()
}

static void PMetaCut()
{
	CutTo(cChoicePointPt(X0)) ; /* Cut to B at clause entry */
	JumpNext()
}
...
static void MarkCPredInst()
{
	FatalError("MarkCPred") ;
}
...
Pt	Nop, FAllocate, FDeallocate, Proceed, DeallocProceed, LocalJump,
	...
	Cut, PutCutLevel, Fail, Undef, NotRunning, MarkCPred,
...
//	II(&DynamicCut,		PDynamicCut,			"DynamicCut",		"") ;
	...
	II(&MarkCPred,		MarkCPredInst,			"MarkCPred",		"") ;
