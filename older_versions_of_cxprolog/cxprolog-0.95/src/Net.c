...
static StreamPt NetFILEToStream(FILE *file, CharPt p, StreamMode m, StreamKind k)
{
	return FILEToStream(file, GStrFormat("%s_%lx", p, cWord(file)), m, k, true) ;
}
...
static void PNetAccept()
{
	...
	if( !InUse(server) )
		Error("The CxProlog server is not active") ;
	OSAccept(server, &r, &w) ;
	BindVarWithExtra(X0, NetFILEToStream(r, "_%ssocket", mRead, textFileStream)) ;
	BindVarWithExtra(X1, NetFILEToStream(w, "_%ssocket", mWrite, textFileStream)) ;
	JumpNext() ;
}
...
static void PNetConnect()
{
	...
	BindVarWithExtra(X2, NetFILEToStream(r, "_%csocket", mRead, textFileStream)) ;
	BindVarWithExtra(X3, NetFILEToStream(w, "_%csocket", mWrite, textFileStream)) ;
	JumpNext() ;
}
