...
{		/* t is usually a structure */ 
	...
	if( functor == listFunctor )
		return IsList(t)
				&& Unify(XListHead(t), args[0])
				&& Unify(XListTail(t), args[1]) ;
		...
		int i ;
		dotimes(i, FunctorArity(functor))
			if( !Unify(XStructArg(t, i), *args++) )
				return false ;
		return true ;
	}	
	else
...
Bool UnifyVar(register Pt t1, register Pt t2)
{		/* t is usually a variable */
	VarValue(t1) ;
	if( !IsVar(t1) )
		return false ;
