...
{
	if( StreamKind(srm) == textStdStream ) {
		StreamKind(srm) = textStdStreamInteractive ;
		StreamEofAction(srm) = eofReset ;
	}
...
static void SetStdStreamsIVars()
{
	...
	IVarForceSet(LookupAtom("user_prompt"), TagExtra(streamType, userPrt), true) ;
}
...
static CharPt StreamKindStr(StreamPt srm)
{
	switch( StreamKind(srm) ) {
		...
		case textStdStream: return "std" ;
		case textStdPromptStream: return "std_prompt" ;
		case textStdStreamInteractive: return "std_interactive" ;
...
StreamPt FileStreamOpen(CharPt name, StreamMode mode, OpenStreamOptionsPt s)
{
	...
	StreamIsATty(srm) = OSIsATty(FileFD(f)) ;
	return srm ;
}
														...
														Bool interactive)
{
	...
	StreamIsATty(srm) = OSIsATty(FileFD(f)) ;
	if( interactive ) {
		if( m == mRead ) SetInteractive(srm) ;
		else InternalError("FILEToStream") ;
	}
	return srm ;
}
...
WChar StreamGet(StreamPt srm)
{
	...
	switch( StreamKind(srm) ) {
		...
		case textStdStreamInteractive: {
			MyUseLocale(StreamLocale(srm)) ;
			c = FileGetCharInteractive(StreamChannel(srm)) ;
			if( c == EOF ) CheckDoubleEOF(srm, true) ;
			return c ;
		}
		case binaryFileStream: {
...
WChar StreamPeek(StreamPt srm)
{
	...
	switch( StreamKind(srm) ) {
		...
		case textStdStreamInteractive: {
			MyUseLocale(StreamLocale(srm)) ;
			c = FilePeekCharInteractive(StreamChannel(srm)) ;
			if( c == EOF ) CheckDoubleEOF(srm, false) ;
			return c ;
		}
		case binaryFileStream: {
...
{
	if( SetRawInput(FileFILE(StreamChannel(userIn))) ) {
		WChar c = StreamGet(userIn) ;
		UnsetRawInput() ;
		return c ;
	}
	else {
...
Bool StreamAtEnd(StreamPt srm)
{
	...
	switch( StreamKind(srm) ) {
		...
		case textStdStreamInteractive: {
			MyUseLocale(StreamLocale(srm)) ;
			return FilePeekCharInteractive(StreamChannel(srm)) == EOF ;
		}
		case binaryFileStream: {
...
/* 7.84 - 9.42 - 32.32 */
	StreamPt srm ;
	int i ;
	srm = FileStreamOpen("tttest", mWrite, nil) ;
	dotimes(i, 30000000) {
		StreamPut(srm, 'a') ;
	}
	StreamClose( srm, nil) ;
...
void StreamsInit()
{
	...
	currIn = userIn = origIn =
		FILEToStream(stdin, "user_input", mRead, textStdStream, true) ;
	currOut = userOut = origOut =
		FILEToStream(stdout, "user_output", mWrite, textStdStream, false) ;
	userErr = origErr =
		FILEToStream(stderr, "user_error", mWrite, textStdStream, false) ;
	userPrt = FILEToStream(stdout, "user_prompt", mWrite, textStdPromptStream, false) ;
