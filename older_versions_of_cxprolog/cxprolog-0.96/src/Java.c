...
static void LibClassLazyCompile(CharPt name)
{
	CharPt cl = FullPath("lib/cxprolog", name, "class") ;
	CharPt j = FullPath("lib/cxprolog", name, "java") ;
	if( !OSExists(cl) && OSExists(j) )
	  	OSRun(GStrFormat("javac \"%s\"", j)) ;
	if( !OSExists(cl) )
		FatalError("Cannot access '%s'", cl) ;
}

static void LibClassesDelete(void)
{
	CharPt cls = FullPath("lib/cxprolog", "*", "class") ;
#if _WIN32
	OSRun(GStrFormat("echo del \"%s\"", cls)) ;
	OSRun(GStrFormat("del %s", cls)) ;
#else
	OSRun(GStrFormat("rm -f %s", cls)) ;
#endif
}
