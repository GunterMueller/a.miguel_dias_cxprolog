...
static StreamPt NetFILEToStream(FILE *file, CharPt p, StreamMode m)
{
	return FILEToStream(file, GStrFormat("%s_%lx", p, cWord(file)), m, m == mRead ? interactiveStream : netStream) ;
}
