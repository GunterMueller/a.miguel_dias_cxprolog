...
/* Allocate(nWords, false) must return aligned addresses,
   ...
   2 - Allocate reserves blocks of even size (this also helps
       in combating fragmentation; is will not be necessary in the
       case of 64-bit machines).
 */
...
#define initialSegmentSize		32 K /* words */
...
/* must be at the end to ensure there is memory to run MemoryGrowWarning itself */
	if( oldCodeAreaSize > 0 )  /* first growth not shown */
		MemoryGrowWarning("program space", oldCodeAreaSize, codeAreaSize, nil) ;
}
