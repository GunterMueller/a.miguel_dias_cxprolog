...
static Bool StrSimilar(CharPt a, CharPt b, int n, Bool ignoreCase)
{
	if( ignoreCase )
		while( n-- ) {
			if( *a == 0 || *b == 0 ) return *a == *b ;
			if( cx_toupper(*cUCharPt(a)) != cx_toupper(*cUCharPt(b))) /* @#@# */
				return false ;
			a++ ; b++ ;
		}
	else
...
static Bool GetVolByName(CharPt vname, short *r)
{
	...
	for( i = 1 ; ; i++ ) {
		...
		if( StrSimilar(vname, cCharPt(buff+1), 9999, true) )
			return true ;
	}
	return false ;
}
...
{
	return fd <= STDERR_FILENO ; /* @@ Not tested yet */
}
...
CharPt OSGetEnv(CharPt var, Bool err)
{
	CharPt res = getenv(envVarName) ;
	if( res == nil && err )
