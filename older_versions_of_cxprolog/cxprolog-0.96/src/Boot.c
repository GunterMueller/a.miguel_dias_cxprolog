...
'$$_handle_arg'(quiet) :-						\
	os_arg(['-q','--quiet'],'')					\
	 -> set_prolog_flag(memory_warnings,false), \
		set_prolog_flag(info_messages,false).	\
'$$_handle_arg'(boot) :-						\
	os_arg('--boot',X)							\
			-> silent_consult(X)				\
			; '$$_default_boot'.				\
...
static CharPt defaultBuiltinsStr = "			\
												\
'$cxprolog_initialise':-						\
	try (current_prolog_flag(info_messages,true)\
								-> version),	\
	'$env_context' := [main].					\
												\
...
CharPt FullPath(CharPt path, CharPt name, CharPt extension)
{
	CharPt prefix ;
#ifdef PREFIX
	prefix = PREFIX ;
...
#endif	
	return GStrFormat("%s%s%s/%s.%s", prefix,
...
static void PShow()
{
    ...
    Write(" floats.     foreigns. ivars.    locale.       ops.   queues.\n") ;
...
{
	CharPt libName = FullPath("lib/cxprolog", "lib", "pl") ;	
	if( !OSExists(libName) )
