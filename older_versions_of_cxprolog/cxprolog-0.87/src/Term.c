...
PInt XTestChar(register Pt t)
{
	VarValue(t) ;
	if( IsInt(t) ) return XChar(t) ;
...
static void PName()
{
	Pt t0 = Drf(X0) ;
	if( IsAtomic(t0) ) {
		CharPt s = IsAtom(t0) ? XAtomName(t0) : XNumberAsStr(t0) ;	
		ZEnsureFreeSpaceOnStacks(2 * strlen(s), "name/2") ; /* stacks may grow */
		if( Unify(X1, StringToPString(s)) ) JumpNext()
		DoFail()
	}
	elif( IsVar(t0) ) {
		Pt t1 = Drf(X1) ;
	...
	else TypeError2("ATOM, NUMBER or VAR", t0) ;
}
...
static void PSlice()
{
	...
	if( i0 >= 0 && i1 >= 0 ) {
		...
		ScratchAddCh('\0') ;
		if( Unify(X3, MakeTempAtom(FreeScratch())) ) JumpNext()
		DoFail()
	}
	elif( i0 < 0 && i1 < 0 ) {
		if( i0 < -i ) i0 = -i ;
...
static void PUnique()
{
	...
	if( Unify(X0, MakeTempAtom(FreeScratch())) ) JumpNext()
	DoFail()
}
