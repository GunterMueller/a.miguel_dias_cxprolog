...
//#	error "We do not support yet WxWidgets along multibyte encodings."
//#endif
...
Pt MakeWxObj(VoidPt obj, Pt bind) /* pre: obj must be an alive wx object or nil */
{
 	if( obj == nil )
		return BindVar(bind, tNullAtom) ;
	
	else {
		register WxObjPt w ;
		UChar slot = cWord(obj) >> 3 ;
		if( (w = cWxObjPt(ExtraFindFirst(wxobjType, slot, WxObjNewAux, obj))) == nil ) {
			w = cWxObjPt(ExtraNew(wxobjType, slot, nil)) ;
			WxObjValue(w) = obj ;
		}
		return BindVar(bind, TagExtra(wxobjType, w)) ;
	}
}

static void CreateNullStuff(void)
{
	WxObjPt nullWxObj = cWxObjPt(ExtraNew(wxobjType, 0, nil)) ;
...
public:
	...
	void OnGuiCall(wxCommandEvent& WXUNUSED(event)) {
								/* Handles the call in the GUI thread */
		RestrictedMode(true) ;	/* Protects against reentrant calls in */
		fun() ;					/*     the GUI thread, but nothing more. */
...
void InstallWxBuiltinPredAux(CharPt n, int a, Fun fINIT, Fun fGUI, Fun fEND)
{
...
void WxBuiltinsInit(void) ;
	...
	InstallWxBuiltinPred("zwx", 0, PZWX) ;

	RegisterForeignInterface(fWxWidgets) ;
}
