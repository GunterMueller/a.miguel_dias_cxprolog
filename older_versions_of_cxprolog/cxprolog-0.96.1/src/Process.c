...
{
	return ExtraFindFirst(processType, 0, IdToProcessAux, cVoidPt(id)) ;
}

static ProcessPt ProcessNew(Pt startGoal, Pt restartGoal, Pt bind)
{
	int child ;
	if( rw[0] == -1 )
		OSPipe(rw) ;
	if( (child = OSFork()) > 0 ) {	/* PARENT */
		ProcessPt p = ExtraNew(processType, 0, bind) ;
		ProcessId(p) = child ;
		ProcessAlive(p) = true ;
		return p ;
	}
...
static Size ProcessSizeFun(VoidPt x)
{
...
{
	CharPt s = OSGetEnv(XTestAtomName(X0), false) ;
