...
static Size ArraySizeFun(VoidPt x)
{
...
ArrayPt ArrayNew(Pt bind)
{
	ArrayPt a = ExtraNew(arrayType, 0, bind) ;
	ArrayInit(a, 4) ;
	return a ;
}
...
{
	ArraySet(XTestExtra(arrayType,X0), XTestPosInt(X1)-1, TestNonVar(X2)) ;
	JumpNext() ;
}
