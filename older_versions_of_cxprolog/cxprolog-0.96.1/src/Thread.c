...
static void ThreadReset(register ThreadPt th)
{
    ...
    ThreadStacksEnd(th)[-1] = endOfChainMark ; /* Fill end-of-chains slot */
    ThreadReg(th,B0) = ThreadReg(th,B) ;
    ThreadReg(th,R) = ThreadReg(th,B) ;
	ThreadReg(th,E) = cEnvironmentPt(ThreadStacksEnd(th)) ; /* E>B for TopOfLocalStack() */
...
static ThreadPt ThreadNew(Pt startGoal, Pt restartGoal, Pt bind)
{
	ThreadPt th = ExtraNew(threadType, 0, bind) ;
...
{
	root = ThreadNew(startGoal, restartGoal, nil) ;
...
static Size ThreadSizeFun(VoidPt x)
{
...
void StatisticsShow()
{
	ShowVersion() ;
	Write("STATISTICS:\n") ;
	AreaStatistics("Program space", CodeAreaSize(), CodeAreaUsed()) ;
	ThreadSave(active) ;	/* trailBegin, stacksBegin0 might have moved */
