...
static jclass systemClass, classClass, stringClass ;
...
static jmethodID classClass_getFields, systemClass_identityHashCode ;
...
static jmethodID cxPrologClass_CallPrologStr ;
...
static Bool JObjNewAux(VoidPt x, VoidPt ref)
{
...
{
	JObjPt nullJObj = ExtraNew(jobjType, 0, nil) ;
	JObjValue(nullJObj) = nil ;
	JObjHash(nullJObj) = 0 ;
	ExtraSetPermanent(nullJObj) ;
...
static jobject XTestJObj(register Pt t)
{
	...
	return TypeError("JOBJ", t) ;
}

static Size JObjSizeFun(VoidPt x)
{
	return WordsOf(JObj) ;
}
...
{
	DeleteFilesWithExtension(PrefixDirPath("lib/cxprolog", nil, nil) , ".class") ;
}

static void CallJavaCompiler(CharPt path)
{
	if( OSExists(path) )
		OSRun(GStrFormat("javac \"%s\"", path)) ;
}

static void LazyCompileLibDirClass(CharPt name)
{
	if( !OSExists(PrefixDirPath("lib/cxprolog", name, "class")) )
		CallJavaCompiler(PrefixDirPath("lib/cxprolog", name, "java")) ;
}

static void LazyCompileCurrDirClass(CharPt name)
{
	if( !OSExists(CurrentDirPath("", name, "class")) )
		CallJavaCompiler(CurrentDirPath("", name, "java")) ;
}
...
/* Java native method NotifyProlog */
static void JNICALL JavaNotifyProlog(JNIEnv *env, jobject self)
{
...
static void RegisterNotifyProlog(jclass cls)
{
	JNINativeMethod nm ;
	nm.name = "NotifyProlog" ;
	nm.signature = "()V" ;
	nm.fnPtr = JavaNotifyProlog ;
	(*env)->RegisterNatives(env, cls, &nm, 1) ;
}

/* Java native method CallPrologStr */
static jboolean JNICALL JavaCallPrologStr(JNIEnv *env, jobject self, jstring obj)
{
	if( JIsInstanceOf(obj, stringClass) ) {
		CharPt jstr, str ;
		if( (jstr = cCharPt(JGetStringUTFChars(obj))) == nil )
		...
		str = GStrMake(jstr) ;
		JReleaseStringUTFChars(obj, jstr) ;
		return CallPrologStr(str) ;
	}
	return false ;
}

/* Register native method on the Java class cls */
static void RegisterCallPrologStr(jclass cls)
{
	JNINativeMethod nm ;
	nm.name = "CallPrologStr" ;
	nm.signature = "(Ljava/lang/String;)Z" ;
	nm.fnPtr = JavaCallPrologStr ;
	(*env)->RegisterNatives(env, cls, &nm, 1) ;
}
...
static void JavaStart(void)
{
	...
	strcat(path, PrefixDirPath("lib/cxprolog", nil, nil)) ;
	...
	LazyCompileLibDirClass("CxProlog") ;	/* Ensures this library class is ready */
	cxPrologClass = FindNormalClassChecked("CxProlog") ;
	RegisterNotifyProlog(cxPrologClass) ;
	RegisterCallPrologStr(cxPrologClass) ;
	...
	cxPrologClass_CallPrologStr =
		GetStaticMethodIDChecked(cxPrologClass, "CallPrologStr", "(Ljava/lang/String;)Z") ;
...
static void PCallPrologThroughJava()	/* For testing reentrant calls */
{
	...
	z = JCallMethodA(Boolean, true, cxPrologClass, cxPrologClass_CallPrologStr, args) ;
	CheckException() ;
	MustBe( z ) ;
}
...
'$$_call_prolog_through_java_2'(G) :-	/* 	For testing reentrant calls 2 */	\
    atom_termq(A, G),															\
    java_call('CxProlog', 'CallPrologStr:(Ljava/lang/String;)Z', [A], true).	\
" ;
...
void JavaInit()
{
	...
	ZBasicLoadStr(moreJavaBuiltinsStr) ;

	RegisterForeignInterface(fJava) ;
}
