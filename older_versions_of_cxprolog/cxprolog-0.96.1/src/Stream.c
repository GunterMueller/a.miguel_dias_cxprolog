...
static StreamPt StreamNew(VoidPt chn, CharPt n, StreamMode m, StreamKind k, Pt bind)
{
	StreamPt srm = ExtraNew(streamType, 0, bind) ;
	StreamKind(srm) = k ;
	StreamMode(srm) = m ;
	...
	StreamPath(srm) =
		origErr == nil || !IsFileStream(srm)
				? StreamAtom(srm)
				: AbsoluteFileName(AtomName(StreamAtom(srm))) ;
...
{
	IVarForceSet(LookupAtom("user_input"), TagExtra(streamType,userIn), true) ;
	IVarForceSet(LookupAtom("user_output"), TagExtra(streamType, userOut), true) ;
	IVarForceSet(LookupAtom("user_error"), TagExtra(streamType, userErr), true) ;
}
...
static StreamPt FileEdinburghStreamOpen(CharPt name, StreamMode mode)
{
	...
	if( (srm = FindEdinburghStream(name, mode)) != nil )
		return srm ;
	srm = FileStreamOpen(name, mode, nil, nil)  ;
...
StreamPt FileStreamOpen(CharPt name, StreamMode mode, OpenStreamOptionsPt s, Pt bind)
{
...
StreamPt BufferStreamOpen(BufferPt buff, StreamMode mode, OpenStreamOptionsPt s, Pt bind)
{
	OpenStreamOptions soDef ;
	OpenStreamOptionsPt so =
				s != nil ? s : StreamPropertyGetDefaultOpenOptions(&soDef) ;
...
StreamPt NullStreamOpen(Pt bind)
{
	static int n = 0 ;
	CharPt name = GStrFormat("_%%null_%d", n++) ;
	StreamPt srm = StreamNew(nil, name, mWrite, nullStream, bind) ;
	return srm ;
}
...
StreamPt StringStreamOpen(CharPt string)
{
	...
	if( srm == nil ) { /* There is only one string stream */
		srm = StreamNew(nil, "_%string", mRead, stringStream, nil) ;	
		ExtraSetHidden(srm) ;
...
StreamPt ListStreamOpen(Pt list)
{
	...
	if( srm == nil ) { /* There is only one  stream */
		srm = StreamNew(nil, "_%list", mRead, listStream, nil) ;
		ExtraSetHidden(srm) ;
		ExtraSetPermanent(srm) ;
	}
...
StreamPt InnerStreamOpen()
{
	...
	if( srm == nil ) { /* There is only one inner stream */
		srm = StreamNew(nil, "_%inner", mWrite, innerStream, nil) ;
		ExtraSetHidden(srm) ;
		ExtraSetPermanent(srm) ;
	}
	BigStrOpen() ;
...
StreamPt FILEToStream(FILE *file, CharPt n, StreamMode m, StreamKind k, Pt bind)
{
	FilePt f = FileNew(file, n, m) ;
	StreamPt srm = StreamNew(f, n, m, k, bind) ;
	StreamIsATty(srm) = FileIsATty(f) ;
	return srm ;
}
...
CharPt StreamClose(StreamPt srm, CloseStreamOptionsPt co)
{
	...
	if( !ExtraIsAlive(srm) ) return nil ;
...
{
	FileStreamOpen(XTestFileName(X0), XTestStreamMode(X1), nil, X2) ;
	JumpNext() ;
}
...
static void POpenFileStream4()
{
	...
	FileStreamOpen(XTestFileName(X0), XTestStreamMode(X1),
						StreamPropertyGetOpenOptions(X2, &so), X3) ;
	JumpNext() ;
}
#else
static void POpenFileStream4()
{
	OpenStreamOptions so ;
	FileStreamOpen(XTestFileName(X0), XTestStreamMode(X1),
						StreamPropertyGetOpenOptions(X3, &so), X2) ;
	JumpNext() ;
}
#endif
...
{
	BufferStreamOpen(XTestBuffer(X0), XTestStreamMode(X1), nil, X2) ;
	JumpNext() ;
}
...
{
	OpenStreamOptions so ;
	BufferStreamOpen(XTestBuffer(X0), XTestStreamMode(X1),
								StreamPropertyGetOpenOptions(X2, &so), X3) ;
	JumpNext() ;
}
#else
...
static Size PStreamsAux(VoidPt x)
{
	...
	Write("name('%s'), kind(%s), type(%s), mode(%s), ",
		...
		StreamModeStr(srm)) ;
	Write("\n%10s", "") ;	
	Write("encoding(%s), reposition(%s), eof_action(%s), tty(%s)",
		StreamLocale(srm),
...
static void PSee()
{
	...
	VarValue2(t, X0) ;
	if( IsAtom(t) )
		currIn = FileEdinburghStreamOpen(XTestFileName(t), mRead) ;
...
static void PTell()
{
	...
	VarValue2(t, X0) ;
	if( IsAtom(t) )
		currOut = FileEdinburghStreamOpen(XTestFileName(t), mWrite) ;
	elif( IsThisExtra(streamType, t) )
...
/* 2.37 - 3.59 - 9.57 - 172 */
	...
	int i ;
	srm = FileStreamOpen("/tmp/tttest", mWrite, nil, nil) ;
	dotimes(i, 30000000) {
		StreamPut(srm, 'a') ;
	}
	StreamClose( srm, nil) ;
...
StreamPt XTestStream(Pt t, StreamMode mode)
{
	if( Drf(t) == tUserAtom )
			switch( mode ) {
				case mNone: return userIn ; /* user == user_input by default */
				case mRead: return userIn ;
				case mWrite: return userOut ;
				case mAppend: return userOut ;
			}
	return UsingStream(XTestExtra(streamType, t), mode) ;
}
...
void StreamsInit()
{
	...
	currIn = userIn = origIn = FILEToStream(stdin, "user_input", mRead, inKind, nil) ;
	StreamEofAction(userIn) = eofReset ;
	
	currOut = userOut = origOut = FILEToStream(stdout, "user_output", mWrite, textFileStream, nil) ;
	userErr = origErr = FILEToStream(stderr, "user_error", mWrite, textFileStream, nil) ;
