...
{
	StreamPt srm = FileStreamOpen(fileName, mRead, nil, nil) ;
...
void ConsultInit()
{
	consultStack = StackNew(nil) ;
	ExtraSetHidden(consultStack) ;
	ExtraSetPermanent(consultStack) ;
	initializeQueue = QueueNew(nil) ;
	ExtraSetHidden(initializeQueue) ;
	ExtraSetPermanent(initializeQueue) ;
	ConsultRestart() ;
