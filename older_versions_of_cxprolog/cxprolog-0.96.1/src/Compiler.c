...
/* The compiler generates special code when cuts (!/0) are used
	...
	SetMetaPredicate("!", 0) ;
	SetMetaPredicate("call", 1) ;		SetMetaPredicate("call", 2) ;
...
/* Unsuported, as now: must check before announce support
	SetMetaPredicate("catch", 3) ;		SetMetaPredicate("catch", 4) ;
	SetMetaPredicate("on_exception", 3) ;
*/
	...
	SetCorePredicate("restart", 0) ;	SetCorePredicate("abort", 0) ;
...
static void CompileGoal(Pt goal)
{
	if( IsAtom(goal) ) {
		...
		if( IsUnitParam(goal) ) {
			Gen2(PutZValue, OutParam(XUnitParam(goal)), cC99Fix(OutTemp(0))) ;
			CompileCallVar() ;
		}
		else {
			ScratchSave() ;
