...
#define ExtraAlias(x)				(cAtomPt(x)[-2])
#define ExtraNextInAlias(x)			(cExtraPt(x)[-1])
...
#define ExtraIsAliased(x)			(cExtraPt(x)->aliased)
...
VoidPt ExtraNewWithSize(ExtraTypePt e, Size size, int slot, Pt bind)
{
	ExtraPt x, *z = (ExtraPt *)(&ExtraTypeHashTable(e)[slot]) ;
	Pt t = Drf(bind) ;
/* Allocates at even word boundary, allowing 3 bits for the extra-tag */
	if( IsAtom(t) ) {
		x = cExtraPt(cHdl(Allocate(size += 2, false)) + 2) ;
		ExtraIsAliased(x) = true ;
	}
	else {
		x = Allocate(size, false) ;
		ExtraIsAliased(x) = false ;
	}
...
/* Return */
	return BindVarWithExtra(t, x) ;
}

VoidPt ExtraNew(ExtraTypePt e, int slot, Pt bind)
{
	return ExtraNewWithSize(e, ExtraTypeSizeFun(e)(nil), slot, bind) ;
}
...
static void ExtraRelease(ExtraTypePt e, VoidPt x)
{
	...
	if( ExtraIsAliased(x) )
		Release(cExtraPt(cHdl(x) - 2), ExtraTypeSizeFun(e)(x) + 2) ;
	else
		Release(cExtraPt(x), ExtraTypeSizeFun(e)(x)) ;
}
...
VoidPt BindVarWithExtra(Pt t, VoidPt x)
{
