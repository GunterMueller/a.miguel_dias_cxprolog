...
{
	CharPt applDirPath ;
	if( (applDirPath = OSPrefixDir()) == nil )
		return Error("Cannot discover the application directory path") ;	
	return WithSomethingDirPath(applDirPath, path, name, extension) ;
}
...
static void PZPl(void)
{
	...
	/*
	path = WithPrefixDirPath("share/cxprolog/pl", nil, nil) ;
	if( OSExists(path) ) {
		SetCurrDir(LookupTempAtom(path), path) ;
		JumpNext() ;
	} */
...
static void PZEx(void)
{
	...
	/*
	path = WithPrefixDirPath("share/cxprolog/examples", nil, nil) ;
	if( OSExists(path) ) {
		SetCurrDir(LookupTempAtom(path), path) ;
		JumpNext() ;
	} */
