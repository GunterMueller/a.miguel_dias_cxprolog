...
static CharPt defaultBuiltinsStr = "			\
												\
'$cxprolog_initialise':-						\
	try (current_prolog_flag(info_messages, 0)	\
			-> true ; version),					\
	'$env_context' := [main].					\
												\
...
#if USE_JAVA
	Write("   - Java suppport: yes\n") ;
#else
	Write("   - Java suppport: no\n") ;
#endif

#if USE_WXWIDGETS
	Write("   - wxWindows suppport: yes\n") ;
#else
	Write("   - wxWindows suppport: no\n") ;
#endif

#if USE_READLINE
	Write("   - GNU Readline suppport: yes\n") ;
#else
	Write("   - GNU Readline suppport: no\n") ;
#endif
