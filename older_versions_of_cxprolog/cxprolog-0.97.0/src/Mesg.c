...
{
	if( infoMessages_flag >= 0 || SysTraceFlag() > 0 ) {
		va_list v ;	
		va_start(v, fmt) ;
		WriteErr("%% ") ;
...
{
	if( infoMessages_flag >= level || SysTraceFlag() > 0 ) {
		va_list v ;	
		va_start(v, fmt) ;
...
{
	InterruptHandle() ;
	if( memoryWarnings_flag || infoMessages_flag >= 2 || SysTraceFlag() > 0 ) {
		va_list v ;	
		va_start(v, fmt) ;
...
void MemoryGrowInfo(CharPt what, Size oldSize, Size newSize, CharPt where)
{
	if( memoryWarnings_flag || infoMessages_flag >= 2 || SysTraceFlag() > 0 ) {
		...
		if( !Running() )
			...
			MemoryInfo("Expanding %s from %s to %s  [%s]", what, o, n, where) ;
		}
	}
}
