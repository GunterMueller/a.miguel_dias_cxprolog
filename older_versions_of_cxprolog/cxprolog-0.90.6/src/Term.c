...
CharPt VarName(Pt t)
{
	if( IsVar(t) ) {
		if( IsLocalRef(t) )
			return GStrFormat("L%d", Df(stacksEnd, t) ) ;
		elif( IsGlobalRef(t) )
			return GStrFormat("_%d", Df(t, stacksBegin) ) ;
		elif( Eq(t,freeVar) )
...
static Pt GetFreeVars(register Pt t, Size *nVars)
{
	...
	TrailSave() ;	/* trick: trail is instrumental in not repeating vars */
	UseScratch() ;
	for(;;) {
		VarValue(t) ;
		if( IsVar(t) ) {
...
Int64 XTestInt64(register Pt t)  /* used in the java interface */
{
	VarValue(t) ;
	if( IsNumber(t) ) return XAsInt64(t) ;
...
static Bool AtomOrNumber_CodesOrChars(Bool any, Bool atom, Bool codes)
{
	...
	if( IsAtom(t0) && (any || atom) ) {
		...
		ZEnsureFreeSpaceOnStacks(2 * CharLen(s), CurrCPredNameArity()) ; /* stacks may grow */
...
void TermsInit()
{
	termSegm = PermAllocate(termSegmSize) ; /* must have alloc addr */
	freeVar = PermAllocate(1) ; /* must have alloc addr too */
	ResetVar(freeVar) ;
