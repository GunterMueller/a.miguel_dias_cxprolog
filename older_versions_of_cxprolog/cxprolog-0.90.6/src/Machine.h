...
#define IsGlobalRecord(t)	( IsRecord(t) && IsGlobalRef(XPt(t)) )
#define IsAllocRecord(t)	( IsRecord(t) && !IsStacksRef(XPt(t)) )
...
#define Jump(v)			{ (*cVFun(v))() ; JumpNext() }

#define Ensure(c)		{ if( !(c) ) DoFail() }
#define MustBe(c)		{ if( c ) JumpNext() else DoFail() }
