...
/* The Scratch Pad is a single block of memory, but it is internally
   ...
   Scratch Pad is grows automatically when needed. */

extern Hdl scratchBegin, scratchEnd, scratchPt, scratchActive ;
...
#define ScratchCheck()			((scratchPt>=scratchEnd) ? ScratchExpand() : 0)

int ScratchExpand(void) ;
void ScratchMakeRoom(Size nWords) ;
Size ScratchSize(void) ;
