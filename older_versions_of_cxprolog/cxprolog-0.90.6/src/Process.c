...
static Size ProcessDeleteAllAux(VoidPt x)
{
...
{
	ForEachExtra(processType, ProcessDeleteAllAux) ;
}
...
static void PProcessReceive()
{
	...
	MustBe( Unify(X0, TagExtra(p)) && Unify(X1, MakeInt(mesg.info)) )
}
...
static Size ProcessesCleanDeadAux(VoidPt x)
{
...
{
	ForEachExtra(processType, ProcessesCleanDeadAux) ;
}

static void PNDCurrentProcess()
{
	PNDCurrentExtra(processType, nil, 1) ;
	JumpNext()
}

static Size ProcessesAux(VoidPt x)
{
	ProcessPt p = cProcessPt(x) ;
...
static void PProcesses()
{
	ShowVersion() ;
	Write("Processes:\n") ;
	ForEachExtra(processType, ProcessesAux) ;
	JumpNext()
}
...
{
	CharPt s = OSGetEnv(XTestAtomName(X0)) ;
...
void ProcessesInit()
{
	processType = ExtraTypeNew("PROCESS", WordsOf(Process), nil) ;
...
static Size ProcessesFinishAux(VoidPt x)
{
	ProcessPt p =  cProcessPt(x) ;
	ExtraDelete(processType, p) ;
	OSKill(ProcessId(p)) ;
...
void ProcessesFinish()
{
	if( ProcessIsChild() )
		...
		ForEachExtra(processType, ProcessesFinishAux) ;
}
