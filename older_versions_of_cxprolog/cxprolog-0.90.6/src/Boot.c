...
/* The following builtin predicates should notult
be redefined, not even inside
...
/* ACTIVATION */							  ""\
												\
'$$_call_prolog'(X) :-							\
	once X, '$$_set_success', fail.				\
'$$_call_prolog'(_) :- '$$_return'.				\
												\
'$$_run_thread'(X) :-							\
	once X, '$$_active_thread_set_success', fail. \
...
'$$_lux1' :-									\
	...
	restart.									\
'$$_stress_testing' :-							\
	flag(test_relocation,_,off),				\
    flag(test_garbage_collection,_,off),		\
 	(flag(test_relocation,on)					\
...
/* CORE PREDICATES	*/						  ""\
												\
/* The following predicates run interpreted		\
   when lauched by call/1 */ 					\
												\
call(X) :- X.									\
...
/* The following builtin predicates may be redefined inside
   an alternative boot file */
...
silent_consult(File0) :-						\
	...
	seeing(Cslt),								\
	with_ivar('$consulting',Cslt,'$$_c_loop'),	\
	'$$_check_curr_unit_imports',				\
	seen, see(SaveIn).						  ""\
												\
consulting(Cslt) :- '$consulting' =: Cslt.		\
												\
...
static Bool ZBasicLoadStrMac(CharPt str)
{
    if( strlen(coreBuiltinsStr) < 2 K ) {
        ...
        else InternalError("ZBasicLoadStrMac") ;
        return true ;
    }
    else return false ;
}
...
void ZBasicLoadFile(CharPt fileName)
{
    ...
    See(fileName) ;
    HSave() ;
    while( (t = ZReadTerm(currIn)) != eofPt ) {
        if( t == nil )
...
void ZBasicLoadStr(CharPt str)
{
	...
	input = StringStreamOpen(str) ;
	HSave() ;
    while( (t = ZReadTerm(input)) != eofPt ) {
        if( t == nil )
            FatalError("Installing special builtin predicate") ;
        else CompileClause(t, true) ;
        HRestore() ;
...
static void PShow()
{
	ShowVersion() ;
	Write("Show what?\n") ;
	Write(" arrays.  atoms.   builtins.    dicts.   flags.    floats.   ivars.  locale.\n") ;
	Write(" ops.     queues.  statistics.  stacks.  streams.  threads.  units.  version.\n") ;
	JumpNext()
}
...
static void POSDependencies()
{
	...
	Write("   Machine = 32 BITS") ;
#else
	Write("   Machine = 64 BITS") ;
#endif
#if BACKWARDS
	Write(", BACKWARDS\n") ;
#else
	...
	Write("   Memory allocation policy = %s\n",
				UsingMMap() ? "MMAP" : "MALLOC (BAD: No MMAP available!)") ;
