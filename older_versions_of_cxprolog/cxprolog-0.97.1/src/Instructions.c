...
static void FinalizeUntilHere(ChoicePointPt here)
{
	if( here == nil ) {
		...
		while( Lt(F,trailEnd) && Lt(F->cp,here) ) {
			FinalizerPt f = F++ ;
			SetChoicePoint(f->cp) ;
			f->proc(f->arg) ; /* Call finalizer */
		}
...
static void PCallCleanup()
{
	...
	CreateChoicePoint(InstLoc(CallCleanupFail), 0) ;
	SetupFinalizer((FunV)CallPrologV, X1) ;
	ExecuteVarPredOpaque(LookupPredicateForMetaCall(PrepareCall(X0))) ;
	JumpNext() ;
}
...
CharPt GetInstNameSearch(InstPt code)
{
	CharPt s ; 
	int i = 4 ;
	do {
