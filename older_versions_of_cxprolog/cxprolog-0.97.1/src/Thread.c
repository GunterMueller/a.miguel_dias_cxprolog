...
#define initialStacksCapacity	(stacks_debugging ? 16000 K : 16 K)
#define initialTrailCapacity	(trail_debugging ? 128000 : 256)
...
static void ThreadReset(register ThreadPt th)
{
    ...
    ThreadReg(th,B) = cChoicePointPt(ThreadStacksEnd(th)-2) ; /* Near top of stacks */
    ThreadStacksEnd(th)[-1] = endOfChainMark ; /* Fill end-of-chains slot */
    ThreadStacksEnd(th)[-2] = endOfChainMark ; /* Fill end-of-chains slot */
    ThreadReg(th,B0) = ThreadReg(th,B) ;
    ThreadReg(th,R) = ThreadReg(th,B) ;
	ThreadReg(th,E) = cEnvironmentPt(ThreadStacksEnd(th)-1) ; /* B<E for TopOfLocalStack() */
...
static void PMaxMem()
{
	ZEnsureFreeSpaceOnStacks(128 K) ; /* stacks grow now */
	JumpNext() ;
}
