...
static FlagEntry flagTable[] =
{
	...
	{"test_garbage_collection",
							&testGCollection_flag,	'b', 0, TestGCUpdateFlag},
	{""},
	...
	for( f = flagTable ; f->name != nil ; f++ )
		if( f->invisible )
			...
			switch( f->type ) {
				...
				case 'X': {
					if( StrEqual(f->name, "encoding") )
						Write("%-12s | -various-", EncodingName(DefaultEncoding())) ;
					elif( StrEqual(f->name, "encoding_os") )
						Write("%-12s | -various-", EncodingName(SystemEncoding())) ;
...
static void SetDebuggingFlag(CharPt f)
{
	...
	elif( StrEqual(f, "systrace1") ) { sysTrace_flag = 1 ; sysTrace_debugging = 1 ; }
	elif( StrEqual(f, "systrace2") ) { sysTrace_flag = 2 ; sysTrace_debugging = 1 ; }
	elif( StrEqual(f, "systrace3") ) { sysTrace_flag = 3 ; sysTrace_debugging = 1 ; }
	elif( StrEqual(f, "systrace") )  { sysTrace_flag = 3 ; sysTrace_debugging = 1 ; }
	elif( StrEqual(f, "reloc") )     { testRelocation_flag = 1 ; testRelocation_debugging = 1 ; }
