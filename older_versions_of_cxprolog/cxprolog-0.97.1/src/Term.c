...
CharPt XTestAtomNameOrPString(Pt t)
{
	...
	if( IsAtom(t) )
		return XAtomName(t) ;
	return TypeError("ATOM or PROLOG STRINGL", t) ;
}
...
Bool3 XTestBool3(register Pt t)
{
	...
	if( t == tAbsentAtom ) return absent3 ;
...
static void PTermVariables()
{
	Size nVars ;
	TermVars(X0, &nVars) ;
	ZEnsureFreeSpaceOnStacks(2 * nVars) ; /* stacks may grow */
...
static Bool AtomOrNumber_CodesOrChars(Bool any, Bool atom, Bool codes)
{
	...
	if( IsAtom(t0) && (any || atom) ) {
		...
		ZEnsureFreeSpaceOnStacks(2 * CharLen(s)) ; /* stacks may grow */
