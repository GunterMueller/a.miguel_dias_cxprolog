...
{
	if( infoMessages_flag >= 0 || sysTrace_flag > 0 || mesg_debugging ) {
		va_list v ;	
		va_start(v, fmt) ;
		WriteErr("%% ") ;
...
{
	if( infoMessages_flag >= level || sysTrace_flag > 0 || mesg_debugging ) {
		va_list v ;	
		va_start(v, fmt) ;
...
{
	InterruptHandle() ;
	if( memoryWarnings_flag || infoMessages_flag >= 2 || sysTrace_flag > 0 || mesg_debugging ) {
		va_list v ;	
		va_start(v, fmt) ;
