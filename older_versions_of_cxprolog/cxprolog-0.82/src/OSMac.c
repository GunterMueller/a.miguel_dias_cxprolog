...
static Bool VolRename(CharPt oname, CharPt nname)
{
	...
	short ref ;
	Str255 buff ;
	if( not GetVolByName(oname, &ref) ) return false ;
...
Pt OSPropType(CharPt fname)
{
	...
	if( atVirtualRoot )
		...
		if( not GetFileByName(fname, vRefNum, dirID, &isDir) ) return nil ;
	...
	if( atVirtualRoot )
		...
		for(;;) {
			if( (t = GetDirById(vRefNum, dID, &dParID)) == nil )
				return nil ;
			h[-1] = MakeBinStruct(listFunctor, t, tNilAtom) ;
			h = H ;
...
Bool OSSetCurrDir(Pt t)
{
	...
	if( not GetVolByName(XTestAtomName(h[--len]), &ref) )
		return false ;
	dID = 0 ;
	while( len-- ) {
		if( not GetDirByName(XTestAtomName(h[len]), ref,
...
void OSServerInstall(int port)
{
	Error("CxProlog server not supported on the Classic Mac.") ;
}
	
void OSServerWaitConnection(FILE **r, FILE **w)
{
	Error("CxProlog server not supported on the Classic Mac.") ;
}

void OSServerUninstall()
{
	Error("CxProlog server not supported on the Classic Mac.") ;
}
