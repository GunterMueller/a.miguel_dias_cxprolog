...
StreamPt origIn, origOut, origErr ;
...
void WriteErr(CharPt fmt, ...)
{
	...
	va_start(p, fmt) ;
	StreamFlush(userOut) ;
	vfprintf(StreamFile(userErr), fmt, p) ;
}
...
static void PSetUserStreams()
{
	...
	if( currIn == userIn ) currIn = userIn = s0 ;
	else userIn = s0 ;

	if( currOut == userOut ) currOut = userOut = s1 ;
	else userOut = s1 ;

	if( currOut == userErr ) currOut = userErr = s2 ;
	else userErr = s2 ;

	DefineStdStreamsIVars() ;
...
static void PStreamsAux(VoidPt x)
{
	...
	Write(" %s %14.14s -> %5s, %6s, %s%s",
			...
			StreamName(srm),
			StreamTypeAsStr(StreamType(srm)),
...
void DefineStdStreamsIVars()
{
	IVarConstForceSet(XAtom(tUserAtom), TagExtra(userIn)) ;
	IVarConstForceSet(LookupAtom("user_input"), TagExtra(userIn)) ;
	IVarConstForceSet(LookupAtom("user_output"), TagExtra(userOut)) ;
	IVarConstForceSet(LookupAtom("user_error"), TagExtra(userErr)) ;
}
