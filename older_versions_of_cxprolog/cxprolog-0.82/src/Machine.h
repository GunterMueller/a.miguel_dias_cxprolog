...
/* ZEnsureFreeSpaceOnStacks só pode ser usado em Pfunctions e Zfunctions */
#define ZEnsureFreeSpaceOnStacks(n, where)						\
				( FreeSpace(memReserve*2) < (n)			\
				  && ZStacksExpand(n - FreeSpace(memReserve*2), where) )
...
void MachineStateAtomGCMark(void) ;
Bool TestStacksUpdateFlags(int newValue) ;
Bool TestGCUpdateFlags(int newValue) ;
void InternalTesting(void) ;
