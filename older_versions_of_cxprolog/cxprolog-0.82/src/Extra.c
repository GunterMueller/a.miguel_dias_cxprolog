...
static ExtraType allExtraTypes[10] ;
...
{
	register ExtraPt x, n ;
	dolist(x, ExtraTypeFirst(e), n) {
		n = ExtraNext(x) ;	/* This allows proc to be ExtraDelete */
		proc(x) ;
	}
}
...
{
	register ExtraPt x ;
	dolist(x, ExtraTypeFirst(e), ExtraNext(x))
		if( cond(x, arg) )
			return x ;
	return nil ;
}
...
/* Get type decriptor from name */
	dotimes(i, nExtraTypes)
		if( strcmp(allExtraTypes[i].name, name) == 0 )
			e = allExtraTypes + i ;

/* Final tests */
	if( e == nil || not ExtraIsLinked(e, ref) )
		return nil ;
...
void BindVarWithExtra(register Pt t, VoidPt ref)
{
	VarValue(t) ;
	if( IsVar(t) ) {
		if( not UnifyWithAtomic(t, TagExtra(ref)) )
