...
static void MachineStateRelocForTrail(Size trailOffset,
...
/* Allocate the new trail and relocate the machine state  */
	newTrailBegin = Allocate(newTrailSize) ;
	newTrailEnd = newTrailBegin + newTrailSize ;
	MachineStateRelocForTrail(newTrailBegin - trailBegin,
...
static void TrailRelocForStacks(Size globalOffset, Size localOffset)
{
	register Hdl h ;
...
/* Relocate the trailed vars */
	for( h = trailBegin ; h < TR ; h++ ) {
		...
		else InternalError("TrailRelocForStacks") ;
	}
...
static void XRegsRelocForStacks(Size globalOffset, Size localOffset)
{
	register int i ;
	dotimes(i, maxX) {
		if( IsVar(Xc(i)) ) {
		...
		else InternalError("XRegsRelocForStacks") ;
	}
}
...
static void OtherRegsRelocForStacks(Size globalOffset, Size localOffset)
{
...
static void LocalStackRelocForStacks(Size globalOffset, Size localOffset)
{
	...
	if( localOffset >= 0 )
		for( a = stacksEnd-1, z = a + localOffset ; a >= lTop ; a--, z-- )
		{
			...
			else InternalError("LocalStackRelocForStacks (>=0)") ;
		}
	else
...
static void GlobalStackRelocForStacks(Size globalOffset, Size localOffset) {
	register Hdl a, z ;
	...
	if( globalOffset >= 0 )
		for( a = H - 1, z = a + globalOffset ; a >= stacksBegin ; a--, z-- )
		{
			if( IsVar(*a) ) {
				if( *a == nil )
					InternalError("GlobalStackRelocForStacks (Null at GS)") ;
				elif( IsLocalRef(*a) )
					/* Sole instance of global stack pointing to local stack */
					if( a[-1] == cPt(metaCutFunctor) )
						*z = *a + localOffset ;
					else
					 InternalError("GlobalStackRelocForStacks (Local var at GS)") ;
				elif( IsGlobalRef(*a) ) *z = *a + globalOffset ;
				else *z = *a ;
			}
			elif( IsAtomic(*a) ) *z = *a ;
			elif( IsRecord(*a) ) *z = *a + globalOffset ;
			else InternalError("GlobalStackRelocForStacks (>=0)") ;
		}
	else
		for( a = stacksBegin, z = a + globalOffset ; a < H ; a++, z++ )
		{
			if( IsVar(*a) ) {
				if( *a == nil )
					InternalError("GlobalStackRelocForStacks (Null at GS 2)") ;
				elif( IsLocalRef(*a) )
					/* Sole instance of global stack pointing to local stack */
					if( a[-1] == cPt(metaCutFunctor) )
						*z = *a + localOffset ;
					else
					 InternalError("GlobalStackRelocForStacks (Local var at GS 2)") ;
				elif( IsGlobalRef(*a) ) *z = *a + globalOffset ;
				else *z = *a ;
			}
			elif( IsAtomic(*a) ) *z = *a ;
			elif( IsRecord(*a) ) *z = *a + globalOffset ;
			else InternalError("GlobalStackRelocForStacks (<0 2)") ;
		}
}
...
static void MachineStateRelocForStacks(Size globalOffset, Size localOffset)
{
	LocalStackRelocForStacks(globalOffset, localOffset) ;
	GlobalStackRelocForStacks(globalOffset, localOffset) ;
	TermReadVarTableRelocForStacks(globalOffset, localOffset) ;
	DebuggerRelocForStacks(globalOffset, localOffset) ;
	XRegsRelocForStacks(globalOffset, localOffset) ;
	TrailRelocForStacks(globalOffset, localOffset) ;
	OtherRegsRelocForStacks(globalOffset, localOffset) ;
}
...
Bool ZStacksExpansion(Size ensureThisExtraSpace, CharPt where)
{
	...
	if( stacksBegin != stacksBegin0 ) { /* Only for testRelocation */
		...
		MachineStateRelocForStacks(stacksBegin0 - stacksBegin,
	...
	MachineStateRelocForStacks(newStacksBegin - stacksBegin,
...
Bool ZTestStacksExpansion(CharPt where)
{
	/* Dot('.') ; */
	/* Mesg(where) ; */
	MachineStateRelocForStacks(1, -1) ;
	stacksBegin += 1 ;
	stacksEnd -= 1 ;
	return true ;
}
