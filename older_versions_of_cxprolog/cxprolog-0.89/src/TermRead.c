...
{
	varsBegin = TempAllocate(initialVarsSize) ;
...
static void VarsExpand()
{
	...
	newVarsBegin = Relocate(varsBegin, size * 2) ;
...
static void DoTermReadVarTableRelocForStacks(Size globalOffset, Size localOffset)
{
	register Hdl h ;
...
#define maxTextBuffer	2 K
static Char textBuffer[maxTextBuffer] ;
static CharPt inPt, tkStartPt, inErrPt, inEndPt ;
...
static CharPt PrepareTextBuffer()  /* for read/3 */
{
	register CharPt s ;
	TextBufferAdd('\0') ;
	for( s = textBuffer ; CharType(*s) == _BL ; s++ ) ;
	return s ;
}
...
static CharPt PreviousTokenStart()
{
	return cCharPt(tks[-1]) ;
}
...
#define GetChar0()		(StreamGet(input))
#define PeekChar0()		(StreamPeek(input))
...
static void IssueErrorMesg(CharPt mesg, CharPt here)
{
	register CharPt s ;		
	WriteErr("*** READER ERROR (%s) ***\n", mesg) ;
	for( s = textBuffer ; s < here ; s++ )
		WriteErr("%c", *s) ;		
	WriteErr("#HERE#") ;
	for( s = here ; s < inPt ; s++ )
		WriteErr("%c", *s) ;
...
{
	if( lexErrorMesg != nil )
		IssueErrorMesg(lexErrorMesg, inErrPt - 1) ;
}

static void ParserError(CharPt mesg) /* always called after NextToken() */
{
	IssueErrorMesg(mesg, PreviousTokenStart() - 1) ;
}
...
static void NextCharEOF()
{
	...
	TextBufferAdd(cc) ;
	if( cx_iseof(cc) ) {
		TextBufferBack() ;  /* remove eofMark from the TextBuffer */
...
static CharPt ReadSpacesNoNewLine()
{
	ScratchSave() ;
	do {
		ScratchAddCh(cc) ;
	...
	for(;;) {
		...
		switch( CharType(cc) ) {
			case _BL: {
				...
				if( onlyTokens ) {
					if( cc == '\n' ) goto exitL ;
					GenToken(spacesTk, MakeTempAtom(ReadSpacesNoNewLine())) ;
					if( cc == '\n' ) goto exitL ;
				}
				else
					GenToken(spacesTk, MakeTempAtom(ReadSpaces())) ;
...
afterSpaceL:	if( cc == '(' ) {
            ...
            case _EF: {
				TextBufferBack() ; /* remove eofMark from the text buffer */	
...
void TermReadVarTableRelocForStacks(Size globalOffset, Size localOffset)
{
	DoTermReadVarTableRelocForStacks(globalOffset, localOffset) ;
}
...
static void PSReadTermWithSource()
{
	...
	if( t == eofPt ) t = tEofAtom ;
	MustBe( t != nil && Unify(X1, t)
			&& UnifyWithAtomic(X2, MakeTempAtom(PrepareTextBuffer())) )
}
