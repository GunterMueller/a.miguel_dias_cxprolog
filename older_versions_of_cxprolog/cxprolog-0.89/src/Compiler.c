...
/* The compiler generates special code when cuts (!/0) are used
	...
	FunctorIsMeta(LookupFunctorByName("catch", 3)) = true ;
	FunctorIsMeta(LookupFunctorByName("catch", 4)) = true ;
	FunctorIsMeta(LookupFunctorByName("on_exception", 3)) = true ;
	FunctorIsMeta(LookupFunctorByName("throw", 1)) = true ;
...
#define maxRecs		( retStringSize / sizeof(short) )
static short *recs = (short *)retString ;
static int nRecs ;
