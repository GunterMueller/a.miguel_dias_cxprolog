...
static void DoDebuggerRelocForStacks(Size globalOffset, Size localOffset)
{
	if( DD != nil )
...
Bool DebugCall(PredicatePt pr, Bool visReq)
{
	...
	elif( XInt(DD->type) == 'b' && !PredIsMeta(DD->pred) && PredIsBuiltin(pr) )
		return false ;
	...
	saveB = B ;
	B0 = B = DF2CP(d) ; /* New ChoicePoint covers DebugFrame */
...
void DebugCut()
{
	...
	d->currClause = nil ;
	if( CallLevel() == 1 ) InterDebug(eCut, d) ;
	DD = d ;	/*	This is required by raise_exception/1.
					In the other cases DD == d already	*/
}
...
void DebugRedoCode()
{
	...
	saveTR = d->redoTR ;
	TrailRestore() ;	/* resume the instantiation that was at the exit port */
	Bf(P) = d->redoP ;
	P = &Fail ;			/* Fail */
	InterDebug(eRedo, d) ;
}
...
void DebuggerRelocForStacks(Size globalOffset, Size localOffset)
{
	DoDebuggerRelocForStacks(globalOffset, localOffset) ;
}
