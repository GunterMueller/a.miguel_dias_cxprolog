...
CharPt GetVersionString()
{
	sprintf(retString, "%s version %s", applName, version) ;
	if( strcmp(subVersion, "") != 0 )
		sprintf(retString + strlen(retString), "-%s", subVersion) ;
	if( strcmp(revision, "") != 0 )
		sprintf(retString + strlen(retString), " (revision: %s)", revision) ;
	return retString ;
}
...
{
	MustBe( UnifyWithAtomic(X0, MakeAtom(GetVersionString()))) ;
	JumpNext()
}
