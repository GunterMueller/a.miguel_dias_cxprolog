...
static Pt freeVar = cPt(&freeVar) ; /* free var in code area */
static Pt limA, limB ;
...
{
	VarValue(term) ;
	if( IsRecord(term) && !IsStacksRef(XPt(term)) ) {
		limA = XPt(term) ;
		limB = limA + TempBlockSize(limA) ;
	}
	else {
...
CharPt VarName(Pt t)
{
	if( IsVar(t) ) {
		if( IsLocalRef(t) )
			sprintf(retString, "L%d", Df(stacksEnd, t) ) ;
		elif( IsGlobalRef(t) ) sprintf(retString, "_%d", Df(t, stacksBegin) ) ;
		elif( Eq(t,freeVar) ) sprintf(retString, "M0") ;
		else sprintf(retString, "_0%d", Df(t,limA)) ;
	}
	else InternalError("VarName") ;
	return retString ;
}
...
{
	sprintf(retString, "%s/%d", XStructName(t), XStructArity(t)) ;
	return retString ;
}
...
Pt AllocateTermForAssert(register Pt t)
{
	VarValue(t) ;
	if( IsRecord(t) )
		return CopyTerm(t, TempAllocate(TermSize(t)), nil, true, false) ;
	else
		return t ;
}
...
Pt AllocateTermForAssign(register Pt t)
{
	VarValue(t) ;
	if( IsRecord(t) )
		return CopyTerm(t, TempAllocate(TermSize(t)), nil, false, false) ;
	elif( IsVar(t) )
		return freeVar ;
	else
		return t ;
...
static void PSlice()
{
	...
	if( i0 >= 0 && i1 >= 0 ) {
		...
		ScratchAddCh('\0') ;
		MustBe( Unify(X3, MakeTempAtom(VFreeScratch())) )
	}
	elif( i0 < 0 && i1 < 0 ) {
		if( i0 < -i ) i0 = -i ;
		if( i1 < -i ) i1 = -i ;
...
static void PUnique()
{
	...
	MustBe( Unify(X0, MakeTempAtom(VFreeScratch())) )
}

void TermsInit()
{
	termSegm = Allocate(termSegmSize) ; /* must have alloc addr */
