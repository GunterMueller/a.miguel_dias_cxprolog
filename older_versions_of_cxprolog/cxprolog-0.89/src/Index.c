...
static void IndexTableExpand()
{
	...
	MemoryGrowWarning("index table", oldSize, oldSize * 2, nil) ;
	Release(indexTable) ;
	indexTableSize *= 2 ;
	indexTable = TempAllocate(indexTableSize * WordsOf(IndexElem)) ;
	IndexTableInit() ;
}
...
{
	for( ;
		!(IsVar(ClauseIdxArg(cl,currArg)) || ClauseIdxArg(cl,currArg) == t) ;
			cl = ClauseNext(cl) ) ;
	return cl ;
}
...
void DoIndex(PredicatePt pr)
{
	...
	if( indexParams_flag == 0 || !PredIsIndexable(pr) )
		PredIndexMade(pr) = true ;

/* The first attempt to invalidate the index makes the predicate loose
	the index definitively. But the original index is not deleted
	(it will only if the predicate is abolished later). */  
	if( PredIndexMade(pr) ) {
...
redo:
	...
	if( hasIndex ) {
		...
		if( ScratchUsed() > 1 ) {
			int i ;
			PredIndex(pr) = TempAllocate(ScratchUsed()) ;
			CopyWordsReloc(PredIndex(pr), ScratchBegin(), ScratchUsed()) ;
			dotimes(i, 4) 
				if( InRange(PredStartInstArgs(pr)[i],
							cPt(ScratchBegin()), cPt(ScratchCurr())) )
					PredStartInstArgs(pr)[i] +=
							PredIndex(pr) - ScratchBegin() ;
			}
	}
	else DoNoIndex(pr) ;
