...
int ScratchExpand()
{
	...
	newScratch = Relocate(scratchBegin, scratchSize * 2 + safeScratchPortionSize) ;
...
void ScratchInit()
{
	scratchBegin = TempAllocate(initialScratchSize + safeScratchPortionSize) ;
...
void ScratchAddPString(register Pt l)
{
	...
	for( ; IsList(l) ; l = Drf(XListTail(l)) ) {
		t = Drf(XListHead(l)) ;
		ScratchAddCh(XTestCode(t)) ;
	}
	if( l != tNilAtom )
		TypeError2("PROPERLY-TERMINATED-LIST", nil) ;
}

void ScratchAddAString(register Pt l)
{
	register Pt t ;
	l = Drf(l) ;
	for( ; IsList(l) ; l = Drf(XListTail(l)) ) {
