...
static AtomPt UserInputAtom, UserOutputAtom, UserErrorAtom, UserAtom ;
...
{	/* Sync user streams with the aliases user_input, etc. */
	if( srm != nil ) {
		...
		elif( a == UserOutputAtom ) {
			StreamFlush(userOut) ;
			if( currOut == userOut ) currOut = srm ;
			userOut = srm ;
			InterLineChangedUserStreams() ;
		}
		elif( a == UserErrorAtom ) {
			StreamFlush(userErr) ;
		...
		if( a == UserInputAtom )
			AliasSet(UserInputAtom, origIn) ;
		elif( a == UserOutputAtom )
			AliasSet(UserOutputAtom, origOut) ;
		elif( a == UserErrorAtom )
			AliasSet(UserErrorAtom, origErr) ;
	}
}
...
void StreamPutStr(StreamPt srm, Str s)
{
	switch( StreamKind(srm) ) {
		case textFileStream: {
			FilePutCharStr(StreamChannel(srm), cCharPtz(s)) ;
			break ;
		}
		case binaryFileStream:
		case netStream: {
	...
	while( (c = CharDecode(s)) != '\0' ) {
		if( c == '~' ) {
			...
			switch( c ) {
				...
				case 'a': {
					CharPt str = XTestAtomName(XGetListHead(&list)) ;
					dotimes(i, arg)
				...
				case 's': {
					CharPt s = PStringToString(XGetListHead(&list)) ;
					dotimes(i, arg)
						StreamPutStr(srm, s) ;
					break ;
				}
...
static Size PStreamsAux(CVoidPt x)
{
	...
	Write("\n%10s", "") ;
	if( StreamIsATty(srm) && StreamMode(srm) == mRead ) /* avoids waiting for input */
		Write("at_eof(false)") ;
	else
...
static void PSetUserStreams()
{
	...
	AliasSet(UserInputAtom, s0) ;
	AliasSet(UserOutputAtom, s1) ;
	AliasSet(UserErrorAtom, s2) ;
	JumpNext() ;
}

static void PRestoreUserStreams()
{
	AliasUnset(UserInputAtom, userIn) ;
	AliasUnset(UserOutputAtom, userOut) ;
	AliasUnset(UserErrorAtom, userErr) ;
	JumpNext() ;
}
...
void StreamsInit()
{
	StreamKind userInKind = InteractiveSession() ? interactiveStream : textFileStream ;
	...
	UserInputAtom = LookupAtom("user_input") ;
	UserOutputAtom = LookupAtom("user_output") ;
	UserErrorAtom = LookupAtom("user_error") ;
	UserAtom = LookupAtom("user") ;
	ExtraSpecial(UserInputAtom) ;
	ExtraSpecial(UserOutputAtom) ;
	ExtraSpecial(UserErrorAtom) ;
	ExtraSpecial(UserAtom) ;

	currIn = userIn = origIn =
		FILEToStream(stdin, AtomName(UserInputAtom), mRead, userInKind) ;
	currOut = userOut = origOut =
		FILEToStream(stdout, AtomName(UserOutputAtom), mWrite, textFileStream) ;
	userErr = origErr =
		FILEToStream(stderr, AtomName(UserErrorAtom), mWrite, textFileStream) ;
	...
	AliasSet(UserInputAtom, userIn) ;
	AliasSet(UserOutputAtom, userOut) ;
	AliasSet(UserErrorAtom, userErr) ;
}
