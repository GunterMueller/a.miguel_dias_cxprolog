...
#define PushTrail(v)		{ if( Eq(TR,F) ) TrailExpand() ;		\
 ...
 /* Boundaries must be included because of rellocation */
...
#define CheckFreeSpaceOnStacks(n)									\
				Ignore( FreeHSpace() < (n) && Error("Global stack overflow") )
...
#define ZEnsureFreeSpaceOnStacks(n)									\
				( FreeHSpace2() < cSize(n)							\
				  ? ZStacksExpansion(n - FreeHSpace2())				\
				  : (testRelocation_flag && ZTestStacksExpansion()) )
#endif
