...
static CharPt AnalizeAtom(AtomPt atom, Bool *dirty, Bool *allSymbols)
{
	CharPt name = AtomName(atom), s = name ;
	int first = CharDecode(s) ;
	Bool d = false, a = false ;
	switch( CharType(first) ) {
		case _SO: a = s[0] == '\0' ;                d = quoted && !a ; break ;
		case '|': a = s[0] == '\0' ;                d = quoted && !a ; break ;
		case '[': a = s[0] == ']' && s[1] == '\0' ; d = quoted && !a ; break ;
		case '{': a = s[0] == '}' && s[1] == '\0' ; d = quoted && !a ; break ;
		case _LC: {
			if( quoted )
				while( *s != '\0' ) {
...
static void AddQStr(CharPt s, Bool dirty, Bool opClashL, Bool opClashR)
{
	if( dirty ) {
		...
		if( opClashL && cx_issymbol(CharFirst(s)) ) AddChar(' ') ;
		AddStr(s) ;
...
static void AddAtom(AtomPt atom, Bool opClashL, Bool opClashR)
{
	...
	CharPt name = AnalizeAtom(atom, &dirty, &allSymbols) ;
...
static void WriteStruct(Pt term, int m, Bool opClashL, Bool opClashR)
{
	register FunctorPt func = XStructFunctor(term) ;
	register AtomPt atom = FunctorAtom(func) ;
	int p, lp, rp ;
	Bool port = portray ;
	if( port ) LvrPush(&term) ;
...
static void DoWriteTerm(Pt term, int p, Bool opClashL, Bool opClashR, Bool parAtom)
{
	int q, rq ;
	Bool port = portray ;
	if( port ) LvrPush(&term) ;
...
static CharPt SubtermAsStrMode(Pt subterm, Pt term)
{
...
static CharPt TermAsStrMode(Pt term)
{
	StreamPt s = InnerStreamOpen() ; /* open inner stream afresh */
...
static CharPt TermsAsStrMode(Pt list)
{
	StreamPt s = InnerStreamOpen() ; /* open inner stream afresh */
