...
Pt	tNilAtom, tEmptyAtom, tEofAtom, tCutAtom,
	...
	tUserAtom, tVoidAtom, tNullAtom, tNoResult, tMarkAtom,
...
FunctorPt commaFunctor, semicolonFunctor, arrowFunctor, neckFunctor,
		...
		onceFunctor, tryFunctor, callFunctor,
...
static void PAtomConcat()
{
	...
	{											/* VAR-VAR handle */
		CharPt s2 = XAtomName(A(2)) ;			/* already validated */
...
unify:
	MustBe(	InRange(before, 0, total) && UnifyWithNumber(X1, MakeInt(before))
		...
		&&	Unify(X4, MakeTempAtomSlice(XTestAtomName(X0), before, before+lenght-1)) ) ;
