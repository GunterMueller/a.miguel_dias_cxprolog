...
static void JNICALL Native_Interrupt(JNIEnv *nenv, jobject self)
{
	Unused(nenv) ;
	Unused(self) ;
	ForceInterrupt() ;
...
{	
	...
	//	Mesg("Native_IVarSet nil");
		...
		IVarSetCopy(n, JavaTermUnserialize(arr));
}

static void JNICALL Native_CoroutiningSetAutoOutput(JNIEnv *nenv, jobject self, jstring coroutine, jboolean ao)
{
	Unused(self) ;
	ThreadSetAutoOutput(ThreadByName(GetJavaStringTextEnv(nenv, coroutine)), ao) ;
}

static jboolean JNICALL Native_CoroutiningWaitingInput(JNIEnv *nenv, jobject self, jstring coroutine)
{
	Unused(self) ;
	return ThreadWaitingInput(ThreadByName(GetJavaStringTextEnv(nenv, coroutine))) ;
}

static jboolean JNICALL Native_CoroutiningIsWaitingAtTopLevel(JNIEnv *nenv, jobject self, jstring coroutine)
{
	Unused(self) ;    /* AMD    BUGGY BUGGY Podem ser threads diferentes */
	return InterLineItAtTopLevelRead()
		&& ThreadWaitingInput(ThreadByName(GetJavaStringTextEnv(nenv, coroutine))) ;
}

static void JNICALL Native_CoroutiningInput(JNIEnv *nenv, jobject self, jstring coroutine, jstring text)
{
	Unused(self) ;
	ThreadInput(ThreadByName(GetJavaStringTextEnv(nenv, coroutine)), GetJavaStringTextEnv(nenv, text)) ;
}
...
static jstring JNICALL Native_CoroutiningOutputText(JNIEnv *nenv, jobject self, jstring coroutine)
{
	...
	JNIEnv *saveEnv = env ;
	env = nenv ;	
	Str s = ThreadOutputText(ThreadByName(GetJavaStringTextEnv(nenv, coroutine))) ;
	jstring jstr = NewJString(s) ;
	env = saveEnv ;
	return jstr ;
}

static void JNICALL Native_CoroutiningInterrupt(JNIEnv *nenv, jobject self, jstring coroutine)
{
	Unused(nenv) ;
	Unused(self) ;
	Unused(coroutine) ;
	ThreadRestart(ThreadByName(GetJavaStringTextEnv(nenv, coroutine)));
}

static void JNICALL Native_SetOutputHandler(JNIEnv *nenv, jobject self, jobject handler)
{
	Unused(nenv) ;
	Unused(self) ;
	Unused(handler) ;
...
static void RegisterNativeMethodsOnPrologClass(void)
{
	JNINativeMethod *nm, nativeTable[] = {
		...
		{"coroutiningSetAutoOutput", "(Ljava/lang/String;Z)V", Native_CoroutiningSetAutoOutput},
		{"coroutiningWaitingInput", "(Ljava/lang/String;)Z", Native_CoroutiningWaitingInput},
		{"coroutiningIsWaitingAtTopLevel", "(Ljava/lang/String;)Z", Native_CoroutiningIsWaitingAtTopLevel},
		{"coroutiningInput", "(Ljava/lang/String;Ljava/lang/String;)V", Native_CoroutiningInput},
		{"coroutiningOutputText", "(Ljava/lang/String;)Ljava/lang/String;", Native_CoroutiningOutputText},
		{"coroutiningInterrupt", "(Ljava/lang/String;)V", Native_CoroutiningInterrupt},
		{"coroutiningSetOutputHandler", "(Lprolog/OutputHandler;)V;", Native_SetOutputHandler},
		{nil, nil, nil}
	} ;
...
static void PJObjs(void)
{
	ExtraShow(jobjType, JObjsAux) ;
	JumpNext() ;
}

static void PJavaCall(void)
{
	jobject target ;
	Bool targetIsClass ;
	Str sig ;
...
static void PJavaClean0(void)
{
	DeleteFilesWithExtension(WithCacheDirPath(nil, nil), ".class") ;
	JumpNext() ;
}

static void PJavaClean1(void)
{
