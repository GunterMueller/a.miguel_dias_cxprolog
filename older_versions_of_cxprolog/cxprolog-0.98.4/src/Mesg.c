...
{
	if( infoMessages_flag > 0 || sysTrace_flag > 0 ) {
		va_list v ;
		va_start(v, fmt) ;
		WriteErr("%% ") ;
...
{
	if( infoMessages_flag >= level || (sysTrace_flag > 0 && level != 4) ) {
		va_list v ;
		va_start(v, fmt) ;
...
{
	InterruptHandle() ;
	if( memoryWarnings_flag || infoMessages_flag >= 2 || sysTrace_flag > 0 ) {
		va_list v ;
		va_start(v, fmt) ;
...
void MemoryGrowInfo(Str what, Size oldSize, Size newSize)
{
	if( memoryWarnings_flag || infoMessages_flag >= 2 || sysTrace_flag > 0 ) {
		Str32 o, n ;
		if( oldSize < 1 K )
			sprintf(o, "%"fSize "bytes", WordsAsBytes(oldSize)) ;
		else sprintf(o, "%"fSize "KB", WordsAsKBytes(oldSize)) ;
		if( newSize < 1 K )
			sprintf(n, "%"fSize " bytes", WordsAsBytes(newSize)) ;
		else sprintf(n, "%"fSize "KB", WordsAsKBytes(newSize)) ;
		if( !Running() )
			MemoryInfo("Expanding %s from %s to %s", what, o, n) ;
		else
			MemoryInfo("Expanding %s from %s to %s [@%s]",
									what, o, n, GetInstNameSearch(P-1)) ;
	}
}
