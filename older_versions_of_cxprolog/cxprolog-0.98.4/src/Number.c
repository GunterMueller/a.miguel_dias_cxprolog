...
Str FloatAsStr(Str cfmt, int field, int prec, PFloat f)
{
	if( isnan(f) ) return "nan" ;
		...
		Str fmt =
			field == -1 && prec == -1
				? cfmt
				: GStrFormat("%"fPFloat, field, prec, cfmt) ;
...
void FloatDisplayPrecUpdateFlag(int newValue)
{
	...
	if( floatDisplayPrec_flag > 0 )
		strcpy(floatFormat, GStrFormat("%"fPFloat, 0, floatDisplayPrec_flag, "g")) ;
	else
		strcpy(floatFormat, GStrFormat("%"fPFloat, 0, -floatDisplayPrec_flag, "f")) ;
}
...
{
	if( IsInt(t) )
		return GStrFormat("%"fPInt, XInt(t)) ;
	elif( IsFloat(t) )
