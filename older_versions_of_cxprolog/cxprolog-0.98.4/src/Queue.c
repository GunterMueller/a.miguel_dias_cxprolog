...
static void QueueWrite(StreamPt stm, QueuePt q)
{
	...
	StreamWrite(stm, "     (current size %ld, current capacity %ld)\n",
								QueueSize(q), QueueCapacity(q)) ;
...
void QueueClear(QueuePt q)
{
	...
	for( h = QueueFirst(q) ; h < QueueLast(q) ; h++ )
		ReleaseTerm(*h) ;
	QueueLast(q) = QueueFirst(q) = QueueBegin(q) ;
}
...
static void PQueueCheck(void)
{
	MustBe( XExtraCheck(queueType, X0) ) ;
}

static void PQueueNew(void)
{
	BindVarWithExtra(X0, QueueNew()) ;
	JumpNext() ;
}

static void PQueueClear(void)
{
	QueueClear(XTestExtra(queueType,X0)) ;
	JumpNext() ;
}

static void PQueueDelete(void)
{
...
static void PQueuePut(void)
{
	QueuePut(XTestExtra(queueType,X0), X1) ;
	JumpNext() ;
}

static void PQueueGet(void)
{
	Pt t ;
	QueuePt q = XTestExtra(queueType,X0) ;
