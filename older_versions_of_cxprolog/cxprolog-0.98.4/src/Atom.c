...
#define AtomWordsLen(len)	(WordsOf(Atom) + Words((len) + 10))
...
static Size _n = 0 ;
static Bool LookupTempAtomNAux(CVoidPt x, CVoidPt ref)
{
	return AtomName(x)[_n] == '\0' && StrEqualN(AtomName(x), cCharPt(ref), _n) ;
}
AtomPt LookupTempAtomN(Str name, Size n) /* ascii len */ /* pre: n <= strlen(s) */
{
	int slot = StrHashN(name, n) % atomHashTableCapacity ;	/* hash function */
	register AtomPt a ;
	_n = n ;
	if( (a = ExtraFindFirst(atomType, slot, LookupTempAtomNAux, name)) == nil ) {
		a = ExtraNewWithSize(atomType, AtomWordsLen(n) + 2, slot) ;
		AtomFunctors(a) = nil ;
		AtomToUnit(a) = nil ;
		AtomToAlias(a) = nil ;
		AtomToIVar(a) = nil ;
		AtomToOperator(a) = nil ;
		strncpy(AtomName(a), name, n) ;
		AtomName(a)[n] = '\0' ;
	}
	return a ;
}
...
static void PNDCurrentAtom(void)
{
...
static Pt MakeTempAtomSlice(Str s, Size au, Size bu) /* [a,b[  unicode positions */
{
	CharPt ss = cCharPt(s) ;
	CharPt start = CharPos(ss, au) ;
	CharPt end = CharPos(start, bu-au) ;
	return MakeTempAtomN(start, end - start) ;
}
...
static VoidPt cmp = nil ;

Size CheckFunctor(FunctorPt f)
{
	return cVoidPt(f) == cmp ;
}

Bool IsFunctor(VoidPt pt) {
	cmp = pt ;
	return ForEachFunctor(CheckFunctor) > 0 ;
}
...
static void PAtomConcat()
{
		...
		if( IsAtom(X0) ) {
			...
			if( IsAtom(X1) ) {					/* VAR-ATOM handle */
				...
				MustBe( l1 <= l2
					...
					&& UnifyWithAtomic(X0, MakeTempAtomN(s2, l2 - l1)) ) ;
			}
	...
	{											/* VAR-VAR handle */
		...
		if( *pos == '\0' )
			...
			A(3) = cPt(CharNxt(pos)) ;
		MustBe( UnifyWithAtomic(X0, MakeTempAtomN(s2, pos - s2))
