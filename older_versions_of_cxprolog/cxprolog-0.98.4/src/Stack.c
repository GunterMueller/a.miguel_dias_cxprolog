...
static void PStackCheck(void)
{
	MustBe( XExtraCheck(stackType, X0) ) ;
}

static void PStackNew(void)
{
	BindVarWithExtra(X0, StackNew()) ;
	JumpNext() ;
}

static void PStackClear(void)
{
	StackClear(XTestExtra(stackType,X0)) ;
	JumpNext() ;
}

static void PStackDelete(void)
{
...
static void PStackPush(void)
{
	StackPush(XTestExtra(stackType,X0), X1) ;
	JumpNext() ;
}

static void PStackPop(void)
{
	Pt t ;
	StackPt s = XTestExtra(stackType,X0) ;
	Ensure( StackTop(s, &t) ) ;
