...
#if USE_OWN_MEMORY_MANAGER && defined(M_MMAP_THRESHOLD) && !__clang__
#  define USE_MMAP		1
#  include <sys/mman.h>
#endif
...
Size MemoryLimit(void)
{
	return memLimit;
}

Size MemoryPortion(double portion)
{
	return memLimit * portion ;
}
...
void MemoryInit(void) {
	...
	memLimit = total / sizeof(Word) / 2 ;
#if !defined(__LP64__)
	if( memLimit > 1 K K K )  // max 1 GB on 32-bit systems
		memLimit = 1 K K K ;
#endif
#if DEBUG
