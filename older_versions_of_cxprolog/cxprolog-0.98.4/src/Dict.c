...
static void PDictCheck(void)
{
	MustBe( XExtraCheck(dictType, X0) ) ;
}

static void PDictNew(void)
{
	BindVarWithExtra(X0, DictNew()) ;
	JumpNext() ;
}

static void PDictClear(void)
{
	DictClear(XTestExtra(dictType,X0)) ;
	JumpNext() ;
}

static void PDictDelete(void)
{
...
static void PDictSet(void)
{
	DictSet(XTestExtra(dictType,X0), TestNonVar(X1), X2) ;
	JumpNext() ;
}

static void PDictGet(void)
{
	Pt t ;
...
static void PDictDeleteItem(void)
{
	MustBe( DictDeleteItem(XTestExtra(dictType,X0), TestNonVar(X1)) ) ;
}

static void PDictAsList(void)
{
	DictPt d = XTestExtra(dictType,X0) ;
	DEntryPt h ; Pt t ;
