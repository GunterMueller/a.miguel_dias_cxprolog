...
#define initialStacksCapacity	(stacksDebugging_flag ? MemoryPortion(0.5) : 16 K)
#define initialTrailCapacity	(trailDebugging_flag ? MemoryPortion(0.002) : 256)
...
*/	if( cc == '\n' ) {
		if( ThreadRetries(active) > 0 )
			InterLineGetPrompt(); /* force use of second prompt */
		StreamPutStr(userOut, InterLineGetPrompt()) ;
		StreamFlush(userOut) ; /* in case socket stream */
	}
...
void StatisticsShow()
{
	...
	AreaStatistics("Scratch pads", ScratchCapacity() + ScribblingCapacity()
