...
{	/* deprecated - kept for compatility with Compass */
	BufferEnsureFreeSpace(b, freeSpace);
}
...
{
//	if( BufferLast(b) - BufferBegin(b) > 0 )
//		Mesg("qqq %p %ld", b, BufferLast(b) - BufferBegin(b));
	BigStrOpen();
...
static void PBufferCheck(void)
{
	MustBe( XExtraCheck(bufferType, X0) ) ;
}

static void PBufferNew(void)
{
	BindVarWithExtra(X0, BufferNew()) ;
	JumpNext() ;
}

static void PBufferClear(void)
{
...
static void PBufferDelete(void)
{
	BufferPt b = XTestExtra(bufferType,X0) ;
	BufferDisable(b) ;
	JumpNext() ;
...
static void PBufferGet(void)
{
	BufferPt b = XTestExtra(bufferType,X0) ;
	MustBe( Unify(X2, MakeByte(BufferGet(b, XTestPosInt(X1)-1))) ) ;
}

static void PBufferAsAtom(void)
{
	BufferPt b = XTestExtra(bufferType,X0) ;
	BufferEnsureFreeSpace(b, 1) ;
	*BufferLast(b) = '\0' ;
...
static void PSBufferWrite(void)
{
...
static void PNDCurrentBuffer(void)
{
