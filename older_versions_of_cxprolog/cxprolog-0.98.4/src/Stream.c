...
static void StreamBasicGCMark(void)
{
		...
		if( c == '~' ) {
			...
			switch( c ) {
				...
				case 'G': {
					char fmt[2] = { c, '\0' } ;
					StreamPutStr(srm,
						FloatAsStr(fmt, 0, hasArg ? arg : 6,
									XTestFloat(XGetListHead(&list)))) ;
					break ;
				}
				case 'i': {
					dotimes(i, arg)
...
static void PStreamCheck(void)
{
	MustBe( XExtraCheck(streamType, X0) ) ;
}

static void POpenFileStream3(void)
{
...
#if COMPASS
static void POpenFileStream4(void)
{
...
static void POpenBufferStream3(void)
{
...
#if COMPASS
static void POpenBufferStream4(void)
{
	OpenStreamOptions so ;
...
static void POpenNull(void)
{
	BindVarWithExtra(X0, NullStreamOpen()) ;
	JumpNext() ;
}

static void PClose1(void)
{
...
static void PNDCurrentStream3(void)
{
...
static void PNDCurrentStream4(void)
{
...
static void PStreams(void)
{
	ExtraShow(streamType, PStreamsAux) ;
	JumpNext() ;
}

static void PSetUserStreams(void)
{
...
static void PRestoreUserStreams(void)
{
	AliasUnset(tUserInputAtom, userIn) ;
	AliasUnset(tUserOutputAtom, userOut) ;
	AliasUnset(tUserErrorAtom, userErr) ;
	JumpNext() ;
}

static void PSetInput(void)
{
	currIn = XTestStream(X0, mRead) ;
	JumpNext() ;
}

static void PCurrentInput(void)
{
	MustBe( UnifyWithAtomic(X0, TagExtra(streamType, currIn)) ) ;
}

static void PSetOutput(void)
{
	currOut = XTestStream(X0, mWrite) ;
	JumpNext() ;
}

static void PCurrentOutput(void)
{
	MustBe( UnifyWithAtomic(X0, TagExtra(streamType, currOut)) ) ;
}

static void PSee(void)
{
	StreamPt srm ;
...
static void PGetCode(void)
{
	EnsureTextStream(currIn) ;
	MustBe( UnifyWithAtomic(X0, MakeCode(StreamGet(currIn))) ) ;
}

static void PSGetCode(void)
{
	StreamPt srm = XTestStream(X0, mRead) ;
	EnsureTextStream(srm) ;
...
static void PGetSingleChar(void)
{
	MustBe( UnifyWithNumber(X0, MakeCode(StreamGetSingleChar())) ) ;
}

static void PGetCharWithPrompt(void)
{
	WChar c ;
	int arg ;
...
#if !COMPASS
static void PPeekChar(void)
{
	EnsureTextStream(currIn) ;
	MustBe( UnifyWithAtomic(X0, MakeChar(StreamPeek(currIn))) ) ;
}

static void PSPeekChar(void)
{
	StreamPt srm = XTestStream(X0, mRead) ;
	EnsureTextStream(srm) ;
...
static void PPeekCode(void)
{
	EnsureTextStream(currIn) ;
	MustBe( UnifyWithAtomic(X0, MakeCode(StreamPeek(currIn))) ) ;
}

static void PSPeekCode(void)
{
	StreamPt srm = XTestStream(X0, mRead) ;
	EnsureTextStream(srm) ;
...
static void PPeekByte(void)
{
	EnsureBinaryStream(currIn) ;
	MustBe( UnifyWithAtomic(X0, MakeByte(StreamPeek(currIn))) ) ;
}

static void PSPeekByte(void)
{
	StreamPt srm = XTestStream(X0, mRead) ;
	EnsureBinaryStream(srm) ;
...
static void PAtEndOfStream(void)
{
	MustBe( StreamAtEnd(currIn) ) ;
}

static void PSAtEndOfStream(void)
{
	StreamPt srm = XTestStream(X0, mRead) ;
	MustBe( StreamAtEnd(srm) ) ;
}

static void PFlushOutput(void)
{
	StreamFlush(currOut) ;
	JumpNext() ;
}

static void PSFlushOutput(void)
{
	StreamPt srm = XTestStream(X0, mWrite) ;
	StreamFlush(srm) ;
	JumpNext() ;
...
static void PSFlushOutputAll(void)
{
	StreamFlushAll() ;
	JumpNext() ;
}

static void PGet0(void)
{
