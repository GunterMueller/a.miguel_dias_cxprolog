...
Bool InMainOSThread(Bool warn)
{
	if( OSGetTid() == mainOSThreadId )
		return true ;
#if COMPASS
	RefreshProlog() ;
	if( OSGetTid() == mainOSThreadId )
...
static void SendPrologEvent(PrologEvent ev)
{
	if( currJB == nil ) {
...
/* The CxProlog interpreter can only be used by the thread that has created it */
		mainOSThreadId = OSGetTid() ;
		VersionSet("CxProlog", 0, 98, 4, 0, CONTEXTS) ;
		Boot(argc, argv) ;
...
static void PAbort(void)
{
	if( ThreadActiveIsRoot() )
		EventAbort() ;
	else
...
static void PHalt(void)
{
	EventHalt() ;
	JumpNext() ;
}

static void PExitScriptFast(void)
{
	NormalFinish() = false3 ;
	EventExit() ;
	JumpNext() ;
...
static void PCopyright(void)
{
	ShowVersion() ;
	Write("Copyright (C) 1990-2022 A.Miguel Dias, CITI, DI/FCT/UNL\n") ;
...
static void PRestartAll(void)
{
	RestartAll() ;
	JumpNext() ;
}

static void PMoreSuccess(void)
{
	SendPrologEvent(peMoreSuccess) ;
	JumpNext() ;
}

static void PReturnSuccess(void)
{
...
static void PReturnFailure(void)
{
	SendPrologEvent(peReturnFailure) ;
	JumpNext() ;
}

static void PLevel(void)
{
...
static void PSizes(void)
{
	ShowSizes() ;
	JumpNext() ;
}

static void PCrash(void)
{
	Pt p = cPt(1) ;
	*p = 0 ;
	JumpNext() ;
...
static void PStartTimer(void)
{
	OSStartTimer(5, nil) ;
	JumpNext() ;
}

static void PStopTimer(void)
{
	OSStopTimer();
	JumpNext() ;
}

static void PSegmentationFault(void)
{
	Hdl h;
	for(h = 0 ; ; h++ )
		*h = tNilAtom;
...
void CallPrologInit(void)
{
	...
	InstallCBuiltinPred("$$_restart_all", 0, PRestartAll) ;
	...
	InstallCBuiltinPred("$$_segmentation_fault", 0, PSegmentationFault) ;
