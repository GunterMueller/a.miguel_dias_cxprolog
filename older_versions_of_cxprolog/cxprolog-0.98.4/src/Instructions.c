 ...
 *   Copyright (C) 1990-2022 A.Miguel Dias, CITI, DI/FCT/UNL
...
#define FailMesgPos					21
	*c++ = cPt(WordsOf(Environment) + 0) ;	/* 0 cell local-var env */
	*c++ = cPt(FailMesg) ;
#define ContinuePos					23
	*c++ = cPt(WordsOf(Environment) + 0) ;	/* 0 cell local-var env */
	*c++ = cPt(Continue) ;
#define DeallocProceedPos			25
...
static void NopInst(void)
{
	JumpNext() ;
}

static void LocalJumpInst(void)
{
	P = LookHdl() ;
	JumpNext() ;
}

static void FAllocateInst(void)
{
	AllocEnv() ;
	JumpNext() ;
}

static void EnsureFreeSpaceInst(void)
{
	Z.w = GetWord() ;
...
static void ExecuteInst(void)
{
	ExecutePred(LookPred()) ;
	JumpNext() ;
}

static void DeallocExecuteInst(void)
{
	DeallocEnv() ;
	ExecutePred(LookPred()) ;
	JumpNext() ;
...
static void ExecuteVarInst(void)
{
	ExecuteVarPredOpaque(LookupPredicateForMetaCall(PrepareCall(X0))) ;
	JumpNext() ;
}

static void DeallocExecuteVarInst(void)
{
	DeallocEnv() ;
	ExecuteVarPredOpaque(LookupPredicateForMetaCall(PrepareCall(X0))) ;
	JumpNext() ;
...
static void CallCleanupContinuationInst(void)
{
	if( Ef(B0) == Bf(B) ) /* Check if B is aux CP */
		CutTo(Ef(B0)) ;
	Jump(DeallocProceed) ;
}
static void CallCleanupFailInst(void)
{
	CutTo(Ef(B0)) ;
	DeallocEnv() ;
	DoFail() ;
...
static void PutCutLevelInst(void)
{
...
static void ProceedInst(void)
{
	P = CP ;
	JumpNext() ;
}

static void DeallocProceedInst(void)
{
	DeallocEnv() ;
	P = CP ;
	JumpNext() ;
...
static void EmptyPredInst(void)
{
	DoFail() ;
}

static void FailInst(void)
{
	DoFail() ;
}

static void FailMesgInst(void)
{
	DoFail() ;
}

static void ContinueInst(void)
{
	P = Q.h ;
	EventContinue() ;
}

void DelayedContinue(void)
{
	Q.h = P ;
	P = ContinueAddr ;
}
...
static void PutYVariableInst(void)
{
	Q.h = &Y(GetWord()) ;
...
static void PutXVariableInst(void)
{
	Q.h = GetHdl() ;
...
static void PutXVariableOneInst(void)
{
	X(GetHdl()) = PushHVar() ;
	JumpNext() ;
}

static void PutXValueInst(void)
{
	Q.h = GetHdl() ;
	X(GetHdl()) = X(Q.h) ;
	JumpNext() ;
...
static void PutYValueInst(void)
{
	Z.w = GetWord() ;
	X(GetHdl()) = Y(Z.w) ;
	JumpNext() ;
...
static void PutZValueInst(void)
{
	Z.w = GetWord() ;
	X(GetHdl()) = Z(Z.w) ;
	JumpNext() ;
...
static void PutUnsafeValueInst(void)
{
...
static void PutAtomicInst(void)
{
	Q.t = GetPt() ;
	X(GetHdl()) = Q.t ;
	JumpNext() ;
...
static void PutNilInst(void)
{
	X(GetHdl()) = tNilAtom ;
	JumpNext() ;
}

static void PutStructureInst(void)
{
	PushH(GetPt()) ;
...
static void GetYInitInst(void)
{
	Y(GetWord()) = tNilAtom ;
	JumpNext() ;
}

static void GetYVariableInst(void)
{
	Z.w = GetWord() ;
	Y(Z.w) = X(GetHdl()) ;
	JumpNext() ;
...
static void GetXValueInst(void)
{
	Q.h = GetHdl() ;
...
static void GetYValueInst(void)
{
	Z.w = GetWord() ;
	MustBe( Unify(Y(Z.w), X(GetHdl())) ) ;
}

static void GetZValueInst(void)
{
	Z.w = GetWord() ;
	MustBe( Unify(Z(Z.w), X(GetHdl())) ) ;
}

static void GetAtomicInst(void)
{
	Q.t = GetPt() ;
...
static void GetStructureInst(void)
{
...
static void UnifyVoidInst(void)
{
	if( S == nil ) { /* write mode */
		Z.w = GetWord() ;
		while( Z.w-- )
...
static void UnifyVoidOneInst(void)
{
	if( S == nil ) { /* write mode */
		Ignore(PushHVar()) ;
		JumpNext() ;
...
static void UnifyXVariableInst(void)
{
	if( S == nil ) { /* write mode */
		X(GetHdl()) = PushHVar() ;
		JumpNext() ;
...
static void UnifyYVariableInst(void)
{
	if( S == nil ) { /* write mode */
		Y(GetWord()) = PushHVar() ;
		JumpNext() ;
...
static void UnifyXValueInst(void)
{
	if( S == nil ) { /* write mode */
...
static void UnifyYValueInst(void)
{
	if( S == nil ) { /* write mode */
...
static void UnifyXLocalValueInst(void)
{
	if( S == nil ) { /* write mode */
...
static void UnifyYLocalValueInst(void)
{
	if( S == nil ) { /* write mode */
		VarValue2(D, Y(GetWord())) ;
		if( IsVar(D) && IsLocalVar(D) ) {
...
static void UnifyAtomicInst(void)
{
	if( S == nil ) { /* write mode */
		PushH(GetPt()) ;
		JumpNext() ;
...
static void BuildVoidInst(void)
{
	Z.w = GetWord() ;
	while( Z.w-- )
		Ignore(PushHVar()) ;
...
static void BuildVoidOneInst(void)
{
	Ignore(PushHVar()) ;
	JumpNext() ;
}

static void BuildXVariableInst(void)
{
	X(GetHdl()) = PushHVar() ;
	JumpNext() ;
}

static void BuildYVariableInst(void)
{
	Y(GetWord()) = PushHVar() ;
	JumpNext() ;
}

static void BuildXValueInst(void)
{
	PushH(X(GetHdl())) ; /* oc: no. */
	JumpNext() ;
}

static void BuildYValueInst(void)
{
	PushH(Y(GetWord())) ; /* oc: no. */
	JumpNext() ;
}

static void BuildZValueInst(void)
{
	PushH(Z(GetWord())) ; /* oc: no. */
	JumpNext() ;
}

static void BuildXLocalValueInst(void)
{
	VarValue2(D, X(Q.h = GetHdl())) ;
	if( IsVar(D) && IsLocalVar(D) ) {
		Assign(D, X(Q.h) = PushHVar()) ;
...
static void BuildYLocalValueInst(void)
{
	VarValue2(D, Y(GetWord())) ;
	if( IsVar(D) && IsLocalVar(D) ) {
		Assign(D, PushHVar()) ;
...
static void BuildAtomicInst(void)
{
	PushH(GetPt()) ;
	JumpNext() ;
}

static void BuildNilInst(void)
{
	PushH(tNilAtom) ;
	JumpNext() ;
}
...
static void MakeIndexInst(void)
{
	DoIndex(LookPred()) ;
	P-- ;
	JumpNext() ;
...
static void TryMeElseInst(void)
{
...
static void RetryMeElseInst(void)
{
...
static void DoSwitchOnTerm(void)
{
	VarValue(D) ;
	if( IsList(D) )
		P = cHdl(P[1]) ;
...
static void SwitchOnTerm0Inst(void)
{
	D = X0 ;
	DoSwitchOnTerm() ;
	JumpNext() ;
...
static void SwitchOnTerm1Inst(void)
{
	D = X1 ;
	DoSwitchOnTerm() ;
	JumpNext() ;
...
static void SwitchOnTerm2Inst(void)
{
	D = X2 ;
	DoSwitchOnTerm() ;
	JumpNext() ;
...
static void SwitchOnAtomicInst(void)
{
...
static void SwitchOnStructureInst(void)
{
...
static void DiscardAndFailInst(void)
...
static void DebugExitInst(void)
{
	DebugExitCode() ;
	JumpNext() ;
}

static void DebugRedoInst(void)
{
	DebugRedoCode() ;
	JumpNext() ;
}

static void DebugRetryInst(void)
{
	DebugRetryCode() ;
	JumpNext() ;
}
...
InstPt FailAddr, FailMesgAddr, ContinueAddr ;
