...
static void InterruptMenu(void)
{
...
/* On Linux: All signals in the other threads are blocked. So, for
   ...
   InMainOSThread(false) call is not really necessary... but doesn't hurt. */
...
static void CheckThreadedCode(void)
{
	static int *p = nil ;
	int i ;
	if( p != &i ) {
...
{
	InstallCBuiltinPred("$$_interrupt", 0, PInterrupt) ;
}
