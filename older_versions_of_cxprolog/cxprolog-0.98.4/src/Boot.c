...
'$$_top_prompt'(P) :-								\
	...
	context(C),										\
	atom_term(A, C), atom_concat(A, ' ', AA),		\
	concat(['\n',AA,'?- '], P).					\
'$$_top_prompt'(P) :-								\
...
'$top_level' :-										\
	...
	try((G==end_of_file -> halt)),					\
	question('$$_compiled'(G), V).					\
													\
'$top_level2' :-									\
	repeat,											\
		'$env_context' =: C,						\
		G = call_on_context(C,'$top_level'),		\
		catch(try(G),E,								\
			(E=='abort'->true;write_exception(E))),	\
	fail.											\
													\
													\
/* HANDLING CONTEXT */							  ""\
...
void FeaturesShow(void)
{
	...
	Huge total = OSTotalSystemMemory() ;
	Write("   - Total physical memory: %"fHuge"MB\n", total / (1 K K)) ;
...
static void PEnterUserMode(void)
{
	if( !Booting() )
...
static void PDefaultBoot(void)
{
	ZBasicLoadBuiltinsStr(defaultBuiltinsStr) ;
	JumpNext() ;
}

static void PEndOfFile(void)
{
	EventHalt() ;
	JumpNext() ;
}

static void PLoudEndOfFile(void)
{
	WriteEOF() ;
	EventHalt() ;
	JumpNext() ;
...
static void POSName(void)
{
	MustBe( Unify(X0, MakeAtom(OSName())) ) ;
}

static void PShow(void)
{
	ShowVersion() ;
	Write("Show what?\n") ;
...
static void PFeatures(void)
{
	FeaturesShow() ;
	JumpNext() ;
}

static void PLoadLib(void)
{
