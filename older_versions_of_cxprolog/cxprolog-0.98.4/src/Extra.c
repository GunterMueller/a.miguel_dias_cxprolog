...
void ExtraShowSlot(ExtraTypePt e, int slot) {
	Write("%sS:\n", ExtraTypeName(e)) ;
	Write("slot = %d\n", slot) ;
	register ExtraPt x ;
	doextra_slot(e, slot, x) {
...
void BindVarWithExtra(Pt t, VoidPt x)
{
	...
	if( IsVar(t) ) {
		...
		if( IsAtom(t) )
			...
			IVarSetAtomic(XAtom(t), TagExtra(allExtraTypes + ExtraTag(x), x)) ;
#else
			AliasSet(t, x) ;
#endif
			...
			IVarSetAtomic(XTestAtom(XStructArg(t,0)), TagExtra(ExtraTypeOf(x), x)) ;
		else
