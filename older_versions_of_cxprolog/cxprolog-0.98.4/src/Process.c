...
static void PProcessCheck(void)
{
	MustBe( XExtraCheck(processType, X0) ) ;
}

static void PProcessNew(void)
{
	if( ProcessIsChild() )
...
static void PProcessSend(void)
{
	ProcessMesgToFather(XTestNat(X0)) ;
	JumpNext() ;
}

static void PProcessSendMax(void)
{
	MustBe( Unify(X0, MakeInt(OSPipeBufferSize() / sizeof(Message))) ) ;
}

static void PProcessReceive(void)
{
	Message mesg ;
	ProcessPt p ;
	if( ProcessIsChild() )
...
static void PProcessReceiveReady(void)
{
	if( ProcessIsChild() )
		Error("A child process cannot do that") ;
	if( rw[0] == -1 )
...
static void PNDCurrentProcess(void)
{
...
/*
static void PProcessIsChild(void)
{
	MustBe( ProcessIsChild() )
}

static void PProcessDelete(void)
{
