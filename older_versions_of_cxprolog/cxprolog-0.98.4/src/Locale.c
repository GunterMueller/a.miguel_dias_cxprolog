...
static Str EnableSystemLocaleCType(void)
{
	Str loc ; /* cannot optimize because of fail_on_error */
...
static Str EncodingMakeFromLocale(Str l)
{
	...
	if( (enc = strchr(l, '.')) != nil ) {
		switch( enc[1] ) {
			case 'i': {
				if(		StrEqual(enc, ".iso-8859-1")
					||	StrEqual(enc, ".iso_8859-1")
					||	StrEqual(enc, ".iso88591")
					||	StrEqual(enc, ".iso-8859-15")
					||	StrEqual(enc, ".iso_8859-15") 
					||	StrEqual(enc, ".iso885915") 
				)
...
void FilePutUsingCLib(FILE *file, WChar c, Str locale, Bool numEsc, Str fName)
{
#if UNDERSTAND_EXTERNAL_ENCODINGS
	...
	if( (len = wcrtomb(buff, c, &ps)) == -1 ) {
		if( numEsc ) { /* Instead of giving an error... */
			strcpy(buff, CharAsNumericEscape(c)) ;
			len = strlen(buff) ;
		}
		else
			FileError("Could not convert to multibyte sequence on file '%s'",
														fName) ;
	}
