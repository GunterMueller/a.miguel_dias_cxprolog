...
static void IndexTableInit(void)
{
...
static void IndexTableExpand(void)
{
...
static void IndexTableRefresh(void)
{
	if( ++currVersion == 0 )
		IndexTableInit() ;
}
...
#if unused
static void IndexTableList(void)
{
	register IndexElemPt el ;
	dotable(el, indexTable, indexTableCapacity)
								...
								int nVarClauses, Hdl onlyVarChain)
{
	...
	Gen2(SwitchOnAtomic, cIntAsPt(hTSize), onlyVarChain) ;
									...
									int nVarClauses, Hdl onlyVarChain)
{
	...
	Gen2(SwitchOnStructure, cIntAsPt(hTSize), onlyVarChain) ;
	hTable = (PrologHashElemPt)(ScratchCurr()) ;
	ScratchMakeRoom(hTSize * WordsOf(PrologHashElem)) ;
	if( ScratchHasMoved() ) return nil ;
	InitPrologHash(hTable, hTSize) ;
