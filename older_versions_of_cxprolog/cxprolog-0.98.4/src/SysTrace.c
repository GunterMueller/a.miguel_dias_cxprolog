...
#else
	while( sysTrace_flag == 4 && sysTraceStream != nil ) {
...
void SysTraceUpdateFlag(int newValue)
{
	...
	if( sysTrace_flag > 1 )
		Attention() = true ;
	if( Running() && (oldValue == 4 || sysTrace_flag == 4) )
		DelayedContinue() ;	// may change the interpreter
}
...
void SysTraceHandle(PredicatePt pr)
{
	if( sysTrace_flag > 1 ) {
		Attention() = true ;
		if( sysTrace_flag == 2 && (!PredIsBuiltin(pr) || PredIsMeta(pr)) )
			return ;
		if( sysTraceStream != nil ) {
			Str s ;
			HSave() ;
...
{
	if( sysTrace_flag > 0 && sysTraceStream != nil ) {
...
static void PDeleteSysTraceFile(void)
{
	DeleteSysTraceFile() ;
	JumpNext() ;
}
