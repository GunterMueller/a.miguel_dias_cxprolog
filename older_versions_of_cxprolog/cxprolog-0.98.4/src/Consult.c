...
static void PEnterConsult(void)
{
	register UnitPt u ;
...
static void PEnterInclude(void)
{
...
static void PConsulting(void)
{
	Pt t ;
	MustBe( StackTop(consultStack, &t) && Unify(X0, t) ) ;
}

static void PConsultLevel(void)
{
	MustBe( UnifyWithNumber(X0, MakeInt(StackSize(consultStack)/3)) ) ;
}

static void PConsultingIsSilent(void)
{
	MustBe( silentConsulting ) ;
}

static void PConsultClause(void)
{
	AddNewClause(X0, true, true, false) ;
	JumpNext() ;
}

static void PConsultStoreInit(void)
{
	if( consultFile == nil )
		Error("This directive can only be used in consult") ;
