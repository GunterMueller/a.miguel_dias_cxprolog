...
/* HISTORY:
 ...
 000324: added support for extra types: tag extra.
 970408: Added support for cuts within ;/2, not/1, etc.
                        (implementation based on an idea of Rui Marques)
...
static void ProcessGoalArgVars(Pt *t, Bool inMeta)
{
	...
	{
		int n = XStructArity(arg) ;
		Bool meta = inMeta && PredIsMeta(LookupPredicate2(XStructName(arg), n)) ;
		dotimes(i, n)
			ProcessGoalArgVars(&XStructArg(arg,i), meta) ;
	}
...
static void ProcessGoalVars(Pt goal)
{
	...
	{
		int n = XStructArity(goal) ;
		Bool meta = PredIsMeta(LookupPredicate2(XStructName(goal), n)) ;
		dotimes(currArgN, n)
			ProcessGoalArgVars(&XStructArg(goal,currArgN), meta) ;
	}
...
{
	code = codePt = codeBuffer ;
	codeEnd = code + codeBufferSize/sizeof(Word) - 3 ;
}
...
static void CompileCall(CharPt predName, int predArity)
{
	PredicatePt pr = LookupPredicate2(predName, predArity) ;
...
static int CompileGoalRecord(Pt term, int idx)
{
	...
	dotimes(i, nArgs)
	{
		...
		if( IsRecord(t) )
		{
			...
			{
				int j = FindR() ;
				AddInst1(PutCutLevel, OutTemp(j)) ;
				recs[nRecs] = j ;
			}
			else
	...
	dotimes(i, nArgs)
	{
		...
		elif( IsExtra(t) ) AddInst1(BuildExtra, t) ;
...
static void CompileGoalArgs(Hdl args, int nArgs)
{
	...
	dotimes(currArgN, nArgs)
	{
		...
		elif( IsRecord(t) )
		{
			...
			if( IsCutFlag(t) )
			{
				AddInst1(PutCutLevel, OutTemp(currArgN)) ;
		...
		{
			HandlePutConflicts(currArgN) ;
			AddInst2(PutExtra, t, OutTemp(currArgN)) ;
		}
...
static void CompileHeadRecordArgs(Hdl args, int nArgs)
{
	...
	dotimes(i, nArgs)
	{
		...
		elif( IsExtra(t) ) AddInst1(UnifyExtra, t) ;
...
static void CompileHeadArgs(Hdl args, int nArgs)
{
	...
	dotimes(currArgN, nArgs)
	{
		...
		if( IsNilAtom(t) ) AddInst1(GetNil, OutTemp(currArgN)) ;

		elif( IsAtomic(t) ) AddInst2(GetConstant, t, OutTemp(currArgN)) ;
		...
		elif( IsExtra(t) ) AddInst2(GetExtra, t, OutTemp(currArgN)) ;
