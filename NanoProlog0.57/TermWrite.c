...
{	
	...
	elif( IsInt(term) ) { sprintf(strBuffer, "%ld", XInt(term)) ; PutString(strBuffer) ; }
	elif( IsReal(term) ) { sprintf(strBuffer, "%.5g", XReal(term)) ; PutString(strBuffer) ; }
	elif( IsExtra(term) ) WriteExtra(term) ;
	elif( IsList(term) )
	{
	...
	else { sprintf(strBuffer, "<UNKNOWN TAG::%lx>", GetTag(term)) ; PutString(strBuffer) ; }
}
