...
{
	PredicatePt pr = HeapAlloc(WordsOf(Predicate), true) ;
...
static PredicatePt FindPredicate(FunctorPt f)
{
	return( nil ) ;
}
...
CharPt PredNameArity(PredicatePt pr)
{
	if( PredModule(pr) != SourceModule() && PredModule(pr) != sysModule )
		sprintf(strBuffer, "%s%s%s/%d", ModuleName(PredModule(pr)),
			FunctorName(switchFunctor), PredName(pr), PredArity(pr)) ;
	else sprintf(strBuffer, "%s/%d", PredName(pr), PredArity(pr)) ;
	return(strBuffer) ;
}
...
PredicatePt InstallCSysPred(CharPt name, int arity, Proc cPr)
{
	PredicatePt pr ;
...
void FinishInstallingCSysPreds()
{
...
static ClausePt NewClause(PredicatePt pred, Hdl code, int codeLen, Pt term)
{
	ClausePt cl = HeapAlloc(WordsOf(Clause) + codeLen, false) ;
...
static PredicatePt LookupPredicateByTerm(register Pt term)
{
	...
	elif( IsExtra(term) ) Error("Clause head is an element of an extra type") ;
...
{
	PredicatePt pr = cl->owner ;
	ClausePt prev = ClausePrev(cl),
		   next = ClauseNext(cl) ;
