...
void CreateHeapAndBuffers(long heapS, long strBufferS, long codeBufferS)
{
	CharPt all ;
	if( (all = AllocAligned(heapS + strBufferS + codeBufferS)) == nil )
		Error("Not enough memory") ;
	
	heapCurr = heapBegin = cHdl(all) ;
	heapCurr2 = heapEnd = cHdl(all + heapS) ;
	strBuffer = cCharPt(all + heapS) ;
	codeBuffer = cHdl(all + heapS + strBufferS) ;
}
