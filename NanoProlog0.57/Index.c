...
ClausePt InstallClause(PredicatePt pr, Hdl code, int codeSize, Pt term, Bool end)
{
	ClausePt prev, next,
...
static void PatchPredicate(CharPt name, int arity)
{
	ClausePt cl ;
	PredicatePt pr ;
...
static void InsertIndexElem(IndexElem *ie, ClausePt cl, Pt t)
{
	if( FreeIndexElem(ie) )
	{
		ie->idxArg = t ;
...
static Bool InsertIndexTable(IndexTable table, ClausePt cl, Pt t)
{
	int i ;
...
static void SetIndex(PredicatePt pr,Hdl idx)
{
	PredCode(pr) = idx ;
